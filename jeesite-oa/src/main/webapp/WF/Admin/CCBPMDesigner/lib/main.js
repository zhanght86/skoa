...
//ccflow BPMN para
var CCBPM_Shape_Node = "Node";
var CCBPM_Shape_Event = "Event";
var CCBPM_Shape_Gateway = "Gateway";
var CCBPM_Data_FK_Flow = 0;
/**Connector type
...
function showGrid() {
    ...
    var html_Gridvisible = document.getElementById("div_gridvisible");
    if (html_Gridvisible) {
        html_Gridvisible.innerHTML = gridVisible == true ? "隐藏网格" : "显示网格";
    }
        ...
        case STATE_TEXT_EDITING:
            ...
            */
            ...
            if (currentTextEditor.mouseClickedInside(ev)) {
                ...
                //CCBPM edit Node Name
                var ccObj = currentTextEditor.shape;
                if (ccObj != null && ccObj.CCBPM_Shape != null && ccObj.CCBPM_Shape == CCBPM_Shape_Node) {
                    var shapeText = currentTextEditor.editor.getElementsByTagName('textarea')[0].value;
                    $.ajax({
                        type: 'POST',
                        url: "../../Admin/CCBPMDesignerController/ProcessRequest.do",
...
function save(showInfo) {
    ...
    for (var cn in CONNECTOR_MANAGER.connectors) {
        ...
        var rSecondFigure = STACK.figureGetAsSecondFigureForConnector(connector.id);

        if (rFirstFigure == null || rSecondFigure == null) continue;

        //collection BPM Nodes
...
function exportCanvas() {
    ...
    $.post("../../Admin/CCBPMDesignerController/ProcessRequest.do", { action: 'saveSvg', svg: escape(v2) },
        function (data) {
            if (data == 'svg_ok') {
...
function load(diagramId) {
    ...
    $.post("../../Admin/CCBPMDesignerController/ProcessRequest.do", { action: 'load', diagramId: diagramId },
        function (data) {
            //alert(data);
            try {
                if (data == "") return;
...
function loadTempDiagram(tempDiagramName) {
    ...
    $.post("../../Admin/CCBPMDesignerController/ProcessRequest.do", { action: 'loadTemp', tempName: tempDiagramName },
        function (data) {
            //                        alert(data);
            try {
                var obj = eval('(' + data + ')');
...
function loadQuickStartDiagram() {
    $.post("../../Admin/CCBPMDesignerController/ProcessRequest.do", { action: 'loadQuickStart' },
        function (data) {
            //                        alert(data);
            try {
                var obj = eval('(' + data + ')');
...
function saveAs() {
    ...
    //see: http://api.jquery.com/jQuery.post/
    $.post("../../Admin/CCBPMDesignerController/ProcessRequest.do", { action: 'saveAs', diagram: $serializedDiagram, png: dataURL, linkMap: lMap, svg: svgDiagram },
       function (data) {
           if (data == 'noaccount') {
                ...
                switch (state) {
                    case STATE_FIGURE_CREATE:
                        ...
                        if (window.createFigureFunction) {
                            ...
                            //alert(window.createFigureFunction);
