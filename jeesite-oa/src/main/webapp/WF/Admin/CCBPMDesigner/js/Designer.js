...
var CCPMB_Flow_V = "1";//yqh修改 默认1 ccbpm模式

$(function () {
    CCBPM_Data_FK_Flow = getArgsFromHref("FK_Flow");
    CCPMB_Flow_V = getArgsFromHref("Flow_V");//yqh修改 默认1 ccbpm模式
...
function FlowProperty() {
    var url = "../xap/DoPort.jsp?DoType=En&EnName=BP.WF.Template.FlowExt&PK=" + CCBPM_Data_FK_Flow + "&Lang=CH";
    if (window.parent) {
    ...
    switch (item.name) {
        case "linecondition":
            ...
            if (rFirstFigure.CCBPM_Shape == CCBPM_Shape_Node && rSecondFigure.CCBPM_Shape == CCBPM_Shape_Node) {
                ...
                var url = "../ConditionLine.jsp?FK_Flow=" + CCBPM_Data_FK_Flow + "&FK_MainNode=" + fNode + "&FK_Node=" + fNode + "&ToNodeID=" + tNode + "&CondType=2&Lang=CH";
...
function NodeProperty_Funs(item) {
    ...
    switch (item.name) {
        case "NodePropertyNew": //节点属性.
            url = "../xap/DoPort.jsp?DoType=En&EnName=BP.WF.Template.NodeExt&PK=" + FK_Node + "&Lang=CH";
            if (window.parent) {
        ...
        case "NodeProperty": //节点属性.
            url = "../xap/DoPort.jsp?DoType=En&EnName=BP.WF.Node&PK=" + FK_Node + "&Lang=CH";
            if (window.parent) {
                window.parent.addTab(FK_Node + "PO", "节点属性" + FK_Node, url, item.iconCls);
            } else {
                WinOpen(url);
        ...
        case "NodeAccepterRole": // 工作处理人. NodeFromWorkModel
            url = "../FindWorker/NodeAccepterRole.jsp?FK_Node=" + FK_Node + "&Lang=CH";
            if (window.parent) {
        ...
        case "BindStations":  //绑定岗位.
            url = "../xap/DoPort.jsp?DoType=StaDefNew&PK=" + FK_Node + "&Lang=CH";
            WinOpenIt(url, 500, 400 );
            break;
           // Glo.OpenDialog(Glo.BPMHost + url, "执行", 500, 400);
        case "NodeCCRole": // 抄送人规则.
            url = "../FindWorker/NodeCCRole.jsp?FK_Node=" + FK_Node + "&Lang=CH";
            if (window.parent) {
            ...
            break;
        case "NodeEvent": // 节点事件.
            url = "./../Action.jsp?NodeID=" + FK_Node + "&FK_Flow=" + CCBPM_Data_FK_Flow;
            if (window.parent) {
        ...
        case "FlowCompleteCond": // 流程完成条件..
            url = "../Cond.jsp?CondType=1&FK_Flow=" + CCBPM_Data_FK_Flow + "&FK_MainNode=" + FK_Node + "&FK_Node=" + FK_Node + "&FK_Attr=&DirType=&ToNodeID=" + FK_Node;
            if (window.parent) {
        ...
        case "AdvFunc": // 高级功能..
        case "Node": // 批量设置 ..
            url = "../AttrFlow/NodeAttrs.jsp?CondType=1&FK_Flow=" + CCBPM_Data_FK_Flow + "&FK_MainNode=" + FK_Node + "&FK_Node=" + FK_Node + "&FK_Attr=&DirType=&ToNodeID=" + FK_Node;
            //alert(url);
            if (window.parent) {
            ...
            break;
        case "Listion": // 消息收听..
            url = "/WF/Admin/Listen.jsp?CondType=1&FK_Flow=" + CCBPM_Data_FK_Flow + "&FK_MainNode=" + FK_Node + "&FK_Node=" + FK_Node + "&FK_Attr=&DirType=&ToNodeID=" + FK_Node;
            if (window.parent) {
        ...
        case "SelfToolbar": // 自定义工具栏..
            url = "/WF/Admin/Cond.jsp?CondType=1&FK_Flow=" + CCBPM_Data_FK_Flow + "&FK_MainNode=" + FK_Node + "&FK_Node=" + FK_Node + "&FK_Attr=&DirType=&ToNodeID=" + FK_Node;
            if (window.parent) {
        ...
        case "NodeFromWorkModel": // 设置表单. NodeFromWorkModel
            url = "../AttrNode/NodeFromWorkModel.jsp?FK_Node=" + FK_Node + "&Lang=CH";
            if (window.parent) {
        ...
        case "DesignerNodeFormSL": //设计表单
            //url = "../CCFormDesigner/FormDesigner.jsp?FK_MapData=ND" + FK_Node + "&UserNo=" + window.parent.WebUser.No + "&SID=" + window.parent.WebUser.SID;
            url = "../../MapDef/CCForm/Frm.jsp?FK_Flow=" + CCBPM_Data_FK_Flow + "&FK_MapData=ND" + FK_Node + "&UserNo=" + window.parent.WebUser.No + "&SID=" + window.parent.WebUser.SID;
        ...
        case "bindflowfrms": //绑定独立表单
            url = "../BindFrms.jsp?ShowType=FlowFrms&FK_Flow=" + CCBPM_Data_FK_Flow + "&FK_Node=" + FK_Node + "&Lang=CH";
            if (window.parent) {
...
//运行流程
function Run_Flow() {
    var url = "../TestFlow.jsp?FK_Flow=" + CCBPM_Data_FK_Flow + "&Lang=CH";
    WinOpen(url);
}

//检查流程
function Check_Flow() {
    var url = "../DoType.jsp?RefNo=" + CCBPM_Data_FK_Flow + "&DoType=FlowCheck&Lang=CH";
    if (window.parent) {
