...
A.PFlowNo,
B.FK_Node, B.FK_NodeText AS NodeName,B.FK_Dept as WorkerDept, A.Title, A.RDT, B.RDT AS ADT, 
...
1 as MyNum
  FROM WF_GenerWorkFlow A, WF_CCList B WHERE A.WorkID=B.WorkID AND  B.Sta <=1 AND B.InEmpWorks = 1;
...
ROUND( (SELECT  cast(count(MyPK) AS float) AS Num FROM WF_CH A WHERE CHSta <=1 AND A.FK_Emp=WF_CH.FK_Emp)/(SELECT count(MyPK) AS Num FROM WF_CH A WHERE  A.FK_Emp=WF_CH.FK_Emp)*100,2) 
AS WCRate
...
ROUND( (SELECT  cast(count(MyPK) AS float) AS Num FROM WF_CH A WHERE CHSta <=1 AND A.FK_Emp=WF_CH.FK_Emp AND A.FK_NY=WF_CH.FK_NY )/(SELECT count(MyPK) AS Num FROM WF_CH A WHERE  A.FK_Emp=WF_CH.FK_Emp AND A.FK_NY=WF_CH.FK_NY)*100,2) 
AS WCRate
