...
function querydata(method, kw) {
    var haveSub =  $("#chkHaveSubDept").prop("checked");
    var haveSame = $('#chkHaveSameLevelDept').prop("checked");

    $.ajax({
        type: "POST",
        url: "<%=basePath%>WF/WorkOpt/SearchDepts.do?s=" + Math.random(),
        dataType: 'json',
...
<body class="easyui-layout">
	<form method="post" action="" id="form1">
		<div data-options="region:'north',noheader:true"
			style="overflow-y: hidden; height: 35px; padding: 5px; background-color: #E0ECFF;">
			关键字：<input type="text" id="txtKeyword" maxlength="10"
				style="width: 100px" /> 搜索：<input type="checkbox"
				id="chkHaveSubDept" /><label for="chkHaveSubDept">子部门</label> <input
				type="checkbox" id="chkHaveSameLevelDept" /><label
				for="chkHaveSameLevelDept">同级部门</label> <a href="javascript:void(0)"
				class="easyui-linkbutton" data-options="iconCls:'icon-search'"
				onclick="search1()">搜索</a> <a href="javascript:void(0)"
				class="easyui-linkbutton" onclick="searchAll()">所有</a>
		</div>
		<div data-options="region:'west',title:'字母分组',split:true"
			style="overflow-x: hidden; padding: 5px; width: 145px">
			<ul class="alphalist">
				<li>A</li>
				<li>B</li>
				<li>C</li>
		...
		<div
			data-options="region:'center',title:'部门',tools:[{ iconCls:'icon-ok', handler:selectDeptAll }]"
			style="padding: 5px">
			<ol id="tbDepts" class="deptlist">
			</ol>
		</div>
		<div
			data-options="region:'east',title:'已选部门',split:true,tools:[{ iconCls:'icon-delete', handler:unSelectDeptAll }]"
