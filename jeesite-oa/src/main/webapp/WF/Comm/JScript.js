...
function OpenHelperTBNo(appPath, EnsName, ctl) {
    var url = appPath + '/Comm/DataHelp.htm?' + appPath + '/Comm/HelperOfTBNo.jsp?EnsName=' + EnsName;
...
function OpenItme3(webAppPath, className, url) {
    var url = webAppPath + "/Comm/" + 'Item3.jsp?EnName=' + className + url;
...
function DefaultVal1(appPath, ctrl, className, attrKey, empId) {
    if (event.button != 2)
        return;
    url = appPath + '/Comm/DataHelp.htm?' + appPath + '/Comm/HelperOfTB.jsp?EnsName=' + className + '&AttrKey=' + attrKey + '&empId=' + empId;
    str = ctrl.value;
...
function HalperOfDDL(appPath, EnsName, refkeyvalue, reftext, ddlID) {
    var url = ''; // appPath+"/Comm/DataHelp.htm?HelperOfDDL.jsp?EnsName="+EnsName+"&RefKey="+refkeyvalue+"&RefText="+reftext;
    // modified by ZhouYue 2013-05-20  changed "/DataHelp.htm..." to "../DataHelp.htm"
    if (appPath == '/')
        url = "/WF/Comm/DataHelp.htm?HelperOfDDL.jsp?EnsName=" + EnsName + "&RefKey=" + refkeyvalue + "&RefText=" + reftext;
    else
        url = appPath + "/Comm/DataHelp.htm?HelperOfDDL.jsp?EnsName=" + EnsName + "&RefKey=" + refkeyvalue + "&RefText=" + reftext;
...
function onDDLSelectedMore(ddlID, MainEns, EnsName, refkeyvalue, reftext) {
    var url = '';
    url = "../Comm/DataHelp.htm?HelperOfDDL.jsp?EnsName=" + EnsName + "&RefKey=" + refkeyvalue + "&RefText=" + reftext + "&MainEns=" + MainEns + "&DDLID=" + ddlID;
...
function WorkCheckTBHelp(ctrl, op) {
    var url = "Comm/HelperOfTBEUI.jsp";
...
function SigantureAct(ele, UserNo, FK_MapData, KeyOfEn, WorkID) {
    if (ele) {
        ...
        $.ajax({
            type: "get",
            url: localHref + "/WF/Comm/HelperOfSiganture.jsp",
            data: json_data,
