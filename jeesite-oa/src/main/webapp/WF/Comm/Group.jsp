...
<%
	IndexGroupModel gm = new IndexGroupModel(request, response);
...
<body class="easyui-layout">
	<form method="post" id="form1" style="height: 100%">
		<div id="mainDiv"
			data-options="region:'center',title:'分组分析 '"
			style="padding: 5px">
...
<script type="text/javascript">
	function ddl_SelectedIndexChanged_Goto(){
		var item = $("#GoTo").val();
	    window.location="<%=basePath%>WF/Rpt/" + item + ".jsp?RptNo=<%=rptNo%>&FK_Flow=<%=fkFlow%>";
	}
	function changeSelectedCheckBox(){
		State_Changed();
	}
	function State_Changed(event){
		$('#form1').attr("action", "<%=basePath%>WF/Rpt/Group.jsp?RptNo=<%=rptNo%>&OrderBy=<%=gm.getOrderBy()%>&OrderWay=<%=gm.getOrderWay()%>&NumKey=<%=gm.getNumKey()%>&FK_Flow=<%=fkFlow%>&isPostBack=true");
		$('#form1').submit();
	}
	function ToolBar1_ButtonClick(btnName){
		$('#form1').attr("action", "<%=basePath%>WF/Rpt/Group.jsp?RptNo=<%=rptNo%>&OrderBy=<%=gm.getOrderBy()%>&OrderWay=<%=gm.getOrderWay()%>&NumKey=<%=gm.getNumKey()%>&FK_Flow=<%=fkFlow%>&isPostBack=true&btnName=" + btnName);
		$('#form1').submit();
	}
