<%@page import="cn.jflow.model.wf.mapdef.MapDefDtlFreeFrmModel"%>
<%@page import="BP.Sys.Frm.MapData"%>
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
 <%
 MapDefDtlFreeFrmModel md = new MapDefDtlFreeFrmModel(request, response);
 if(!md.load()){
	 return;
 }
 MapDtl dtl = md.getDtl();
...
<script language="javascript">
    ...
    {
        var url='EditTable.jsp?DoType=Edit&MyPK=' + fk_mapdata + '&RefNo=' + attrMyPK;
        var b=window.showModalDialog( url , 'ass' ,'dialogHeight: 500px; dialogWidth: 700px;center: yes; help: no'); 
        window.location.href = window.location.href;
    }
...
<body>
        ...
        <div data-options="region:'north',noheader:true,split:false,border:false" style='height:30px;overflow-y:hidden'>
            <div style='float:left'>
                <a href="javascript:EditDtl('<%=md.fkMapData %>','<%=dtl.getNo() %>')" class='easyui-linkbutton' data-options="iconCls:'icon-edit',plain:true"><%=dtl.getName() %></a>
            </div>
            <div style='float:right'>
                <a href="javascript:document.getElementById('F<%=dtl.getNo() %>').contentWindow.AddF('<%=dtl.getNo() %>');" class='easyui-linkbutton' data-options="iconCls:'icon-new',plain:true">插入列</a>
                <a href="javascript:document.getElementById('F<%=dtl.getNo() %>').contentWindow.AddFGroup('<%=dtl.getNo() %>');" class='easyui-linkbutton' data-options="iconCls:'icon-new',plain:true">插入列组</a>
                <a href="javascript:document.getElementById('F<%=dtl.getNo() %>').contentWindow.CopyF('<%=dtl.getNo() %>');" class='easyui-linkbutton' data-options="iconCls:'icon-add',plain:true">复制列</a>
                <a href="javascript:document.getElementById('F<%=dtl.getNo() %>').contentWindow.HidAttr('<%=dtl.getNo() %>');" class='easyui-linkbutton' data-options="iconCls:'icon-add',plain:true">隐藏列</a>
                <a href="javascript:document.getElementById('F<%=dtl.getNo() %>').contentWindow.DtlMTR('<%=dtl.getNo() %>');" class='easyui-linkbutton' data-options="iconCls:'icon-add',plain:true">多表头</a>
                <a href='Action.jsp?FK_MapData=<%=md.fkMapDtl %>' class='easyui-linkbutton' data-options="iconCls:'icon-add',plain:true">从表事件</a>
                <a href="javascript:DtlDoUp('<%=dtl.getNo() %>')" class='easyui-linkbutton' data-options="iconCls:'icon-up',plain:true"></a>
                <a href="javascript:DtlDoDown('<%=dtl.getNo() %>')" class='easyui-linkbutton' data-options="iconCls:'icon-down',plain:true"></a>
            </div>
            <div style='clear:both'></div>
        </div>
        <div data-options="region:'center',noheader:true,border:false" style="overflow-y:hidden">
            <iframe ID='F<%=dtl.getNo() %>' frameborder='0' scrolling="auto" style='width:100%;height:100%' src='MapDtlDe.jsp?DoType=Edit&FK_MapData=<%=md.fkMapData %>&FK_MapDtl=<%=dtl.getNo() %>'></iframe>
        </div>
			</div>
</body>
</html>