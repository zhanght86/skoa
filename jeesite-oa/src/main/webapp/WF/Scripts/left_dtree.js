...
dTree.prototype.toString = function() {
	...
	c.document.open("text/plain");
	c.document.write(str);
	*/
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		...
		str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="' + node.url + '" target="'+'main'+'"';
...
dTree.prototype.s = function(id) {
	...
	var oHref = document.getElementById("s" + this.obj + id);

	if(oHref != null)
	{
		dTree.TreeNode.OnNodeClick(oHref.getAttribute("href"));
	}
...
dTree.TreeNode.OnNodeClick = function(sUrl)
{
    if(sUrl != null && sUrl.length > 0 && sUrl.substring(0, 1) != "#")
    {
        var s = "javascript:";

        if(sUrl.length > s.length && sUrl.substring(0, s.length) == s)
        {
            (new Function(sUrl.substring(s.length)))();
        }
        else
		{
            showWaitDialog();
