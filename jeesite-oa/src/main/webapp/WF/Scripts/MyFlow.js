...
function OpenAccepter() {
    ...
    var url = './CCForm/FrmPopVal.jsp?FK_MapExt=' + popNameInXML + '&CtrlVal=' + ctrl.value;
...
function DoSubFlowReturn(fid, workid, fk_node) {
    var url = 'ReturnWorkSubFlowToFHL.jsp?FID=' + fid + '&WorkID=' + workid + '&FK_Node=' + fk_node;
...
function DoDelSubFlow(fk_flow, workid) {
    if (window.confirm('您确定要终止进程吗？') == false)
        return;
    var url = 'Do.jsp?DoType=DelSubFlow&FK_Flow=' + fk_flow + '&WorkID=' + workid;
