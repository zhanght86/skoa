...
public class SwaggerConfig {
	...
	private ApiInfo statelessRestfullApiInfo() {
		...
		sb.append("2.客户端需要在request headers中传递appId,timestamp,nonce,digest头信息,还有版本api-version=v1请求头.").append("<br />");
		sb.append("3.1 客户端按nonce+appKey+timestamp字符串拼装,产生秘钥").append("<br />");
		sb.append("3.2 客户端封装待调用api的参数进map集合,该map集合按照key:value形式进行字符串拼装,形如括号里内容:(content:内容是什么param1:1param12param11param2:param2projectName:项目标题),再拼装上当前调用的url路径(例如，/api/userDemo/hello),所产生的待字符串,使用3.1中的密钥对该字符串进行HmacSHA256信息摘要.").append("<br />");
		sb.append("4 客户端也可以把json序列化字符串至http请求体中,请求头digest的算法为3.1产生的秘钥对json序列化后的字符串及当前uri路径的拼装的字符串进行摘要算法,此时的请求头中还需指定Content-Type:application/json.").append("<br />");
