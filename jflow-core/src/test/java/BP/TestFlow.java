...
//		{
//			throw new RuntimeException(WebUser.getNo() + "启动财务报销流程，下一步的节点, 应该是102.现在是:" + objs.VarToNodeID);
//		}
//
//
//		//检查节点表单表是否有数据？
...
//		{
//			throw new RuntimeException("@撤销发送后的数据不符合预期: 没有停留到开始节点上,而是停留在了:" + gwf.FK_Node);
//		}
//
...
//		}
//
//		int je = Integer.parseInt(dt.Rows[0][0].toString());
//		if (je != 900)
//		{
//			throw new RuntimeException("@ht 数据没有传入到节点表里面去,注意如果是自动填充字段将不能写入里面。");
//		}
//
//		if ( ! objs.VarAcceptersID.equals("yangyilei"))
//		{
//			throw new RuntimeException("@当合计费用小于 10000 时，执行结果不符合预期. 应该提交给财务部经理yangyilei审批.现在是:" + objs.VarAcceptersID);
//		}
//
//		//检查yangyilei 是否有待办？
//		sql = "SELECT FK_Emp FROM WF_EmpWorks WHERE WorkID=" + workid;
//		dt = DBAccess.RunSQLReturnTable(sql);
//		if (dt.Rows.size() != 1 || !dt.Rows[0][0].toString().equals(objs.VarAcceptersID))
//		{
...
//		sql = "SELECT FK_Emp FROM WF_EmpWorks WHERE WorkID=" + workid;
//		dt = DBAccess.RunSQLReturnTable(sql);
//		if (dt.Rows.size() != 1 || !dt.Rows[0][0].toString().equals("qifenglin"))
//		{
...
//		if ( ! objs.VarAcceptersID.equals("zhoupeng"))
//		{
//			throw new RuntimeException("@在节点2时,数据大于1万,方向条件不符合预期, 应该执行到zhoupeng 现在转入了:" + objs.VarAcceptersID);
//		}
//
//		//检查zhoupeng 是否有待办？
//		sql = "SELECT FK_Emp FROM WF_EmpWorks WHERE WorkID=" + workid;
...
//		}
//
//		if (!dt.Rows[0][0].toString().equals("zhoupeng"))
//		{
...
//		if (gwf.FK_Node != 103)
//		{
//			throw new RuntimeException("@方向条件没有转入到 103上，目前转入到了:" + gwf.FK_Node);
//		}
//
...
//		sql = "SELECT FK_Emp FROM WF_EmpWorks WHERE WorkID=" + workid;
//		dt = DBAccess.RunSQLReturnTable(sql);
//		if (dt.Rows.size() != 1 || !dt.Rows[0][0].toString().equals("yangyilei"))
//		{
//			throw new RuntimeException("@应该发送到 yangyilei 但是没有发送给他.");
//		}
//
...
//		dt = DBAccess.RunSQLReturnTable(sql);
//		if (!dt.Rows[0]["FlowEnder"].toString().equals("zhoupeng"))
//		{
//			throw new RuntimeException("@应该是 zhoupeng 是 FlowEnder .");
//		}
//
//		if (!dt.Rows[0]["FlowStarter"].toString().equals("zhoutianjiao"))
//		{
//			throw new RuntimeException("@应该是 zhoutianjiao 是 FlowStarter .");
//		}
//
//		if (!dt.Rows[0]["FlowEndNode"].toString().equals("104"))
//		{
//			throw new RuntimeException("@应该是 104 是 FlowEndNode .");
//		}
//
//		if (Integer.parseInt(dt.Rows[0]["WFState"].toString()) != (int)WFState.Runing)
//		{
//			throw new RuntimeException("@应该是 WFState.Runing 是 WFState .");
//		}
//
//		if (Integer.parseInt(dt.Rows[0]["FID"].toString()) != 0)
//		{
//			throw new RuntimeException("@应该是 FID =0");
//		}
//
//		if (!dt.Rows[0]["FK_NY"].toString().equals(DataType.getCurrentYearMonth()))
//		{
...
//		sql = "SELECT FlowEnder,FlowEndNode,FlowStarter,WFState,FID,FK_NY FROM  ND1Rpt WHERE OID=" + workid;
//		dt = DBAccess.RunSQLReturnTable(sql);
//		if (!dt.Rows[0]["FlowEnder"].toString().equals("yangyilei"))
//		{
...
//		}
//
//		if (Integer.parseInt(dt.Rows[0]["WFState"].toString()) != (int)WFState.Complete)
//		{
