...
public class Send003 {
	...
	public Send003() {
		...
		// this.DescIt = "流程:003最简单的3节点(轨迹模式),执行发送后的数据是否符合预期要求.";
	...
	/**
	 * 过程说明： 1，以流程 003最简单的3节点(轨迹模式)， 为测试用例。 2，仅仅测试发送功能，与检查发送后的数据是否完整. 3,
	...
	public void Do() {
		...
		// 执行第1步骤. 让11197,王文兵 发起流程.
		this.Step1();

		// 执行第2步骤. 让 14893,王文革处理.
		this.Step2();
	...
	public final void Step1() {
		...
		// /#region 检查创建工作是否符合预期.
		// 检查开始节点写入的数据是否正确？
		String sql = "SELECT * FROM ND301 WHERE OID=" + workID;
		...
		for (DataColumn dc : dt.Columns) {
			...
			{
				if (!val.equals("301")) {
					throw new RuntimeException("@应当是 301, 现在是:" + val);
				}
			}
		...
		if (!objs.getVarAcceptersID().contains("14893")) {
			throw new RuntimeException("@下一步的接受人不正确, 应当是: 14893.现在是:"
					+ objs.getVarAcceptersID());
		}

		if (objs.getVarToNodeID() != 302) {
			throw new RuntimeException("@应该是 302 节点. 现在是:" + objs.getVarToNodeID());
		}
		...
		// 判断当前点.
		if (gwf.getFK_Node() != 302) {
			throw new RuntimeException("@当前点应该是 302 现在是:" + gwf.getFK_Node());
		}
		...
		// 判断TodoEmps.
		if (!gwf.getTodoEmps().equals("14893,王文革;")) {
			throw new RuntimeException("@当前点应该是 TodoEmps=14893,周朋;  现在是:"
					+ gwf.getTodoEmps());
		}
		...
		// /#region 第3步: 检查节点数据表.
		sql = "SELECT * FROM ND301 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到开始节点的数据.");
		}
		...
		// 检查节点2的数据.
		sql = "SELECT * FROM ND302 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到 ND302 的数据.");
		}
		for (DataColumn dc : dt.Columns) {
			String val = dt.getValue(0,dc.ColumnName).toString();
			if(dc.ColumnName.equals(WorkAttr.CDT)){
				...
				if (val.contains("14893") == false) {
					throw new RuntimeException("第二步骤的处理人员,应当14893 ,现在是:"
							+ val);
				}
			}else if(dc.ColumnName.equals(WorkAttr.FID)){
				if (!val.equals("0")) {
				...
				if (!val.equals("14893")) {
					throw new RuntimeException("应当 Rec=14893,现在是:" + val);
				}
			}
		}
		...
		for (DataColumn dc : dt.Columns) {
			...
			if(dc.ColumnName.equals(GERptAttr.FID)){
				...
				if (!val.equals("302")) {
					throw new RuntimeException("@应当是 302, 现在是:" + val);
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowStarter)){
				if (!val.equals(BP.Web.WebUser.getNo())) {
					throw new RuntimeException(
	...
	public final void Step2() {
		// 让 zhouepng 登录.
		BP.WF.Dev2Interface.Port_Login("14893");
		...
		// /#region 第1步: 检查发送对象.
		// 从获取的发送对象里获取到下一个工作者. zhangyifan(张一帆)、zhoushengyu(周升雨).
		if (!objs.getVarAcceptersID().equals("11197")) {
			throw new RuntimeException("@下一步的接受人不正确, 应当是: 11197.现在是:"
					+ objs.getVarAcceptersID());
		}

		if (objs.getVarToNodeID() != 303) {
			throw new RuntimeException("@应该是 303 节点. 现在是:" + objs.getVarToNodeID());
		}
		...
		// 判断当前点.
		if (gwf.getFK_Node() != 303) {
			throw new RuntimeException("@当前点应该是 303 现在是:" + gwf.getFK_Node());
		}
		...
		if (!gwl.getSender().contains("11197")) {
			throw new RuntimeException("@应该是 包含当前状态 . ");
		}
		...
		for (DataColumn dc : dt.Columns) {
			...
			if(dc.ColumnName.equals(WorkAttr.CDT)){
				...
				if (!val.equals(objs.getVarAcceptersID())) {
					throw new RuntimeException("应当 Rec=11197,现在是:"
							+ val);
				}
			}
		}
		...
		// 检查节点3的数据.
		sql = "SELECT * FROM ND303 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到 ND303 的数据.");
		}
		for (DataColumn dc : dt.Columns) {
			String val = dt.getValue(0,dc.ColumnName).toString();
			if(dc.ColumnName.equals(WorkAttr.CDT)){
		...
		for (DataColumn dc : dt.Columns) {
			...
			if(dc.ColumnName.equals(GERptAttr.FID)){
				...
				BP.Port.Emp emp = new BP.Port.Emp("11197");
				...
				if (!val.equals("303")) {
					throw new RuntimeException("@应当是 303, 现在是:" + val);
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowStarter)){
				if (val.equals(BP.Web.WebUser.getNo())) {
					throw new RuntimeException(
	...
	public final void Step3() {
		// 让 11197 登录.
		BP.WF.Dev2Interface.Port_Login("11197");
...
//		}
		...
		for (DataColumn dc : dt.Columns) {
			...
			if(dc.ColumnName.equals(GERptAttr.FID)){
				...
				if (BP.WF.Glo.getUserInfoShowModel() != UserInfoShowModel.UserNameOnly) {
					if (val.contains("11197") == false
							|| val.contains("14893") == false) {
						throw new RuntimeException("@应当包含的人员，现在不存在, 现在是:" + val);
					}
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowEnder)){
				if (!val.equals("11197")) {
					throw new RuntimeException("@应当是 11197 , 现在是:"
							+ val);
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowEnderRDT)){
				if (val.contains(DataType.getCurrentData()) == false) {
				...
				if (!val.equals("11197")) {
					throw new RuntimeException("@应当是 11197, 现在是:" + val);
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowStartRDT)){
				if (StringHelper.isNullOrEmpty(val)) {
					throw new RuntimeException("@应当不能为空,现在是:" + val);
