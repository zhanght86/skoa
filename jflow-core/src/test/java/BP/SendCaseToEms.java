...
//		this.EditState = CT.EditState.UnOK;
	...
	*/
	...
	public void Do()
	{
		...
		if (wls.size() != 3)
		{
			throw new RuntimeException("@应当有三个数据，现在是:" + wls.size());
		}
		for (GenerWorkerList wl : GenerWorkerLists.convertGenerWorkerLists(wls))
		{
			...
			if (wl.getFK_Emp().equals("zhanghaicheng"))
			{
				...
				if (wl.getSender() != WebUser.getNo() + ",张海成")
				{
					throw new RuntimeException("@ Sender 错误,应该是" + WebUser.getNo() + ",张海成 ,现在是:" + wl.getSender());
				}
			}
