...
public class Send014 {
	...
	public Send014() {
		...
		// this.DescIt = "流程:014最简单的3节点(轨迹模式),执行发送后的数据是否符合预期要求.";
	...
	/**
	 * 过程说明： 1，以流程 014最简单的3节点(轨迹模式)， 为测试用例。 2，仅仅测试发送功能，与检查发送后的数据是否完整. 3,
	...
	public final void Step1() {
		...
		// /#region 检查创建工作是否符合预期.
		// 检查开始节点写入的数据是否正确？
		String sql = "SELECT * FROM ND1401 WHERE OID=" + workID;
		...
		for (DataColumn dc : dt.Columns) {
			...
			{
				if (!val.equals("1401")) {
					throw new RuntimeException("@应当是 1401, 现在是:" + val);
				}
			}
		...
		if (objs.getVarToNodeID() != 1402) {
			throw new RuntimeException("@应该是 1402 节点. 现在是:" + objs.getVarToNodeID());
		}
		...
		// 判断当前点.
		if (gwf.getFK_Node() != 1402) {
			throw new RuntimeException("@当前点应该是 1402 现在是:" + gwf.getFK_Node());
		}
		...
		// /#region 第3步: 检查节点数据表.
		sql = "SELECT * FROM ND1401 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到开始节点的数据.");
		}
		...
		// 检查节点2的数据.
		sql = "SELECT * FROM ND1402 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到 ND1402 的数据.");
		}
		for (DataColumn dc : dt.Columns) {
			String val = dt.getValue(0,dc.ColumnName).toString();
			if(dc.ColumnName.equals(WorkAttr.CDT)){
		...
		for (DataColumn dc : dt.Columns) {
			...
			if(dc.ColumnName.equals(GERptAttr.FID)){
				...
				if (!val.equals("1402")) {
					throw new RuntimeException("@应当是 1402, 现在是:" + val);
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowStarter)){
				if (!val.equals(BP.Web.WebUser.getNo())) {
					throw new RuntimeException(
	...
	public final void Step2() {
		...
		if (objs.getVarToNodeID() != 1499) {
			throw new RuntimeException("@应该是 1499 节点. 现在是:" + objs.getVarToNodeID());
		}
		...
		// 判断当前点.
		if (gwf.getFK_Node() != 1499) {
			throw new RuntimeException("@当前点应该是 1499 现在是:" + gwf.getFK_Node());
		}
		...
		// 检查节点3的数据.
		sql = "SELECT * FROM ND1499 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到 ND1499 的数据.");
		}
		for (DataColumn dc : dt.Columns) {
			String val = dt.getValue(0,dc.ColumnName).toString();
			if(dc.ColumnName.equals(WorkAttr.CDT)){
		...
		for (DataColumn dc : dt.Columns) {
			...
			if(dc.ColumnName.equals(GERptAttr.FID)){
				...
				if (!val.equals("1499")) {
					throw new RuntimeException("@应当是 1499, 现在是:" + val);
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowStarter)){
				if (val.equals(BP.Web.WebUser.getNo())) {
					throw new RuntimeException(
