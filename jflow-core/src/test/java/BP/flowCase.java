...
//		}
		...
		//检查节点表单表是否有数据？
		sql = "SELECT HeJi FROM ND102 WHERE OID=" + workid;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1)
		{
		...
		int je = Float.valueOf(dt.getValue(0, 0).toString()).intValue();
		if (je != 900)
		{
		...
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt==null || dt.Rows.size() != 1 || !dt.getValue(0, 0).toString().equals(objs.getVarAcceptersID()))
		{
		...
		sql = "SELECT FK_Emp FROM WF_EmpWorks WHERE WorkID=" + workid;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1 || !dt.getValue(0, 0).toString().equals("zhoupeng"))
		{
			throw new RuntimeException("@在节点2执行撤销时,数据不符合预期:  zhoupeng.");
		}
		///#endregion
		...
		sql = "SELECT HeJi FROM ND103 WHERE OID=" + workid;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1)
		{
		...
		dt = DBAccess.RunSQLReturnTable(sql);
		if (!dt.getValue(0, "FlowEnder").toString().equals("周朋"))
		{
		...
		if (Integer.parseInt(dt.getValue(0, "WFState").toString()) != WFState.Runing.ordinal())
		{
...
//		gwf.setWFState(WFState.Complete);
		...
		{
			throw new RuntimeException("@ 流程结束后 WF_GenerWorkFlow 没有Complete. ");
		}
		...
		sql = "SELECT FlowEnder,FlowEndNode,FlowStarter,WFState,FID,FK_NY FROM  ND1Rpt WHERE OID=" + workid;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (!dt.getValue(0, "FlowEnder").toString().equals("杨依雷"))
		{
		...
		if (Integer.parseInt(dt.getValue(0, "WFState").toString()) != WFState.Complete.ordinal())
		{
