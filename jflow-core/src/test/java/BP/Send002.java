...
public class Send002 {
	...
	public Send002() {
		...
		// this.DescIt = "流程:002最简单的3节点(轨迹模式),执行发送后的数据是否符合预期要求.";
	...
	/**
	 * 过程说明： 1，以流程 002最简单的3节点(轨迹模式)， 为测试用例。 2，仅仅测试发送功能，与检查发送后的数据是否完整. 3,
	...
	public final void Step1() {
		...
		// /#region 检查创建工作是否符合预期.
		// 检查开始节点写入的数据是否正确？
		String sql = "SELECT * FROM ND201 WHERE OID=" + workID;
		...
		for (DataColumn dc : dt.Columns) {
			...
			{
				if (!val.equals("201")) {
					throw new RuntimeException("@应当是 201, 现在是:" + val);
				}
			}
		...
		if (objs.getVarToNodeID() != 202) {
			throw new RuntimeException("@应该是 202 节点. 现在是:" + objs.getVarToNodeID());
		}
		...
		// 判断当前点.
		if (gwf.getFK_Node() != 202) {
			throw new RuntimeException("@当前点应该是 202 现在是:" + gwf.getFK_Node());
		}
		...
		// /#region 第3步: 检查节点数据表.
		sql = "SELECT * FROM ND201 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到开始节点的数据.");
		}
		...
		// 检查节点2的数据.
		sql = "SELECT * FROM ND202 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到 ND202 的数据.");
		}
		for (DataColumn dc : dt.Columns) {
			String val = dt.getValue(0,dc.ColumnName).toString();
			if(dc.ColumnName.equals(WorkAttr.CDT)){
		...
		for (DataColumn dc : dt.Columns) {
			...
			if(dc.ColumnName.equals(GERptAttr.FID)){
				...
				if (!val.equals("202")) {
					throw new RuntimeException("@应当是 202, 现在是:" + val);
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowStarter)){
				if (!val.equals(BP.Web.WebUser.getNo())) {
					throw new RuntimeException(
	...
	public final void Step2() {
		...
		if (objs.getVarToNodeID() != 299) {
			throw new RuntimeException("@应该是 299 节点. 现在是:" + objs.getVarToNodeID());
		}
		...
		// 判断当前点.
		if (gwf.getFK_Node() != 299) {
			throw new RuntimeException("@当前点应该是 299 现在是:" + gwf.getFK_Node());
		}
		...
		// 检查节点3的数据.
		sql = "SELECT * FROM ND299 WHERE OID=" + workID;
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() != 1) {
			throw new RuntimeException("@发起流程出错误,不应该找不到 ND299 的数据.");
		}
		for (DataColumn dc : dt.Columns) {
			String val = dt.getValue(0,dc.ColumnName).toString();
			if(dc.ColumnName.equals(WorkAttr.CDT)){
		...
		for (DataColumn dc : dt.Columns) {
			...
			if(dc.ColumnName.equals(GERptAttr.FID)){
				...
				if (!val.equals("299")) {
					throw new RuntimeException("@应当是 299, 现在是:" + val);
				}
			}else if(dc.ColumnName.equals(GERptAttr.FlowStarter)){
				if (val.equals(BP.Web.WebUser.getNo())) {
					throw new RuntimeException(
