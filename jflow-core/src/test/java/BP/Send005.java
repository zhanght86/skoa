...
//		this.EditState = CT.EditState.Passed;
	...
	*/
	public final void Step1()
	{
		...
		// 检查创建这个空白是否有数据完整?;
		sql = "SELECT * FROM WF_EmpWorks WHERE WorkID=" + workid + " AND FK_Emp='" + BP.Web.WebUser.getNo() + "'";
		dt = DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() !=0)
		{
		...
		if (gwf.getStarter() != BP.Web.WebUser.getNo())
		{
			throw new RuntimeException("@应该是发起人员，现在是:" + gwf.getStarter());
		}

		//找出发起人的工作列表.
		gwl = new GenerWorkerList(workid, 501, BP.Web.WebUser.getNo());
		...
		{
			if (item.getStarter() != BP.Web.WebUser.getNo())
			{
		...
		if (!dt.getValue(0,"Rec").toString().equals(BP.Web.WebUser.getNo()))
		{
			throw new RuntimeException("@没有向主线程开始节点表里写入Rec字段，现在是：" + dt.getValue(0,"Rec").toString() + "应当是:" + BP.Web.WebUser.getNo());
		}
	...
	*/
	public final void Step2_1()
	{
		...
		//获得此人的 005 的待办工作.
		DataTable dt = BP.WF.Dev2Interface.DB_GenerEmpWorksOfDataTable(BP.Web.WebUser.getNo(), WFState.Runing, "005");
		if (dt.Rows.size() == 0)
		{
