...
public class SysEnums extends Entities
{
	...
	public final String GenerCaseWhenForOracle(String enName, String mTable,
			String key, String field, String enumKey, int def)
	{
		String sql = (String) Cash.GetObjFormApplication("ESQL" + enName
				+ mTable + key + "_" + enumKey, null);
		// string sql = "";
		if (sql != null)
		{
			return sql;
		...
		for (Object se1 : this)
		{
			sql += " WHEN " + ((SysEnum) se1).getIntKey() + " THEN '"
					+ ((SysEnum) se1).getLab() + "'";
		}
		...
		Cash.AddObj("ESQL" + enName + mTable + key + "_" + enumKey,
				Depositary.Application, sql);
		return sql;
	}
	
	public final String GenerCaseWhenForOracle(String mTable, String key,
			String field, String enumKey, int def)
	{
		if (this.size() == 0)
		{
		...
		if (!this.Full(enumKey))
		{
			try
			{
				...
				if (BP.Sys.SystemConfig.getAppCenterDBType() == DBType.Oracle) {
					BP.DA.DBAccess.RunSQL("UPDATE Sys_Enum SET MyPK=EnumKey||'_'||Lang||'_'||to_char(IntKey)");
				}else{
	...
	{
		Entities ens = (Entities) Cash.GetObjFormApplication("EnumOf" + enumKey
				+ BP.Web.WebUser.getSysLang(), null);
		if (ens != null)
		{
		...
		Cash.AddObj("EnumOf" + enumKey + BP.Web.WebUser.getSysLang(),
				Depositary.Application, this);
		return true;
	}
	...
	public int Delete(String key, Object val)
	{
		try
		{
			...
			ps.SQL = SqlBuilder.DeleteSysEnumsSQL(en.getEnMap()
					.getPhysicsTable(), key);
			ps.Add("p", val);
			return en.RunSQL(ps);
		} catch (java.lang.Exception e)
		{
			...
			Paras ps = new Paras();
			ps.SQL = "DELETE FROM " + en.getEnMap().getPhysicsTable()
					+ " WHERE " + key + "=" + en.getHisDBVarStr() + "p";
			ps.Add("p", val);
			try
			{
				return en.RunSQL(ps);
	...
	public List<SysEnum> ToJavaList()
	{
