...
public class MapDtl extends EntityNoName
{
	...
	{
		this.SetValByKey(MapDtlAttr.IsFLDtl, value);
	}
	/**
	 * 是否可以导入？
	 */
	...
	{
		return this.GetValStringByKey(MapDtlAttr.ImpSQLSearch)
				.replace("~", "'");
	}
	...
	*/
	...
	{
		return this.GetValStringByKey(MapDtlAttr.SubThreadWorker);
	}
	
	public final void setSubThreadWorker(String value)
	{
		this.SetValByKey(MapDtlAttr.SubThreadWorker, value);
	}
	...
	*/
	...
	{
		return this.GetValStringByKey(MapDtlAttr.SubThreadGroupMark);
	}
	
	public final void setSubThreadGroupMark(String value)
	{
		this.SetValByKey(MapDtlAttr.SubThreadGroupMark, value);
	}
	/**
	 * 填充属性sql
	 */
	...
	{
		return DtlShowModel.forValue(this
				.GetValIntByKey(MapDtlAttr.DtlShowModel));
	}
	...
	*/
	...
	{
		return WhenOverSize.forValue(this
				.GetValIntByKey(MapDtlAttr.WhenOverSize));
	}
	...
	{
		return DtlOpenType
				.forValue(this.GetValIntByKey(MapDtlAttr.DtlOpenType));
	}
	...
	*/
	...
	{
		return DtlAddRecModel.forValue(GetValIntByKey(MapDtlAttr.DtlAddRecModel));
	}
	public final void setDtlAddRecModel(DtlAddRecModel value)
	{
		this.SetValByKey(MapDtlAttr.DtlAddRecModel, value);
	}
	/** 保存方式
	...
	*/
	...
	{
		return DtlSaveModel.forValue(GetValIntByKey(MapDtlAttr.DtlSaveModel));
	}
	public final void setDtlSaveModel(DtlSaveModel value)
	{
		this.SetValByKey(MapDtlAttr.DtlSaveModel, value);
	}
	/**
	 * EnMap
	 */
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(MapDtlAttr.FK_MapData, null, "主表", true, false, 0, 30,
				20);
		map.AddTBString(MapDtlAttr.PTable, null, "物理表", true, false, 0, 30, 20);
		map.AddTBString(MapDtlAttr.GroupField, null, "分组字段", true, false, 0,
				50, 20);
		...
		map.AddTBString(MapDtlAttr.ImpFixTreeSql, null, "固定列树形SQL", true,
				false, 0, 500, 20);
		map.AddTBString(MapDtlAttr.ImpFixDataSql, null, "固定列数据SQL", true,
				false, 0, 500, 20);
		...
		map.AddBoolean(MapDtlAttr.IsEnableGroupField, false, "是否启用分组字段", false,
				false);
		...
		map.AddBoolean(MapDtlAttr.IsCopyNDData, true, "IsCopyNDData", false,
				false);
		...
		map.AddBoolean(MapDtlAttr.IsShowTitle, true, "IsShowTitle", false,
				false);
		...
		map.AddBoolean(MapDtlAttr.IsEnablePass, false, "是否启用通过审核功能?", false,
				false);
		...
		map.AddDDLSysEnum(MapDtlAttr.WhenOverSize, 0, "WhenOverSize", true,
				true, MapDtlAttr.WhenOverSize, "@0=不处理@1=向下顺增行@2=次页显示");
		...
		map.AddBoolean(MapDtlAttr.IsEnableSelectImp, false, "是否启用选择数据导入?",
				false, false);
		map.AddTBString(MapDtlAttr.ImpSQLSearch, null, "查询SQL", true, false, 0,
				500, 20);
		map.AddTBString(MapDtlAttr.ImpSQLInit, null, "初始化SQL", true, false, 0,
				500, 20);
		map.AddTBString(MapDtlAttr.ImpSQLFull, null, "数据填充SQL", true, false, 0,
				500, 20);
		// 导入导出填充.
	...
	public final int GetCountByFK(int workID)
	{
		try
		{
			return BP.DA.DBAccess.RunSQLReturnValInt("select COUNT(OID) from "
					+ this.getPTable() + " WHERE WorkID=" + workID);
		} catch (Exception e)
		{
	...
	public final int GetCountByFK(String field, String val)
	{
		...
		{
			return BP.DA.DBAccess.RunSQLReturnValInt("select COUNT(OID) from "
					+ this.getPTable() + " WHERE " + field + "='" + val + "'");
		} catch (Exception e)
		{
	...
	public final int GetCountByFK(String f1, long val1, String f2, String val2)
	{
		...
		{
			return BP.DA.DBAccess.RunSQLReturnValInt("SELECT COUNT(OID) from "
					+ this.getPTable() + " WHERE " + f1 + "=" + val1 + " AND "
					+ f2 + "='" + val2 + "'");
		} catch (Exception e)
		{
...
//			{
//				throw new RuntimeException("您启用了从表单(" + this.getName()
//						+ ")行数据锁定功能，但是该从表里没IsRowLock字段，请参考帮助文档。");
//			}
		}
		...
		if (this.getIsEnablePass())
		{
			...
			{
				throw new RuntimeException("您启用了从表单(" + this.getName()
						+ ")条数据审核选项，但是该从表里没IsPass字段，请参考帮助文档。");
			}
		}
	...
	protected boolean beforeUpdate()
	{
		...
		for (MapAttr attr : MapAttrs.convertMapAttrs(attrs))
		{
			if (attr.getUIIsEnable()
					&& attr.getUIContralType() == UIContralType.TB)
			{
				isHaveEnable = true;
			}
		}
	...
	protected boolean beforeDelete()
	{
		...
		sql += "@DELETE FROM Sys_FrmAttachment WHERE FK_MapData='"
				+ this.getNo() + "'";
