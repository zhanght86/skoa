	...
	*/
	...
	public final Map GenerHisMap()
	{
		...
		for (MapAttr mapAttr : MapAttrs.convertMapAttrs(mapAttrs))
		{
			...
			// System.out.println(((MapAttr)mapAttr).getHisAttr().getKey());
			// System.out.println(((MapAttr)mapAttr).getHisAttr().getField());
			// System.out.println(((MapAttr)mapAttr).getHisAttr().getDesc());
	...
	public Map getEnMap()
	{
		...
		map.AddTBStringPK(MapDataAttr.No, null, "编号", true, false, 1, 200, 20);
		...
		map.AddTBString(MapDataAttr.PTable, null, "物理表", true, false, 0, 500,
				20);
		map.AddTBString(MapDataAttr.Url, null, "连接(对自定义表单有效)", true, false, 0,
				500, 20);
		...
		// 可以为空这个字段。
		map.AddTBString(MapDataAttr.FK_FrmSort, null, "表单类别", true, false, 0,
				500, 20);
		map.AddTBString(MapDataAttr.FK_FormTree, null, "表单树类别", true, false, 0,
				500, 20);
	...
	public final void DoUp()
	{
		try
		{
			this.DoOrderUp(MapDataAttr.FK_FormTree, this.getFK_FormTree(),
					MapDataAttr.Idx);
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	public final void DoOrderDown()
	{
		try
		{
			this.DoOrderDown(MapDataAttr.FK_FormTree, this.getFK_FormTree(),
					MapDataAttr.Idx);
		} catch (Exception e)
		{
			e.printStackTrace();
...
//					System.out.println(dt.Columns.size());
			...
			} else if (dt.TableName.equals("Sys_FrmBtn"))
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						...
						if (!StringHelper.isNullOrEmpty(val))
						{
							en.SetValByKey(dc.ColumnName, val.toString()
									.replace(oldMapID, fk_mapdata));
						}
					}
					if (isSetReadonly)
				...
				for (DataRow dr : dt.Rows)
				{
					...
					if (isSetReadonly)
					{
						if (!StringHelper.isNullOrEmpty(en.getDefValReal())
								&& (en.getDefValReal().contains("@WebUser.") || en
										.getDefValReal().contains("@RDT")))
						{
							en.setDefValReal("");
						}
			...
			} else if (dt.TableName.equals("Sys_GroupField"))
			{
				for (DataRow dr : dt.Rows)
				{
					...
					en.setOID(0);
					en.Insert();
					endDoSQL += "@UPDATE Sys_MapAttr SET GroupID="
							+ en.getOID() + " WHERE FK_MapData='" + fk_mapdata
			...
			} else if (dt.TableName.equals("Sys_Enum"))
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						String val = (String) ((dr.getValue(dc.ColumnName) instanceof String) ? dr
								.getValue(dc.ColumnName) : null);
						se.SetValByKey(dc.ColumnName, val);
					}
					se.setMyPK(se.getEnumKey() + "_" + se.getLang() + "_"
							+ se.getIntKey());
					if (se.getIsExits())
					{
						continue;
			...
			} else if (dt.TableName.equals("Sys_EnumMain"))
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						...
						if (!StringHelper.isNullOrEmpty(val))
						{
							sem.SetValByKey(dc.ColumnName, val.toString()
									.replace(oldMapID, fk_mapdata));
						}
					}
					if (sem.getIsExits())
			...
			} else if (dt.TableName.equals("WF_Node"))
			{
				...
				if (dt.Rows.size() > 0)
				{
					endDoSQL += "@UPDATE WF_Node SET FWCSta=2" + ",FWC_X="
							+ dt.Rows.get(0).get("FWC_X") + ",FWC_Y="
							+ dt.Rows.get(0).get("FWC_Y") + ",FWC_H="
							+ dt.Rows.get(0).get("FWC_H") + ",FWC_W="
							+ dt.Rows.get(0).get("FWC_W") + ",FWCType="
							+ dt.Rows.get(0).get("FWCType") + " WHERE NodeID="
							+ fk_mapdata.replace("ND", "");
				}
			} else
			{
			}
	...
	public final void RepairMap()
	{
		...
		if (gfs.size() == 0)
		{
			...
			gf.Insert();
			String sqls = "";
			sqls += "@UPDATE Sys_MapDtl SET GroupID=" + gf.getOID()
					+ " WHERE FK_MapData='" + this.getNo() + "'";
			sqls += "@UPDATE Sys_MapAttr SET GroupID=" + gf.getOID()
					+ " WHERE FK_MapData='" + this.getNo() + "'";
			sqls += "@UPDATE Sys_MapFrame SET GroupID=" + gf.getOID()
					+ " WHERE FK_MapData='" + this.getNo() + "'";
			sqls += "@UPDATE Sys_MapM2M SET GroupID=" + gf.getOID()
					+ " WHERE FK_MapData='" + this.getNo() + "'";
			sqls += "@UPDATE Sys_FrmAttachment SET GroupID=" + gf.getOID()
					+ " WHERE FK_MapData='" + this.getNo() + "'";
			try
			{
		...
		{
			GroupField gfFirst = (GroupField) ((gfs.get(0) instanceof GroupField) ? gfs
					.get(0) : null);
			String sqls = "";
			sqls += "@UPDATE Sys_MapDtl SET GroupID="
					+ gfFirst.getOID()
					+ "        WHERE  No   IN (SELECT X.No FROM (SELECT No FROM Sys_MapDtl WHERE GroupID NOT IN (SELECT OID FROM Sys_GroupField WHERE EnName='"
					+ this.getNo() + "')) X) AND FK_MapData='"
					+ this.getNo() + "'";
			sqls += "@UPDATE Sys_MapAttr SET GroupID="
					+ gfFirst.getOID()
					...
					+ this.getNo() + "')) X) AND FK_MapData='"
					+ this.getNo() + "' ";
			sqls += "@UPDATE Sys_MapFrame SET GroupID="
					+ gfFirst.getOID()
					...
					+ this.getNo() + "')) X) AND FK_MapData='"
					+ this.getNo() + "' ";
			sqls += "@UPDATE Sys_MapM2M SET GroupID="
					+ gfFirst.getOID()
					...
					+ this.getNo() + "')) X) AND FK_MapData='"
					+ this.getNo() + "' ";
			sqls += "@UPDATE Sys_FrmAttachment SET GroupID="
					+ gfFirst.getOID()
		...
		{
			if (!attr.IsExit(MapAttrAttr.KeyOfEn, "OID",
					MapAttrAttr.FK_MapData, this.getNo()))
			{
		...
		{
			if (!attr.IsExit(MapAttrAttr.KeyOfEn, this.getEnPK(),
					MapAttrAttr.FK_MapData, this.getNo()))
			{
				attr.setFK_MapData(this.getNo());
		...
		if (!attr.IsExit(MapAttrAttr.KeyOfEn, "RDT", MapAttrAttr.FK_MapData,
				this.getNo()))
		{
	...
	*/
	...
	{
		String sql = "UPDATE Sys_MapData SET VER='"
				+ BP.DA.DataType.getCurrentDataTimess() + "' WHERE No='"
				+ this.getNo() + "'";
	 ...
	 * @return
	 */
	public static String GenerAutoFull(String pk, MapAttrs attrs, MapExt me,
			String tbPer)
	{
		String left = "\n document.forms[0]." + tbPer + "_TB"
				+ me.getAttrOfOper() + "_" + pk + ".value = ";
		String right = me.getDoc();
		for (Object mattr : attrs)
		{
			right = right.replace("@" + ((MapAttr) mattr).getKeyOfEn(),
					" parseFloat( document.forms[0]." + tbPer + "_TB_"
							+ ((MapAttr) mattr).getKeyOfEn() + "_" + pk
							+ ".value) ");
		}
		return " alert( document.forms[0]." + tbPer + "_TB"
				+ me.getAttrOfOper() + "_" + pk + ".value ) ; \t\n " + left
				+ right;
	}
}