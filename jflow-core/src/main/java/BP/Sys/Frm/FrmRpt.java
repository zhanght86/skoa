...
public class FrmRpt extends EntityNoName
{
	...
	public final DtlShowModel getHisDtlShowModel()
	{
		return DtlShowModel.forValue(this
				.GetValIntByKey(FrmRptAttr.DtlShowModel));
	}
	...
	*/
	public final WhenOverSize getHisWhenOverSize()
	{
		return WhenOverSize.forValue(this
				.GetValIntByKey(FrmRptAttr.WhenOverSize));
	}
	...
	{
		return DtlOpenType
				.forValue(this.GetValIntByKey(FrmRptAttr.DtlOpenType));
	}
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(FrmRptAttr.FK_MapData, null, "主表", true, false, 0, 30,
				20);
		map.AddTBString(FrmRptAttr.PTable, null, "物理表", true, false, 0, 30, 20);
		
		map.AddTBString(FrmRptAttr.SQLOfColumn, null, "列的数据源", true, false, 0,
				300, 20);
		map.AddTBString(FrmRptAttr.SQLOfRow, null, "行数据源", true, false, 0, 300,
				20);
		...
		map.AddBoolean(FrmRptAttr.IsCopyNDData, true, "IsCopyNDData", false,
				false);
		...
		map.AddBoolean(FrmRptAttr.IsShowTitle, true, "IsShowTitle", false,
				false);
		...
		map.AddBoolean(FrmRptAttr.IsEnablePass, false, "是否启用通过审核功能?", false,
				false);
		...
		map.AddDDLSysEnum(FrmRptAttr.WhenOverSize, 0, "WhenOverSize", true,
				true, FrmRptAttr.WhenOverSize, "@0=不处理@1=向下顺增行@2=次页显示");
		
		map.AddDDLSysEnum(FrmRptAttr.DtlOpenType, 1, "数据开放类型", true, true,
				FrmRptAttr.DtlOpenType, "@0=操作员@1=工作ID@2=流程ID");
		
		map.AddDDLSysEnum(FrmRptAttr.DtlShowModel, 0, "显示格式", true, true,
				FrmRptAttr.DtlShowModel, "@0=表格@1=卡片");
