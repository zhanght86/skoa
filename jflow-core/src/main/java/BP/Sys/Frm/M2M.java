...
public class M2M extends EntityMyPK
{
	...
	public Map getEnMap()
	{
		...
		map.setEnDesc("M2M数据存储");
		map.setDepositaryOfMap(Depositary.Application);
		
		map.AddMyPK();
		map.AddTBString(M2MAttr.FK_MapData, null, "FK_MapData", true, true, 0,
				20, 20);
		map.AddTBString(M2MAttr.M2MNo, null, "M2MNo", true, true, 0, 20, 20);
		
		map.AddTBInt(M2MAttr.EnOID, 0, "实体OID", true, false);
		map.AddTBString(M2MAttr.DtlObj, null, "DtlObj(对于m2mm有效)", true, true,
				0, 20, 20);
	...
	{
		this.setMyPK(this.getFK_MapData() + "_" + this.getM2MNo() + "_"
				+ this.getEnOID() + "_" + this.getDtlObj());
	}
}