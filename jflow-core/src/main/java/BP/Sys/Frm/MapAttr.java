...
public class MapAttr extends EntityMyPK
{
	...
	public final EntitiesNoName getHisEntitiesNoName()
	{
		...
		{
			EntitiesNoName ens = (EntitiesNoName) ClassFactory.GetEns(this
					.getUIBindKey());
			if (null == ens)
			{
				return null;
		...
		GENoNames myens = null;
		if("local".equals(sf.getFK_SFDBSrc()))
		{
			myens = new GENoNames(this.getUIBindKey(), this.getName());
			myens.RetrieveAll();
		}else
        {
            Log.DebugWriteError("getHisEntitiesNoName @非实体类实体不能获取EntitiesNoName。");
        }
		return myens;
			...
			default:
				...
				switch (this.getMyDataType())
				{
					...
					case DataType.AppDate:
						...
						{
							attr.setDefaultVal(DataType
									.getCurrentDateByFormart("yyyy-MM-dd"));
						}
						break;
					case DataType.AppDateTime:
						if (this.getTag().equals("1"))
						{
							attr.setDefaultVal(DataType.getCurrentDataTime());
						}
						break;
					default:
						break;
	...
	{
		if (this.getKeyOfEn().equals("OID") || this.getKeyOfEn().equals("No")
				|| this.getKeyOfEn().equals("MyPK"))
		{
			return true;
		} else
		{
	...
	{
		return AutoFullWay.forValue(this
				.GetValIntByKey(MapAttrAttr.AutoFullWay));
	}
	...
	public final String getDefVal()
	{
		...
		switch (this.getMyDataType())
		{
			case BP.DA.DataType.AppDate:
				...
				{
					return DataType.getCurrentDateByFormart("yyyy-MM-dd");
				} else
				{
					return "          ";
				}
	...
	public final int getUIWidthOfLab()
	{
		...
		// int textWidth =
		// getFontMetrics(g2.getFont()).bytesWidth(str.getBytes(), 0,
		// str.getBytes().length);
	...
	{
		return UIContralType.forValue(this
				.GetValIntByKey(MapAttrAttr.UIContralType));
	}
	...
	public MapAttr(String fk_mapdata, String key)
	{
		...
		this.setKeyOfEn(key);
		this.Retrieve(MapAttrAttr.FK_MapData, this.getFK_MapData(),
				MapAttrAttr.KeyOfEn, this.getKeyOfEn());
	}
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(MapAttrAttr.FK_MapData, null, "实体标识", true, true, 1,
				200, 20);
		...
		map.AddTBString(MapAttrAttr.DefVal, null, "默认值", false, false, 0, 4000,
				20);
		
		// map.AddDDLSysEnum(MapAttrAttr.UIContralType, 0, "空件类型", true, false,
		// MapAttrAttr.UIContralType, "@0=文本框@1=下拉框");
		// map.AddDDLSysEnum(MapAttrAttr.MyDataType, 0, "数据类型", true, false,
		// MapAttrAttr.MyDataType,
		...
		map.AddDDLSysEnum(MapAttrAttr.LGType, 0, "逻辑类型", true, false,
				MapAttrAttr.LGType, "@0=普通@1=枚举@2=外键");
		...
		map.AddTBString(MapAttrAttr.UIBindKey, null, "绑定的信息", true, false, 0,
				100, 20);
		map.AddTBString(MapAttrAttr.UIRefKey, null, "绑定的Key", true, false, 0,
				30, 20);
		map.AddTBString(MapAttrAttr.UIRefKeyText, null, "绑定的Text", true, false,
				0, 30, 20);
		...
		// map.AddTBString(MapAttrAttr.AutoFullDoc, null, "自动填写内容", false,
		// false, 0, 500, 20);
		// // map.AddDDLSysEnum(MapAttrAttr.AutoFullWay, 0, "自动填写方式", true,
		// false, MapAttrAttr.AutoFullWay,
		...
		map.AddTBString(MapAttrAttr.Tag, null, "标识（存放临时数据）", true, false, 0,
				100, 20);
	...
	public final void DoDownTabIdx()
	{
		try
		{
			this.DoOrderDown(MapAttrAttr.FK_MapData, this.getFK_MapData(),
					MapAttrAttr.IDX);
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	public final void DoUpTabIdx()
	{
		try
		{
			this.DoOrderUp(MapAttrAttr.FK_MapData, this.getFK_MapData(),
					MapAttrAttr.IDX);
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	public final void DoUp()
	{
		try
		{
			this.DoOrderUp(MapAttrAttr.GroupID,
					(new Integer(this.getGroupID())).toString(),
	...
	public final void DoDown()
	{
		try
		{
			this.DoOrderDown(MapAttrAttr.GroupID,
					(new Integer(this.getGroupID())).toString(),
					MapAttrAttr.UIVisible, "1", MapAttrAttr.IDX);
		} catch (Exception e)
		{
	...
	{
		String sql = "UPDATE Sys_MapAttr SET IDX=IDX+1 WHERE IDX <="
				+ attrTo.getIDX() + " AND FK_MapData='" + this.getFK_MapData()
				+ "' AND GroupID=" + this.getGroupID();
		DBAccess.RunSQL(sql);
	...
	protected boolean beforeUpdateInsertAction()
	{
		...
		{
			if (this.getUIIsEnable() && this.getDefVal() != null
					&& this.getDefVal().contains("@"))
			{
				throw new RuntimeException(
						"@不能在非只读(不可编辑)的字段设置具有@的默认值. 您设置的默认值为:"
								+ this.getDefVal());
			}
		}
	...
	protected boolean beforeInsert()
	{
		...
		if (this.getKeyOfEn() == null || this.getKeyOfEn().trim().equals(""))
		{
			try
			{
				...
				if (this.getKeyOfEn() == null
						|| this.getKeyOfEn().trim().equals(""))
				{
					throw new RuntimeException("@请输入字段描述或者字段名称。");
				}
			} catch (RuntimeException ex)
			{
				throw new RuntimeException("@请输入字段描述或字段名称。异常信息:"
						+ ex.getMessage());
			}
		} else
		{
		...
		String keyofenC = (String) ((tempVar instanceof String) ? tempVar
				: null);
		keyofenC = keyofenC.toLowerCase();
		
		if (PubClass.getKeyFields().contains("," + keyofenC + ","))
		{
			throw new RuntimeException("@错误:[" + this.getKeyOfEn()
					+ "]是字段关键字，您不能用它做字段。");
		}
		
		if (this.IsExit(MapAttrAttr.KeyOfEn, this.getKeyOfEn(),
				MapAttrAttr.FK_MapData, this.getFK_MapData()))
		{
			return false;
...
//					+ this.getName() + "]字段[" + this.getKeyOfEn() + "]");
		...
		this.setIDX(999); // BP.DA.DBAccess.RunSQLReturnValInt("SELECT COUNT(*) FROM Sys_MapAttr WHERE FK_MapData='"
							// + this.FK_MapData + "'") + 1;
		this.setMyPK(this.getFK_MapData() + "_" + this.getKeyOfEn());
	...
	{
		String sql = "DELETE FROM Sys_MapExt WHERE (AttrOfOper='"
				+ this.getKeyOfEn() + "' OR AttrsOfActive='"
				+ this.getKeyOfEn() + "' ) AND (FK_MapData='')";
		// 删除权限管理字段.
		sql += "@DELETE FROM Sys_FrmSln WHERE KeyOfEn='" + this.getKeyOfEn()
				+ "' AND FK_MapData='" + this.getFK_MapData() + "'";
		try
		{
