...
import BP.En.Map;
import BP.En.UAC;
import BP.Sys.ToolbarExcelAttr;
...
public class ToolbarExcelSln extends EntityMyPK
{
	...
	public ToolbarExcelSln(String fk_flow, int fk_node, String fk_frm)
	{
		int i = this.Retrieve(ToolbarExcelSlnAttr.FK_Flow, fk_flow,
				ToolbarExcelSlnAttr.FK_Node, fk_node,
				ToolbarExcelSlnAttr.FK_Frm, fk_frm);
		if (i == 0)
		{
	...
	public Map getEnMap()
	{
		...
		map.setEnDesc("ToolbarExcelSln功能控制");
		
		map.setDepositaryOfEntity(Depositary.Application);
		map.setDepositaryOfMap(Depositary.Application);
		
		map.AddMyPK();
		map.AddTBString(ToolbarExcelSlnAttr.FK_Frm, null, "表单ID", true, true,
				1, 32, 32);
		map.AddTBInt(ToolbarExcelSlnAttr.FK_Node, 0, "节点编号", true, true);
		map.AddTBString(ToolbarExcelSlnAttr.FK_Flow, null, "流程编号", true, true,
				1, 20, 20);
