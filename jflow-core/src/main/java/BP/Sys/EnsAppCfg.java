...
public class EnsAppCfg extends EntityMyPK
{
	...
	public EnsAppCfg(String ensName, String cfgkey)
	{
		...
		} catch (java.lang.Exception e)
		{
			...
			int i = xmls.Retrieve(BP.Sys.EnsAppXmlEnsName.EnsName, ensName,
					"No", cfgkey);
			if (i == 0)
			{
				...
				for (Attr attr : attrs)
				{
					if (attr.getKey().equals(cfgkey))
					{
						...
						{
							Entity en = BP.En.ClassFactory.GetEns(ensName)
									.getGetNewEntity();
	...
	public Map getEnMap()
	{
		...
		map.AddMyPK();
		map.AddTBString(EnsAppCfgAttr.EnsName, null, "实体集合", true, false, 0,
				100, 60);
		map.AddTBString(EnsAppCfgAttr.CfgKey, null, "键", true, false, 0, 100,
				60);
		map.AddTBString(EnsAppCfgAttr.CfgVal, null, "值", true, false, 0, 200,
				60);
