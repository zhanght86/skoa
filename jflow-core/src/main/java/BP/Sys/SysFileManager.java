...
public class SysFileManager extends EntityOID
{
	...
	*/
	...
	{
		return BP.Sys.SystemConfig.getPathOfUsersFiles()
				+ this.GetValStringByKey(SysFileManagerAttr.MyFilePath);
	}
	...
	public Map getEnMap()
	{
		...
		map.setEnDesc("文件管理者");
		map.AddTBIntPKOID();
		map.AddTBString(SysFileManagerAttr.AttrFileName, null, "指定名称", true,
				false, 0, 50, 20);
		map.AddTBString(SysFileManagerAttr.AttrFileNo, null, "指定编号", true,
				false, 0, 50, 20);
		
		map.AddTBString(SysFileManagerAttr.EnName, null, "关联的表", false, true,
				1, 50, 20);
		map.AddTBString(SysFileManagerAttr.RefVal, null, "主键值", false, true, 1,
				50, 10);
		map.AddTBString(SysFileManagerAttr.WebPath, null, "Web路径", false, true,
				0, 100, 30);
		
		map.AddMyFile("文件名称");
		
		// map.AddTBString(SysFileManagerAttr.MyFileName, null, "文件名称", true,
		// false, 1, 50, 20);
		...
		// map.AddTBString(SysFileManagerAttr.MyFileExt, null, "文件类型", true,
		// true, 0, 50, 20);
		
		map.AddTBString(SysFileManagerAttr.RDT, null, "上传时间", true, true, 1,
				50, 20);
		map.AddTBString(SysFileManagerAttr.Rec, null, "上传人", true, true, 0, 50,
				20);
		map.AddTBStringDoc();
		this.set_enMap(map);
		return this.get_enMap();
	...
	{
		if (this.getRec().equals(BP.Web.WebUser.getNo()))
		{
	...
	{
		// FileInfo fi = new FileInfo(MyFilePath);// Replace with your file
		// MyFileName
		// if (!fi.Exists )
		...
		// string strQuery =
		// "UPDATE Sys_FileManager SET FileData=@FileData WHERE OID=" +
		// this.OID;
