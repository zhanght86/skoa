...
public class SFDBSrc extends EntityNoName
{
    ...
    */
	...
	public Map getEnMap()
	{
		...
		map.AddTBStringPK(SFDBSrcAttr.No, null, "数据源编号(必须是英文)", true, false, 1,
				20, 20);
		map.AddTBString(SFDBSrcAttr.Name, null, "数据源名称", true, false, 0, 30, 20);
		
		map.AddTBString(SFDBSrcAttr.UserID, null, "数据库登录用户ID", true, false, 0,
				30, 20);
		map.AddTBString(SFDBSrcAttr.Password, null, "数据库登录用户密码", true, false,
				0, 30, 20);
		map.AddTBString(SFDBSrcAttr.IP, null, "IP地址", true, false, 0, 50, 20);
		map.AddTBString(SFDBSrcAttr.DBName, null, "数据库名称", true, false, 0, 30,
				20);
		
		map.AddDDLSysEnum(SFDBSrcAttr.DBSrcType, 0, "数据源类型", true, true,
				SFDBSrcAttr.DBSrcType,
				"@0=应用系统主数据库@1=SQLServer@2=Oracle@3=MySQL@4=Infomix");
	...
	public final String DoConn()
	{
		...
		if (this.getDBSrcType() == BP.Sys.DBSrcType.SQLServer)
		{
			try
			{
				...
				// ";Persist Security Info=True;User ID=" + this.getUserID() +
				// ";Initial Catalog=" + this.getDBName() + ";Data Source=" +
				// this.getIP() + ";Timeout=999;MultipleActiveResultSets=true";
				...
				try
				{
					BP.DA.DBAccess.RunSQL("Exec sp_droplinkedsrvlogin "
							+ this.getNo() + ",Null ");
				...
				// 创建应用.
				String sql = "";
				sql += "sp_addlinkedserver @server='" + this.getNo()
						+ "', @srvproduct='', @provider='SQLOLEDB', @datasrc='"
						+ this.getIP() + "'";
				BP.DA.DBAccess.RunSQL(sql);
				
				// 执行登录.
				sql = "";
				sql += " EXEC sp_addlinkedsrvlogin '" + this.getNo()
						+ "','false', NULL, '" + this.getUserID() + "', '"
						+ this.getPassword() + "'";
				BP.DA.DBAccess.RunSQL(sql);
		...
		// try
		// {
		// //Host=10.0.2.36;Service=8001;Server=niosserver; Database=ccflow;
		// UId=npmuser; Password=npmoptr2012;Database locale=en_US.819;Client
		// Locale=en_US.CP1252
		// dsn =
		// "Data Source="+this.IP+";Persist Security info=True;Initial Catalog="+this.DBName+";User ID="+this.UserID+";Password="+this.Password+";";
		// MySql.Data.MySqlClient.MySqlConnection conn = new
		// MySql.Data.MySqlClient.MySqlConnection();
		// conn.ConnectionString = dsn;
		// conn.Open();
	...
	public final DataTable GetTables()
	{
		...
		sql.append(String
				.format("SELECT ss.SrcTable FROM Sys_SFTable ss WHERE ss.FK_SFDBSrc = '%1$s'",
						this.getNo()));
	...
	public final DataTable GetColumns(String tableName)
	{
		...
		switch (dbType)
		{
			case SQLServer:
				...
				sql.append("       cast(ISNULL(ep.[value],'') as nvarchar(100)) as [Desc]");
				...
				sql.append(String.format(
						"WHERE  sc.id = OBJECT_ID('dbo.%1$s')", tableName));
				break;
			case Oracle:
				...
				sql.append(String.format(" WHERE utc.TABLE_NAME = '%1$s'",
			...
			case MySQL:
				...
				sql.append(String.format("    table_schema = '%1$s'",
						SystemConfig.getAppCenterDBDatabase()));
				sql.append(String.format("        AND table_name = '%1$s';",
						tableName));
				break;
			case Infomax:
				break;
