...
//		}
	...
	{
		return this.getHisMap().getPhysicsTable() + "."
				+ this.getHisMap().GetFieldByKey(attr);
	}
	...
	public final int GetCount()
	{
		...
		switch (this.getEn().getEnMap().getEnDBUrl().getDBType())
		{
			...
			default:
				...
				// sql="SELECT COUNT(*) as C "+this._endSql +sql.Substring(
				// sql.IndexOf("FROM ") ) ;
				// sql="SELECT COUNT(*) as C FROM "+
				// this._ens.GetNewEntity.EnMap.PhysicsTable+ "  "
			...
			String[] fullAttrs)
	{
			...
			try
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (Attr attr : attrs)
					{
						// if (attr.IsRefAttr)
						// continue;
						ArrayList<DataColumn> list = dt.Columns;
						for (int i = 0; i < list.size(); i++)
						{
							DataColumn column = list.get(i);
							if (column.ColumnName.equals(attr.getKey()
									.toLowerCase()))
							{
		...
		} else
		{
			...
			for (DataRow dr : dt.Rows)
			{
				...
				{
					en.getRow().SetValByKey(str, dr.getValue(str));
					/*
					 * warning en.getRow().SetValByKey(str, dr[str]);
					 */
				}
				ens.AddEntity(en);
			}
