...
public class Attr
{
	...
	public final boolean getIsFK()
	{
		if (this.getMyFieldType() == FieldType.FK
				|| this.getMyFieldType() == FieldType.PKFK)
		{
			return true;
		} else
		{
	...
	{
		if (this.getMyFieldType() == FieldType.Enum
				|| this.getMyFieldType() == FieldType.PKEnum
				|| this.getMyFieldType() == FieldType.FK
				|| this.getMyFieldType() == FieldType.PKFK
				|| this.getMyFieldType() == FieldType.BindTable)
		{
			return true;
		} else
		{
	...
	{
		if (getMyDataType() == DataType.AppBoolean
				|| getMyDataType() == DataType.AppDouble
				|| getMyDataType() == DataType.AppFloat
				|| getMyDataType() == DataType.AppInt
				|| getMyDataType() == DataType.AppMoney
				|| getMyDataType() == DataType.AppRate)
		{
			return true;
		} else
		{
	...
	{
		if (getMyFieldType() == FieldType.Enum
				|| getMyFieldType() == FieldType.PKEnum)
		{
			return true;
		} else
		{
	...
	public final boolean getIsBindTable()
	{
		if (getMyFieldType() == FieldType.BindTable)
		{
			return true;
		} else
		{
	...
	{
		if (getMyFieldType() == FieldType.PK
				|| getMyFieldType() == FieldType.PKFK
				|| getMyFieldType() == FieldType.PKEnum)
		{
			return true;
		} else
	...
	public final String getEnterDesc()
	{
		...
		{
			if (this.getUIIsReadonly() || !this.getUIVisible())
			{
				return "此字段只读";
			} else
			{
				...
				{
					return "输入要求最小长度" + this.getMinLength() + "字符，最大长度"
							+ this.getMaxLength() + "字符";
		...
		} else if (this.getUIContralType() == UIContralType.DDL
				|| this.getUIContralType() == UIContralType.CheckBok)
		{
	 ...
	 * @param desc
	 */
	public Attr(String key, String field, Object defaultVal, int dataType,
			boolean isPK, String desc, int minLength, int maxlength)
	{
		this._key = key;
		this._field = field;
		this._desc = desc;
	...
	/*
	 * warning public AutoFullWay AutoFullWay = AutoFullWay.Way0;
	 */
		...
		switch (this.getMyDataType())
		{
			...
			case DataType.AppInt:
				...
				} catch (java.lang.Exception e)
				{
					return 0;
					// throw new
					// Exception("@设置["+this.Key+"]默认值出现错误，["+_defaultVal.ToString()+"]不能向 int 转换。");
				}
			case DataType.AppMoney:
				...
				} catch (java.lang.Exception e2)
				{
					return 0;
					// throw new
					// Exception("@设置["+this.Key+"]默认值出现错误，["+_defaultVal.ToString()+"]不能向 AppMoney 转换。");
				}
			case DataType.AppFloat:
				...
				} catch (java.lang.Exception e3)
				{
					return 0;
					// throw new
					// Exception("@设置["+this.Key+"]默认值出现错误，["+_defaultVal.ToString()+"]不能向 float 转换。");
				}
				
			case DataType.AppBoolean:
				if (this._defaultVal == null
						|| this._defaultVal.toString().equals(""))
				{
					return 0;
				}
				try
			...
			default:
				throw new RuntimeException(
						"@bulider insert sql error: 没有这个数据类型，字段名称:"
								+ this.getDesc() + " 英文:" + this.getKey());
		}
	...
	public final String getDescHelper()
	{
		...
		if (this.HelperUrl.contains("script"))
		{
			return "<a href=\""
					+ this.HelperUrl
					+ "\" ><img src='../../Img/Help.png'  height='20px' border=0/>"
					+ this._desc + "</a>";
		} else
		{
			return "<a href=\"javascript:window.open('"
					+ this.HelperUrl
					+ "', '_blank', 'resizable=1,scrollbars=yes,height=650,width=950')\" target=_blank ><img src='../../Img/Help.png'  height='20px' border=0/>"
					+ this._desc + "</a>";
		}
	}
	...
	public final String getDescHelperIcon()
	{
		...
		return "<a href=\"javascript:window.open('"
				+ this.HelperUrl
				+ "', '_blank', 'resizable=1,scrollbars=yes,height=650,width=950')\" target=_blank ><img src='../../Img/Help.png' height='20px' border=0/></a>";
	}
	...
	/**
	 * 要Bind 的Key. 在TB 里面就是 DataHelpKey 在DDL 里面是 SelfBindKey.
	 */
	...
	{
		if (this.getUIHeight() != 0
				&& this.getUIContralType() == UIContralType.TB)
		{
			return true;
		} else
		{
	...
	public final Entities getHisFKEns()
	{
		if (_HisFKEns == null)
		{
			...
			{
				return null;
			} else if (this.getMyFieldType() == FieldType.FK
					|| this.getMyFieldType() == FieldType.PKFK)
			{
				...
				} else
				{
					_HisFKEns = new GENoNames(this.getUIBindKey(),
							this.getDesc()); // ClassFactory.GetEns(this.UIBindKey);
				}
			} else
			{
				return null;
	...
	public final String get_UIRefParentKeyValue()
	{
		return this._UIRefParentKey;
	}
	...
	/**
	 * 要Bind 的Key. 在TB 里面就是 DataHelpKey 在DDL 里面是SelfBindKey.
	 */
	...
	private String _ChildKey = null;
	private String _GrandChildKey = null;
	private String _ParentKey = null;
	private String _refParentCol;
	...
	public void setChildKey(String _ChildKey)
	{
		this._ChildKey = _ChildKey;
	}
	
	public String getGrandChildKey()
	{
		return _GrandChildKey;
	}
	
	public void setGrandChildKey(String _GrandChildKey)
	{
		this._GrandChildKey = _GrandChildKey;
	}
	...
	public void setParentKey(String _ParentKey)
	{
		this._ParentKey = _ParentKey;
	}
	
	public String getRefParentCol()
	{
		return _refParentCol;
	}
	
	public void setRefParentCol(String _refParentCol)
	{
		this._refParentCol = _refParentCol;
	}
}
