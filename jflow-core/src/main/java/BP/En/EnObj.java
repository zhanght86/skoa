...
public abstract class EnObj implements Serializable
{
	...
	public final String GetMapExtAttrByKey(String key) throws Exception
	{
		...
		return (String) DBAccess.RunSQLReturnVal(
				"select attrValue from Sys_ExtMap WHERE className="
						+ SystemConfig.getAppCenterDBVarStr()
						+ "enName AND attrKey="
						+ SystemConfig.getAppCenterDBVarStr() + "key", ps);
	}
		...
		for (Attr attr : attrs)
		{
			...
			} else if (v.equals("@RDT"))
			{
				if (attr.getUIIsReadonly())
				{
					...
					{
						this.SetValByKey(attr.getKey(),
								DataType.getCurrentDateByFormart("yyyy-MM-dd"));
					}
				...
				} else
				{
					if (StringHelper.isNullOrEmpty(myval) || v.equals(myval))
					{
						if (attr.getMyDataType() == DataType.AppDate)
						{
							this.SetValByKey(attr.getKey(), DataType
	...
	public final void SetValByDesc(String attrDesc, Object val)
	{
		...
		this.getRow().SetValByKey(
				this.getEnMap().GetAttrByDesc(attrDesc).getKey(), val);
	}
	...
	public final void SetDateValByKey(String attrKey, String val)
	{
		...
		{
			throw new RuntimeException("@不合法的日期数据格式:key=[" + attrKey
					+ "],value=" + val + " " + ex.getMessage());
		}
	}
	...
	{
		return DataType.ParseSysDateTime2DateTime(this
				.GetValStringByKey(attrKey));
	}
	...
	public final String GetValDocHtml()
	{
		...
		s = SysDocFile
				.GetValHtmlV2(this.toString(), this.getPKVal().toString());
		this.SetValByKey("Doc", s);
		return s;
	}
		...
		} catch (RuntimeException ex)
		{
			...
			if (!SystemConfig.getIsDebug())
			{
				throw new RuntimeException("@["
						+ this.getEnMap().GetAttrByKey(key).getDesc()
						+ "]请输入数字，您输入的是[" + this.GetValStrByKey(key) + "]。");
			} else
			{
				throw new RuntimeException(
	...
	public final java.math.BigDecimal GetValMoneyByKey(String key)
	{
		try
		{
			return this.GetValDecimalByKey(key).setScale(2,
		...
		} catch (java.lang.Exception e)
		{
			if (this.GetValStringByKey(key).equals(""))
			{
				...
				{
					return new java.math.BigDecimal(attr.getDefaultVal()
							.toString());
				}
			}
	...
	public final double GetValDoubleByKey(String key)
	{
		...
		{
			throw new RuntimeException("@表[" + this.getEnDesc() + "]在获取属性["
					+ key + "]值,出现错误，不能将[" + this.GetValStringByKey(key)
					+ "]转换为double类型.错误信息：" + ex.getMessage());
		}
	}
	...
	public final String GetValAppDateByKey(String key)
	{
		...
		{
			throw new RuntimeException("@实例：[" + this.getEnMap().getEnDesc()
					+ "]  属性[" + key + "]值["
					+ this.GetValStringByKey(key).toString() + "]日期格式转换出现错误："
					+ ex.getMessage());
		}
		// return "2003-08-01";
	}
	...
	public final boolean getIsBlank()
	{
		...
		for (Attr attr : attrs)
		{
			...
			if (str.equals("") || attr.getDefaultVal().toString().equals(str)
					|| str == null)
			{
				continue;
			}
	...
	public final boolean getIsEmpty()
	{
		...
		{
			if (this.getPKVal() == null
					|| this.getPKVal().toString().equals("0")
	...
	{
		if (this.getPK().equals("OID") || this.getPK().equals("No")
				|| this.getPK().equals("MyPK"))
		{
			return 1;
		} else
		{
		...
		if (i == 0)
		{
			throw new RuntimeException("@没有给【" + this.getEnDesc() + "，"
					+ this.getEnMap().getPhysicsTable() + "】定义主键。");
		} else
		{
			return i;
		}
