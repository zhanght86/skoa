...
public class EmpDept extends EntityMM
{
	...
	public Map getEnMap()
	{
		...
		// map.AddTBStringPK(EmpDeptAttr.FK_Emp, null, "Emp", false, false, 1,
		// 15,1);
		// map.AddTBStringPK(EmpDeptAttr.FK_Dept, null, "Dept", false, false, 1,
		// 15,1);
		// map.AddDDLEntitiesPK(EmpDeptAttr.FK_Emp,null,"操作员",new Emps(),true);
		
		map.AddTBStringPK(EmpDeptAttr.FK_Emp, null, "操作员", false, false, 1, 20,
				1);
		map.AddDDLEntitiesPK(EmpDeptAttr.FK_Dept, null, "部门", new Depts(), true);
		
		// map.AddDDLEntitiesPK(EmpDeptAttr.FK_Emp,0, DataType.AppInt,"操作员",new
		// 县局(),"OID","Name",true);
