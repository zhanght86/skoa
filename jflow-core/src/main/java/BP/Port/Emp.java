...
public class Emp extends EntityNoName
{
	 ...
	 * 工作部门集合
	 */
	public final Depts getHisDepts()
	{
		EmpDepts sts = new EmpDepts();
		Depts dpts = sts.GetHisDepts(this.getNo());
		try
		{
			if (dpts.size() == 0)
			{
				String sql = "select fk_dept from port_emp where no='"
						+ this.getNo()
						+ "' and fk_dept in(select no from port_dept)";
				Object tempVar = BP.DA.DBAccess.RunSQLReturnVal(sql);
				String fk_dept = (String) ((tempVar instanceof String) ? tempVar
						: null);
				if (fk_dept == null)
				{
					return dpts;
...
//		}
	...
	protected boolean beforeDelete()
	{
		try
		{
			...
			// 删除部门
			EmpDepts depts = new EmpDepts();
			depts.Delete(EmpDeptAttr.FK_Emp, getNo());
		} catch (Exception e)
		{
		}
	...
	public Map getEnMap()
	{
		...
		// 增加多对多属性
		// 他的部门权限
		map.getAttrsOfOneVSM().Add(new EmpDepts(), new Depts(),
				EmpDeptAttr.FK_Emp, EmpDeptAttr.FK_Dept, DeptAttr.Name,
				DeptAttr.No, "部门权限");
