...
public class WorkFlow
{
	...
	{
		String sql = "SELECT COUNT(*) FROM V_WF_CURRWROKS WHERE FK_Emp='"
				+ FK_Emp + "' AND WorkTimeState=0";
	...
	public final boolean IsCanDoCurrentWork(String empId)
	{
		...
		// 判断他的工作生成的工作者.
		GenerWorkerLists gwls = new GenerWorkerLists(this.getWorkID(), wn
		...
		if (gwls.size() == 0)
		{
			...
			// );
			throw new RuntimeException("@工作流程定义错误,没有找到能够执行此项工作的人员.相关信息:WorkID="
					+ this.getWorkID() + ",NodeID="
					+ wn.getHisNode().getNodeID());
		}
		
		for (Entity obj : gwls.ToJavaList())
		{
	...
	public final String DoReject(long fid, int fk_node, String msg)
	{
		...
		// 更新当前流程管理表的设置当前的节点。
		DBAccess.RunSQL("UPDATE WF_GenerWorkFlow SET FK_Node=" + fk_node
				+ ", NodeName='" + nd.getName() + "' WHERE WorkID="
				+ this.getWorkID());
	...
	public final void DoDeleteWorkFlowByFlag(String msg)
	{
		try
		{
			...
			wn.AddToTrack(ActionType.DeleteFlowByFlag, WebUser.getNo(), WebUser
					.getName(), wn.getHisNode().getNodeID(), wn.getHisNode()
					.getName(), msg);
			
			String sql = "UPDATE  " + this.getHisFlow().getPTable()
					+ " SET WFState=" + WFState.Delete.getValue()
					+ " WHERE OID=" + this.getWorkID();
			DBAccess.RunSQL(sql);
		} catch (RuntimeException ex)
		{
			Log.DefaultLogWriteLine(LogType.Error,
					"@逻辑删除出现错误:" + ex.getMessage());
	...
	public final void DoUnDeleteWorkFlowByFlag(String msg)
	{
		try
		{
			DBAccess.RunSQL("UPDATE WF_GenerWorkFlow SET WFState="
					+ WFState.Runing.getValue() + " WHERE  WorkID="
					+ this.getWorkID());
			...
			WorkNode wn = new WorkNode(getWorkID(), gwf.getFK_Node());
			wn.AddToTrack(ActionType.UnDeleteFlowByFlag, WebUser.getNo(),
					WebUser.getName(), wn.getHisNode().getNodeID(), wn
							.getHisNode().getName(), msg);
		} catch (RuntimeException ex)
		{
			Log.DefaultLogWriteLine(LogType.Error,
					"@逻辑删除出现错误:" + ex.getMessage());
	 ...
	 * @return 删除信息
	 */
	public static String DoDeleteWorkFlowAlreadyComplete(String flowNo,
			long workID, boolean isDelSubFlow, String note)
	{
		Log.DebugWriteInfo("开始删除流程:流程编号:" + flowNo + "-WorkID:" + workID + "-"
				+ ". 是否要删除子流程:" + isDelSubFlow + ";删除原因:" + note);
		...
		DBAccess.RunSQL("DELETE FROM ND" + Integer.parseInt(flowNo)
				+ "Track WHERE WorkID=" + workID);
		DBAccess.RunSQL("DELETE FROM " + fl.getPTable() + " WHERE OID="
				+ workID);
		...
		try
		{
			...
			DBAccess.RunSQL("DELETE FROM WF_GenerWorkFlow WHERE (WorkID="
					+ workID + " OR FID=" + workID + " ) AND FK_Flow='"
					+ flowNo + "'");
			...
			for (Node nd : nds.ToJavaList())
			{
				try
				{
					DBAccess.RunSQL("DELETE FROM ND" + nd.getNodeID()
							+ " WHERE OID=" + workID + " OR FID=" + workID);
				} catch (RuntimeException ex)
				{
		...
		if (isDelSubFlow)
		{
			...
			for (Entity obj : gwfs.ToJavaList())
			{
				GenerWorkFlow item = (GenerWorkFlow) obj;
				BP.WF.Dev2Interface.Flow_DoDeleteFlowByReal(item.getFK_Flow(),
						item.getWorkID(), true);
			}
		}
		// 删除该流程下面的子流程.
		
		BP.DA.Log.DefaultLogWriteLineInfo("@[" + fl.getName() + "]流程被["
				+ WebUser.getNo() + WebUser.getName() + "]删除，WorkID[" + workID
				+ "]。");
	...
	private String DoDeleteSubThread()
	{
		...
		{
			// 删除质量考核信息.
			DBAccess.RunSQL("DELETE FROM WF_CHEval WHERE WorkID="
					+ this.getWorkID()); // 删除质量考核数据。
			
			// 删除抄送信息.
			DBAccess.RunSQL("DELETE FROM WF_CCList WHERE WorkID="
					+ this.getWorkID());
			
			// 删除单据信息.
			DBAccess.RunSQL("DELETE FROM WF_Bill WHERE WorkID="
					+ this.getWorkID());
			// 删除退回.
			DBAccess.RunSQL("DELETE FROM WF_ReturnWork WHERE WorkID="
					+ this.getWorkID());
			// 删除移交.
			// DBAccess.RunSQL("DELETE FROM WF_ForwardWork WHERE WorkID=" +
			// this.WorkID);
			...
			// 删除它的工作.
			// DBAccess.RunSQL("DELETE FROM WF_GenerFH WHERE  FID=" +
			// this.WorkID + " AND FK_Flow='" + this.HisFlow.No + "'");
			DBAccess.RunSQL("DELETE FROM WF_GenerWorkFlow WHERE (WorkID="
					+ this.getWorkID() + " ) AND FK_Flow='"
		...
		{
			String err = "@删除工作流程[" + this.getHisStartWork().getOID() + ","
					+ this.getHisStartWork().getTitle() + "] Err "
					+ ex.getMessage();
			Log.DefaultLogWriteLine(LogType.Error, err);
			throw new RuntimeException(err);
		}
		...
		if (1 == 2)
		{
			...
			//
			sql = "SELECT FK_Node FROM WF_GenerWorkerList WHERE WorkID="
					+ this.getFID() + " AND IsPass=3";
			...
			if (fk_node != 0)
			{
				...
				if (nextNode.getPassRate().compareTo(new BigDecimal(0)) > 0)
				{
					...
					// 处理完成率
					sql = "SELECT COUNT(*) AS Num FROM WF_GenerWorkerList WHERE FK_Node="
							+ priNode.getNodeID()
							+ " AND FID="
							+ this.getFID()
							+ " AND IsPass=1";
					BigDecimal ok = new BigDecimal(
							DBAccess.RunSQLReturnValInt(sql));
					sql = "SELECT COUNT(*) AS Num FROM WF_GenerWorkerList WHERE FK_Node="
							+ priNode.getNodeID() + " AND FID=" + this.getFID();
					BigDecimal all = new BigDecimal(
							DBAccess.RunSQLReturnValInt(sql));
					if (all.equals(0))
					{
						// 说明:所有的子线程都被杀掉了, 就应该整个流程结束。
						WorkFlow wf = new WorkFlow(this.getHisFlow(),
								this.getFID());
						info += "@所有的子线程已经结束。";
						info += "@结束主流程信息。";
						info += "@"
								+ wf.DoFlowOver(ActionType.FlowOver, "合流点流程结束",
										null, null);
					}
			...
			if (fk_node == 0)
			{
				...
				switch (fND.getHisNodeWorkType())
				{
					...
					default:
						...
						// +this.HisGenerWorkFlow +" AND FID=" + this.FID;
		...
		// 写入删除日志.
		wn.AddToTrack(ActionType.DeleteSubThread, empOfWorker.getNo(),
				empOfWorker.getName(), wn.getHisNode().getNodeID(), wn
						.getHisNode().getName(), "子线程被:" + WebUser.getName()
						+ "删除.");
		// 写入删除日志.
	 ...
	 * @return 删除错误会抛出异常
	 */
	public static void DeleteFlowByReal(String flowNo, long workid,
			boolean isDelSubFlow)
	{
		...
		for (Entity obj : fns.ToJavaList())
		{
			...
			try
			{
				...
				DBAccess.RunSQL("DELETE FROM " + md.getPTable() + " WHERE OID="
						+ workid);
		...
		DBAccess.RunSQL("DELETE FROM WF_GenerFH WHERE  FID=" + workid
				+ " AND FK_Flow='" + flowNo + "'");
		DBAccess.RunSQL("DELETE FROM WF_GenerWorkFlow WHERE (WorkID=" + workid
				+ " OR FID=" + workid + " ) AND FK_Flow='" + flowNo + "'");
		DBAccess.RunSQL("DELETE FROM WF_GenerWorkerList WHERE (WorkID="
				+ workid + " OR FID=" + workid + " ) AND FK_Flow='" + flowNo
				+ "'");
		...
		if (1 == 2)
		{
			...
			if (fk_node != 0)
			{
				...
				if (nextNode.getPassRate().compareTo(new BigDecimal(0)) > 0)
				{
					...
					BigDecimal passRate = ok.divide(all).multiply(
							new BigDecimal(100));
					if (nextNode.getPassRate().compareTo(passRate) <= 0)
					{
	...
	public final String DoDeleteWorkFlowByReal(boolean isDelSubFlow)
	{
		...
		if (wn != null)
		{
			wn.getHisFlow().DoFlowEventEntity(EventListOfNode.BeforeFlowDel,
		...
		DBAccess.RunSQL("DELETE FROM " + this.getHisFlow().getPTable()
				+ " WHERE OID=" + this.getWorkID());
		DBAccess.RunSQL("DELETE FROM WF_CHEval WHERE  WorkID="
				+ this.getWorkID()); // 删除质量考核数据。
		
		// 正常的删除信息.
		BP.DA.Log.DefaultLogWriteLineInfo("@[" + this.getHisFlow().getName()
				+ "]流程被[" + WebUser.getNo() + WebUser.getName() + "]删除，WorkID["
				+ this.getWorkID() + "]。");
		String msg = "";
		try
		{
		...
		try
		{
			...
			DBAccess.RunSQL("DELETE FROM WF_GenerFH WHERE  FID="
					...
					+ this.getHisFlow().getNo() + "'");
			DBAccess.RunSQL("DELETE FROM WF_GenerWorkFlow WHERE (WorkID="
					+ this.getWorkID() + " OR FID=" + this.getWorkID()
					+ " ) AND FK_Flow='" + this.getHisFlow().getNo() + "'");
			DBAccess.RunSQL("DELETE FROM WF_GenerWorkerList WHERE (WorkID="
					+ this.getWorkID() + " OR FID=" + this.getWorkID()
					+ " ) AND FK_Flow='" + this.getHisFlow().getNo() + "'");
			...
			for (Node nd : nds.ToJavaList())
			{
				...
				{
					DBAccess.RunSQL("DELETE FROM ND" + nd.getNodeID()
							+ " WHERE OID=" + this.getWorkID() + " OR FID="
							+ this.getWorkID());
				} catch (RuntimeException ex)
				{
					msg += "@ delete data error " + ex.getMessage();
		...
		if (this.getFID() != 0)
		{
			...
			// * 取出来获取停留点,没有获取到说明没有任何子线程到达合流点的位置.
			sql = "SELECT FK_Node FROM WF_GenerWorkerList WHERE WorkID="
					+ wn.getHisWork().getFID() + " AND IsPass=3";
			int fk_node = DBAccess.RunSQLReturnValInt(sql, 0);
			if (fk_node != 0)
			{
				// 说明它是待命的状态
				...
				if (nextNode.getPassRate().compareTo(new BigDecimal(0)) > 0)
				{
					...
					sql = "SELECT COUNT(*) AS Num FROM WF_GenerWorkerList WHERE FK_Node="
							+ priNode.getNodeID()
							+ " AND FID="
							+ wn.getHisWork().getFID() + " AND IsPass=1";
					java.math.BigDecimal ok = new BigDecimal(
							DBAccess.RunSQLReturnValInt(sql));
					sql = "SELECT COUNT(*) AS Num FROM WF_GenerWorkerList WHERE FK_Node="
							+ priNode.getNodeID()
							+ " AND FID="
							+ wn.getHisWork().getFID();
					java.math.BigDecimal all = new BigDecimal(
							DBAccess.RunSQLReturnValInt(sql));
					if (all.equals(0))
					{
						// 说明:所有的子线程都被杀掉了, 就应该整个流程结束。
				...
				switch (fND.getHisNodeWorkType())
				{
					...
					default:
						...
						if (num == 0)
						{
							...
							info += "@结束主流程信息。";
							info += "@"
									+ wf.DoFlowOver(ActionType.FlowOver,
											"主流程结束", null, null);
						}
						break;
				}
		...
		if (wn != null)
		{
			wn.getHisFlow().DoFlowEventEntity(EventListOfNode.AfterFlowDel,
					wn.getHisNode(), wn.getHisWork(), null);
		}
		return info;
	}
	 ...
	 * @return
	 */
	public final String DoDeleteWorkFlowByWriteLog(String info,
			boolean isDelSubFlow)
	{
		GERpt rpt = new GERpt("ND"
				+ Integer.parseInt(this.getHisFlow().getNo()) + "Rpt",
				this.getWorkID());
		WorkFlowDeleteLog log = new WorkFlowDeleteLog();
	...
	public final void DoComeBackWorkFlow(String msg)
	{
		try
		{
			...
			// 增加消息
			WorkNode wn = this.GetCurrentWorkNode();
			BP.WF.GenerWorkerLists wls = new BP.WF.GenerWorkerLists(wn.getHisWork()
					.getOID(), wn.getHisNode().getNodeID());
			if (wls.size() == 0)
			{
				throw new RuntimeException("@恢复流程出现错误,产生的工作者列表");
			}
			BP.WF.MsgsManager.AddMsgs(wls, "恢复的流程", wn.getHisNode().getName(),
					"回复的流程");
		} catch (RuntimeException ex)
		{
			Log.DefaultLogWriteLine(LogType.Error,
					"@恢复流程出现错误." + ex.getMessage());
	...
	public final WorkNode GetCurrentWorkNode()
	{
		...
		if (gwf.RetrieveFromDBSources() == 0)
		{
			this.DoFlowOver(ActionType.FlowOver, "非正常结束，没有找到当前的流程记录。", null,
					null);
			throw new RuntimeException("@"
					+ String.format("工作流程%1$s已经完成。", this.getHisStartWork()
							.getTitle()));
		}
		...
		{
			Log.DefaultLogWriteLineError("@WorkID=" + this.getWorkID()
					+ ",FK_Node=" + gwf.getFK_Node() + ".不应该出现查询不出来工作."); // 没有找到当前的工作节点的数据，流程出现未知的异常。
	...
	{
		// 查询出来有少没有完成的流程。
		int i = BP.DA.DBAccess
				.RunSQLReturnValInt("SELECT COUNT(*) FROM WF_GenerWorkFlow WHERE FID="
						+ gwf.getFID() + " AND WFState!=1");
		switch (i)
		{
			case 0:
			...
			case 1:
				BP.DA.DBAccess
						.RunSQL("DELETE FROM WF_GenerWorkFlow  WHERE FID="
								+ gwf.getFID() + " OR WorkID=" + gwf.getFID());
				BP.DA.DBAccess
						.RunSQL("DELETE FROM WF_GenerWorkerlist WHERE FID="
								+ gwf.getFID() + " OR WorkID=" + gwf.getFID());
				BP.DA.DBAccess.RunSQL("DELETE FROM WF_GenerFH WHERE FID="
						+ gwf.getFID());
			...
			default:
				BP.DA.DBAccess
						.RunSQL("UPDATE WF_GenerWorkFlow SET WFState=1 WHERE WorkID="
								+ this.getWorkID());
				BP.DA.DBAccess
						.RunSQL("UPDATE WF_GenerWorkerlist SET IsPass=1 WHERE WorkID="
								+ this.getWorkID());
				return "@当前的工作已经完成。";
		}
	}
	...
	public final String DoFlowSubOver()
	{
		...
		String sql = "SELECT count(*) FROM WF_GenerWorkFlow WHERE  FID="
				+ this.getFID();
		int num = DBAccess.RunSQLReturnValInt(sql);
	...
	public final String LetParentFlowAutoSendNextSetp()
	{
		...
		// 检查是否是最后的一个.
		int num = BP.WF.Dev2Interface.Flow_NumOfSubFlowRuning(this
				.getHisGenerWorkFlow().getPWorkID(), this.getHisGenerWorkFlow()
				.getWorkID());
		if (num != 0)
		{
			return "";
		...
		// string pEmp =
		// DBAccess.RunSQLReturnStringIsNull("SELECT FK_Emp FROM WF_GenerWorkerList WHERE WorkID="
		// + this.HisGenerWorkFlow.PWorkID + " AND FK_Node=" +
		// this.HisGenerWorkFlow.FK_Node + " AND IsPass=0", null);
		// NDXRptBaseAttr
		...
		try
		{
			...
			{
				throw new RuntimeException("@吊起子流程上的人员编号(" + pEmp
						+ ")已不存在,无法启动父流程.");
			}
			...
			SendReturnObjs objs = BP.WF.Dev2Interface.Node_SendWork(this
					.getHisGenerWorkFlow().getPFlowNo(), pGWF.getWorkID(), rpt
					.getRow(), null, 0, null, emp.getNo(), emp.getName(), emp
					.getFK_Dept(), emp.getFK_DeptText(), null);
	 ...
	 * @return
	 */
	public final String DoFlowOver(ActionType at, String stopMsg,
			Node currNode, GERpt rpt)
	{
		if (null == currNode)
		{
			return null;
		...
		// 调用结束前事件.
		this.getHisFlow().DoFlowEventEntity(EventListOfNode.FlowOverBefore,
				currNode, rpt, null);
		...
		if (Glo.getIsDeleteGenerWorkFlow() == true)
		{
			// 是否删除流程注册表的数据？
			ps = new Paras();
			ps.SQL = "DELETE FROM WF_GenerWorkFlow WHERE WorkID=" + dbstr
		...
		} else
		{
			// 求出参与人,以方便已经完成的工作查询.
			ps = new Paras();
			ps.SQL = "SELECT EmpFrom FROM ND"
					+ Integer.parseInt(this._HisFlow.getNo())
					+ "Track WHERE WorkID=" + dbstr + "WorkID OR FID=" + dbstr
					+ "FID ";
			...
			// 更新流程注册信息.
			ps = new Paras();
			ps.SQL = "UPDATE WF_GenerWorkFlow SET WFState=" + dbstr
					+ "WFState,WFSta=" + dbstr + "WFSta,Emps=" + dbstr
		...
		// 设置流程完成状态.
		ps = new Paras();
		ps.SQL = "UPDATE " + this.getHisFlow().getPTable() + " SET WFState="
				+ dbstr + "WFState WHERE OID=" + dbstr + "OID";
		ps.Add("WFState", WFState.Complete.getValue());
		ps.Add("OID", this.getWorkID());
		DBAccess.RunSQL(ps);
		
		// 加入轨迹.
		WorkNode wn = new WorkNode(getWorkID(), this.getHisGenerWorkFlow()
				.getFK_Node());
		wn.AddToTrack(at, WebUser.getNo(), WebUser.getName(), wn.getHisNode()
				.getNodeID(), wn.getHisNode().getName(), stopMsg);
		
		// 调用结束后事件.
		this.getHisFlow().DoFlowEventEntity(EventListOfNode.FlowOverAfter,
				currNode, rpt, null);
		// 处理后续的业务.
		...
		ps = new Paras();
		ps.SQL = "UPDATE ND" + Integer.parseInt(currNode.getFK_Flow())
				...
				+ "WorkID AND ActionType=" + ActionType.WorkCheck.getValue();
	...
	public final String GenerFHStartWorkInfo()
	{
		...
		DataTable dt = DBAccess
				.RunSQLReturnTable("SELECT Title,RDT,Rec,OID FROM ND"
						+ this.getStartNodeID() + " WHERE FID=" + this.getFID());
		switch (dt.Rows.size())
		{
			case 0:
				Node nd = new Node(this.getStartNodeID());
				throw new RuntimeException("@没有找到他们开始节点的数据，流程异常。FID="
						+ this.getFID() + "，节点：" + nd.getName() + "节点ID："
						+ nd.getNodeID());
			case 1:
				msg = String.format(
						"@发起人： %1$s  日期：%2$s 发起的流程 标题：%3$s ，已经成功完成。", dt.Rows
								.get(0).get("Rec").toString(), dt.Rows.get(0)
								.get("RDT").toString(),
						dt.Rows.get(0).get("Title").toString());
				break;
			default:
				msg = "@下列(" + dt.Rows.size() + ")位人员发起的流程已经完成。";
				for (DataRow dr : dt.Rows)
				{
					msg += "<br>发起人：" + dr.get("Rec") + " 发起日期："
							+ dr.get("RDT") + " 标题：" + dr.get("Title")
							+ "<a href='./../../WF/WFRpt.jsp?WorkID="
							+ dr.get("OID") + "&FK_Flow="
							+ this.getHisFlow().getNo()
							+ "' target=_blank>详细...</a>";
				}
	...
	public final String CCTo(DataTable dt)
	{
		...
		pss.Add("Title", "工作流抄送：工作名称:" + this.getHisFlow().getName()
				+ "，最后处理人：" + WebUser.getName());
		pss.Add("Context",
				"工作轨迹 http://" + ip + "/WF/WFRpt.jsp?WorkID="
						+ this.getWorkID() + "&FID=0");
		...
		{
			return "@抄送出现错误，没有把该流程的信息抄送到(" + empsExt + ")请联系管理员检查系统异常"
					+ ex.getMessage();
		}
	}
	...
	public final String DoFix(String fixMsg)
	{
		...
		Paras ps = new Paras();
		ps.SQL = "UPDATE WF_GenerWorkFlow SET WFState=" + dbstr
				+ "WFState WHERE WorkID=" + dbstr + "WorkID";
		...
		// 记录日志..
		WorkNode wn = new WorkNode(this.getWorkID(), this.getHisGenerWorkFlow()
				.getFK_Node());
	...
	public final WorkNodes getHisWorkNodesOfWorkID()
	{
		if (this._HisWorkNodesOfWorkID == null)
		{
			...
			this._HisWorkNodesOfWorkID.GenerByWorkID(this.getHisFlow(),
					this.getWorkID());
		}
	...
	public final WorkNodes getHisWorkNodesOfFID()
	{
		if (this._HisWorkNodesOfFID == null)
		{
			...
			this._HisWorkNodesOfFID
					.GenerByFID(this.getHisFlow(), this.getFID());
		}
	...
	public WorkFlow(String fk_flow, long wkid)
	{
		...
		this.getHisGenerWorkFlow().RetrieveByAttr(GenerWorkerListAttr.WorkID,
				wkid);
	...
	public final StartWork getHisStartWork()
	{
		...
		{
			StartWork en = (StartWork) this.getHisFlow().getHisStartNode()
					.getHisWork();
	...
	public final boolean getIsComplete()
	{
		...
		// bool s =
		// !DBAccess.IsExits("select workid from WF_GenerWorkFlow WHERE WorkID="
		// + this.WorkID + " AND FK_Flow='" + this.HisFlow.No + "'");
	...
	public static boolean IsCanDoWorkCheckByEmpStation(int nodeId, String empId)
	{
		boolean isCan = false;
		// 判断岗位对应关系是不是能够执行.
		String sql = "SELECT a.FK_Node FROM WF_NodeStation a,  "
				+ BP.WF.Glo.getEmpStation()
				+ " b WHERE (a.FK_Station=b.FK_Station) AND (a.FK_Node="
				+ nodeId + " AND b.FK_Emp='" + empId + "' )";
		...
		sql = "select FK_Node from WF_NodeStation WHERE FK_Node=" + nodeId
				+ " AND ( FK_Station in (select FK_Station from "
				+ BP.WF.Glo.getEmpStation() + " WHERE FK_Emp='" + empId
				+ "') ) ";
	...
	{
		String sql = "select a.FK_Node, b.EmpID from WF_NodeStation  a,  "
				+ BP.WF.Glo.getEmpStation()
				+ " b WHERE (a.FK_Station=b.FK_Station) AND (a.FK_Node="
				+ nodeId + " )";
	...
	public final String DoHungUp(HungUpWay way, String relData, String hungNote)
	{
		...
		// 获取它的工作者，向他们发送消息。
		GenerWorkerLists wls = new GenerWorkerLists(this.getWorkID(), this
				.getHisFlow().getNo());
		/*String url = Glo.getServerIP() + "/" + this.getVirPath()
				+ this.getAppType() + "/WorkOpt/OneWork/Track.jsp?FK_Flow="*/
			String url = Glo.getCCFlowAppPath()+"WF/WorkOpt/OneWork/Track.jsp?FK_Flow="
				+ this.getHisFlow().getNo() + "&WorkID=" + this.getWorkID()
				+ "&FID=" + this.getHisGenerWorkFlow().getFID() + "&FK_Node="
				+ this.getHisGenerWorkFlow().getFK_Node();
		String mailDoc = "详细信息:<A href='" + url + "'>打开流程轨迹</A>.";
		String title = "工作:" + this.getHisGenerWorkFlow().getTitle() + " 被"
				+ WebUser.getName() + "挂起" + hungNote;
		String emps = "";
		for (Entity obj : GenerWorkerLists.convertEntities(wls))
		{
			...
			BP.WF.Dev2Interface.Port_SendMsg(wl.getFK_Emp(), title, mailDoc,
					"HungUp" + wl.getWorkID(), SMSMsgType.Self,
					wl.getFK_Flow(), wl.getFK_Node(), wl.getWorkID(),
					wl.getFID());
		}
		...
		// 更新工作者的挂起时间。
		ps = new Paras();
		ps.SQL = "UPDATE WF_GenerWorkerlist SET DTOfHungUp=" + dbstr
				+ "DTOfHungUp,DTOfUnHungUp=" + dbstr
				+ "DTOfUnHungUp, HungUpTimes=HungUpTimes+1 WHERE FK_Node="
		...
		ps.Add(GenerWorkerListAttr.FK_Node, this.getHisGenerWorkFlow()
				.getFK_Node());
		ps.Add(GenerWorkFlowAttr.WorkID, this.getWorkID());
		DBAccess.RunSQL(ps);
		...
		WorkNode wn = new WorkNode(this.getWorkID(), this.getHisGenerWorkFlow()
				.getFK_Node());
		wn.AddToTrack(ActionType.HungUp, WebUser.getNo(), WebUser.getName(), wn
				.getHisNode().getNodeID(), wn.getHisNode().getName(), hungNote);
	...
	public final String DoUnHungUp()
	{
		...
		// 更新工作者的挂起时间。
		ps = new Paras();
		ps.SQL = "UPDATE WF_GenerWorkerlist SET  DTOfUnHungUp=" + dbstr
				+ "DTOfUnHungUp WHERE FK_Node=" + dbstr + "FK_Node AND WorkID="
				+ dbstr + "WorkID";
		...
		// 更新他的主键。
		ps = new Paras();
		ps.SQL = "UPDATE WF_HungUp SET MyPK="
				+ SystemConfig.getAppCenterDBVarStr() + "MyPK WHERE MyPK="
				+ dbstr + "MyPK1";
		...
		String mailDoc = "详细信息:<A href='" + url + "'>打开流程轨迹</A>.";
		String title = "工作:" + this.getHisGenerWorkFlow().getTitle() + " 被"
				+ WebUser.getName() + "解除挂起.";
		String emps = "";
		for (Entity obj : wls.ToJavaList())
		{
			GenerWorkerList wl = (GenerWorkerList) obj;
			if (wl.getIsEnable() == false)
			{
			...
			// 写入消息。
			BP.WF.Dev2Interface.Port_SendMsg(wl.getFK_Emp(), title, mailDoc,
					"HungUp" + wl.getFK_Node() + this.getWorkID(),
					SMSMsgType.Self, getHisGenerWorkFlow().getFK_Flow(),
					getHisGenerWorkFlow().getFK_Node(), this.getWorkID(),
					this.getFID());
		...
		WorkNode wn = new WorkNode(this.getWorkID(), this.getHisGenerWorkFlow()
				.getFK_Node());
		wn.AddToTrack(ActionType.UnHungUp, WebUser.getNo(), WebUser.getName(),
	...
	public final String DoUnShift()
	{
		...
		wn.AddToTrack(ActionType.UnShift, WebUser.getNo(), WebUser.getName(),
				nd.getNodeID(), nd.getName(), "撤消移交");
		...
		if (wls.size() == 1)
		{
			...
			wl.setIsPass(false);
			wl.Update();
			return "@撤消移交成功，<a href='" + Glo.getCCFlowAppPath()
					+ "WF/MyFlow.jsp?FK_Flow=" + this.getHisFlow().getNo()
					+ "&FK_Node=" + wl.getFK_Node() + "&FID=" + wl.getFID()
					+ "&WorkID=" + this.getWorkID() + "'><img src='"
		...
		if (mywl != null)
		{
			return "@撤消移交成功，<a href='" + Glo.getCCFlowAppPath()
					+ "WF/MyFlow.jsp?FK_Flow=" + this.getHisFlow().getNo()
					+ "&FK_Node=" + mywl.getFK_Node() + "&FID=" + mywl.getFID()
					+ "&WorkID=" + this.getWorkID() + "'><img src='"
					+ Glo.getCCFlowAppPath()
					+ "WF/Img/Btn/Do.gif' border=0/>执行工作</A>";
		}
		...
		// 删除撤销信息.
		BP.DA.DBAccess.RunSQL("DELETE FROM WF_ShiftWork WHERE WorkID="
				+ this.getWorkID() + " AND FK_Node=" + wk.getFK_Node());
		
		return "@撤消移交成功，<a href='" + Glo.getCCFlowAppPath()
				+ "WF/MyFlow.jsp?FK_Flow=" + this.getHisFlow().getNo()
				+ "&FK_Node=" + wk.getFK_Node() + "&FID=" + wk.getFID()
				+ "&WorkID=" + this.getWorkID() + "'><img src='"
				+ Glo.getCCFlowAppPath()
				+ "WF/Img/Btn/Do.gif' border=0/>执行工作</A>";
	}
