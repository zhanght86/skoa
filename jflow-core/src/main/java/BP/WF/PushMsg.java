...
///#endregion
		...
		if (this.getMailPushWay() != 0)
		{
			...
			if (this.getFK_Event().equals(BP.Sys.Frm.EventListOfNode.WorkArrive) || this.getFK_Event().equals(BP.Sys.Frm.EventListOfNode.ReturnAfter))
			{
				/*工作到达.*/
				if (this.getMailPushWay() == 1)
				{
					...
					for (String emp : emps)
					{
						...
						//发送邮件.
							BP.WF.Dev2Interface.Port_SendEmail(empEn.getEmail(), mailTitleTmp, mailDocReal, "ToDo", "WKAlt" + currNode.getNodeID() + "_" + workid);
					}
					generAlertMessage += "@已向:{" + toEmpIDs + "}发送提醒邮件，由 " + this.getFK_Event() + " 发出.";
				}
				...
				if (this.getMailPushWay() == 2)
				{
					...
					//发送邮件
						BP.WF.Dev2Interface.Port_SendEmail(emailAddress, mailTitleTmp, mailDocTmp, "ToDo", "WKAlt" + currNode.getNodeID() + "_" + workid);
						generAlertMessage += "@已向:{" + emailAddress + "}发送提醒邮件，由 " + this.getFK_Event() + " 发出.";
				}
			}
			///#endregion 发送成功事件.
			...
			if (this.getFK_Event().equals(EventListOfNode.SendSuccess))
			{
				...
				if (this.getMailPushWay() == 1 && objs.getVarAcceptersID() != null)
				{
					...
					for (String emp : emps)
					{
						...
						//发送邮件.
						BP.WF.Dev2Interface.Port_SendEmail(empEn.getEmail(), mailTitleTmp, mailDocReal, "ToDo", "WKAlt" + objs.getVarToNodeID() + "_" + workid);
					}
				...
				if (this.getMailPushWay() == 2)
				{
					...
					//发送邮件
					BP.WF.Dev2Interface.Port_SendEmail(emailAddress, mailTitleTmp, mailDocTmp, "ToDo", "WKAlt" + objs.getVarToNodeID() + "_" + workid);
		...
		if (this.getSMSPushWay() != 0)
		{
			...
			if (this.getFK_Event().equals(EventListOfNode.WorkArrive) || this.getFK_Event().equals(EventListOfNode.ReturnAfter))
			{
				...
				if (this.getMailPushWay() == 2)
				{
					...
					BP.WF.Dev2Interface.Port_SendSMS(tel, smsDocTmp, msgType, "WKAlt" + currNode.getNodeID() + "_" + workid);
			...
			if (this.getFK_Event().equals(EventListOfNode.SendSuccess))
			{
				...
				if (this.getMailPushWay() == 2)
				{
					...
					{
						//发送短信.
						BP.WF.Dev2Interface.Port_SendSMS(tel, smsDocTmp, "ToDo", "WKAlt" + objs.getVarToNodeID() + "_" + workid);
