...
import BP.Sys.Frm.FrmImgAttr;
import BP.Sys.Frm.FrmImgs;
import BP.Sys.Frm.FrmType;
import BP.Sys.Frm.ImgAppType;
...
import BP.WF.Template.FindWorker;
...
public class WorkNode
{
	...
	*/
	public final GenerWorkerLists Func_GenerWorkerLists(WorkNode town) throws RuntimeException
	{
		this.town = town;
	 ...
	 * @throws ParseException 
	*/
	public final GenerWorkerLists Func_GenerWorkerList_DiGui(String deptNo, String empNo) throws ParseException
	{
		String sql = "SELECT NO FROM Port_Emp WHERE No IN " + "(SELECT  FK_Emp  FROM " + BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN (SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + dbStr + "FK_Node ) )" + " AND  NO IN " + "(SELECT  FK_Emp  FROM Port_EmpDept WHERE FK_Dept=" + dbStr + "FK_Dept )" + " AND No!=" + dbStr + "FK_Emp";
		...
		if (dt.Rows.size() == 0)
		{
			...
			else
			{
				...
				sql += "(SELECT FK_Emp FROM Port_EmpDept WHERE FK_Emp!=" + dbStr + "FK_Emp AND FK_Dept = '" + deptP.getParentNo() + "')";
			}
			...
			if (dt.Rows.size() == 0)
			{
				...
				if (dt.Rows.size() == 0)
				{
					throw new RuntimeException("@岗位(" + town.getHisNode().getHisStationsStr() + ")下没有人员，对应节点:" + town.getHisNode().getName());
				}
			}
			return InitWorkerLists(town, dt);
		}
	 ...
	 * @throws IOException 
	*/
	public final void DoPrint() throws IOException
	{
...
//ORIGINAL LINE: private GenerWorkerLists InitWorkerLists(WorkNode town, DataTable dt, Int64 fid=0)
	private GenerWorkerLists InitWorkerLists(WorkNode town, DataTable dt, long fid) throws RuntimeException
	{
		if (dt.Rows.size() == 0)
		{
		...
		else
		{
			...
			//增加天数. 考虑到了节假日.
			dtOfShould = Glo.AddDayHoursSpan(new Date(), this.town.getHisNode().getTSpanDay(),this.town.getHisNode().getTSpanMinues());
		}

		//求警告日期.
		Date dtOfWarning = new Date();
		if (this.town.getHisNode().getTSpanDay() == 0 && this.town.getHisNode().getTSpanHour() == 0)
		{
		...
		else
		{
			//计算警告日期。
			// 增加小时数. 考虑到了节假日.
			dtOfWarning = Glo.AddDayHoursSpan(DateUtils.getCurrentDate(), this.town.getHisNode().getTSpanDay(), this.town.getHisNode().getTSpanMinues());
		}
		...
		if (dt.Rows.size() == 1)
		{
			...
			if (this.IsHaveSubThreadGroupMark == true)
			{
				...
				if (StringHelper.isNullOrEmpty(wl.getGroupMark()))
				{
					throw new RuntimeException("@｛" + wl.getFK_Emp() + "｝分组标记中没有值,会导致无法按照分组标记去生成子线程,请检查配置的信息是否正确.");
				}
			}

			if (this.IsHaveSubThreadGroupMark == true && town.getHisNode().getHisFormType() == NodeFormType.SheetAutoTree)
			{
				/*是分组标记，并且是自动表单树.*/
				wl.setGroupMark(dt.Rows.get(0).getValue(2).toString()); //第3个列是分组标记.
		...
		else
		{
			...
			if (this.town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySelected || this.town.getHisNode().getIsAllowRepeatEmps() == true || town.getHisNode().getIsRememberMe() == false) //允许接受人员重复
			{
				if (rm != null)
				{
					rm.setObjs("");
			...
			for (DataRow dr : dt.Rows)
			{
				...
				if (this.IsHaveSubThreadGroupMark == true)
				{
					...
					if (dt.Columns.size() == 4 && this.town.getHisNode().getHisFormType() == NodeFormType.SheetAutoTree)
					{
						...
						if (StringHelper.isNullOrEmpty(dr.getValue(3).toString()))
						{
				...
				else
				{
					if (dt.Columns.size() == 3 && this.town.getHisNode().getHisFormType() == NodeFormType.SheetAutoTree)
					{
						...
						if (StringHelper.isNullOrEmpty(dr.getValue(2).toString()))
						{
							throw new RuntimeException("@组织的接受人数据源不正确,表单IDs不能为空.");
						}
					}
		...
		{
			throw new RuntimeException("@根据[" + this.town.getHisNode().getHisRunModel() + "]部门产生工作人员出现错误，流程[" + this.getHisWorkFlow().getHisFlow().getName() + "],中节点[" + town.getHisNode().getName() + "]定义错误,没有找到接受此工作的工作人员.");
		}
		...
		{
			GenerWorkerList wl = (GenerWorkerList)((this.HisWorkerLists.get(0) instanceof GenerWorkerList) ? this.HisWorkerLists.get(0) : null);
			this.AddToTrack(at, wl.getFK_Emp(), wl.getFK_EmpText(), wl.getFK_Node(), wl.getFK_NodeText(), null, this.getndFrom());
		}
		else
		{
			...
			this.AddToTrack(at, this.getExecer(), "多人接受(见信息栏)", town.getHisNode().getNodeID(), town.getHisNode().getName(), info, this.getndFrom());
		}
		///#endregion
	...
	*/
	public final Dept getHisDeptOfUse()
	{
		if (_HisDeptOfUse == null)
		{
			...
			if (depts.size() == 0)
			{
				throw new RuntimeException("您没有给[" + this.getHisWork().getRec() + "]设置部门。");
			}
			...
			{
					/* 如果还没找到，就返回第一个部门。 */
				_HisDeptOfUse = (Dept)((depts.get(0) instanceof Dept) ? depts.get(0) : null);
			}
		}
		return _HisDeptOfUse;
	}
	...
	*/
	public final Conds getHisFlowCompleteConditions()
	{
		...
		{
			_HisFlowCompleteConditions = new Conds(CondType.Flow, this.getHisNode().getNodeID(), this.getWorkID(), this.rptGe);
		}
	...
	public final Node NodeSend_GenerNextStepNode()
	{
		...
		//if (toNDs.Count == 1)
		//{
		//    Node mynd = toNDs.get(0) as Node;
		//    //写入到达信息.
		//    this.addMsg(SendReturnMsgFlag.VarToNodeID, mynd.NodeID.ToString(), mynd.NodeID.ToString(),
		//     SendReturnMsgType.SystemMsg);
		//    this.addMsg(SendReturnMsgFlag.VarToNodeName, mynd.getName(), mynd.getName(), SendReturnMsgType.SystemMsg);
		//    return mynd;
		//}
		...
		//写入到达信息.
		this.addMsg(SendReturnMsgFlag.VarToNodeID, String.valueOf(nd.getNodeID()), String.valueOf(nd.getNodeID()), SendReturnMsgType.SystemMsg);
		this.addMsg(SendReturnMsgFlag.VarToNodeName, nd.getName(), nd.getName(), SendReturnMsgType.SystemMsg);
		return nd;
	}
	/** 
	 知否执行了跳转.
...
//					if (this.getHisWork().getEnMap().getPhysicsTable() == nd.getHisWork().getEnMap().getPhysicsTable())
//					{
//						/*这是数据合并模式, 就不执行copy*/
//					}
//					else
...
//							if (athDBs.size() > 0)
//							{
//								athDBs.Delete(FrmAttachmentDBAttr.FK_MapData, "ND" + nd.getNodeID(), FrmAttachmentDBAttr.RefPKVal, this.getWorkID());
//
//								/*说明当前节点有附件数据*/
...
//
//									// athDB_N.getMyPK()(this.WorkID + "_" + idx + "_" + athDB_N.FK_MapData);
//									// if (athDB.dbt
//									// athDB_N.getMyPK()(athDB_N.FK_FrmAttachment.replace("ND" + this.HisNode.NodeID, "ND" + nd.NodeID) + "_" + this.WorkID);
//
//									athDB_N.setMyPK(DBAccess.GenerGUID()); // athDB_N.FK_FrmAttachment.replace("ND" + this.HisNode.NodeID, "ND" + nd.NodeID) + "_" + this.WorkID);
//
//									athDB_N.setFK_FrmAttachment(athDB_N.getFK_FrmAttachment().replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID()));
//
//									athDB_N.Save();
//								}
//							}
...
//									athDB_N.setFK_MapData("ND" + nd.getNodeID());
//									athDB_N.setRefPKVal(String.valueOf(this.getWorkID()));
//									athDB_N.setMyPK(this.getWorkID() + "_" + idx + "_" + athDB_N.getFK_MapData()); // athDB_N.FK_FrmAttachment.replace("ND" + this.HisNode.NodeID, "ND" + nd.NodeID) + "_" + this.WorkID);
//									athDB_N.setFK_FrmImgAth(athDB_N.getFK_FrmImgAth().replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID()));
//
//									athDB_N.Save();
//								}
//							}
...
//							if (eleDBs.size() > 0)
//							{
//								eleDBs.Delete(FrmEleDBAttr.FK_MapData, "ND" + nd.getNodeID(), FrmEleDBAttr.RefPKVal, this.getWorkID());
//
//								/*说明当前节点有附件数据*/
...
//									m2.setFK_MapData(m2.getFK_MapData().replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID()));
//									m2.InitMyPK();
//									try
//									{
//										m2.DirectInsert();
...
//						String recDtlLog = "@记录测试明细表Copy过程,从节点ID:" + this.getHisNode().getNodeID() + " WorkID:" + this.getWorkID() + ", 到节点ID=" + nd.getNodeID();
//						if (dtls.size() > 0)
//						{
...
//							{
//								startDtls = new MapDtls("ND" + Integer.parseInt(nd.getFK_Flow()) + "01");
//							}
//
...
//								//    continue;
//
//								//Sys.MapDtl toDtl = (Sys.MapDtl)toDtls.get(i);
//
//
//								i++;
//								//if (toDtls.Count <= i)
...
//								{
//									if (todtl.getName().substring(6, todtl.getName().length()).equals(dtl.getName().substring(6, dtl.getName().length())))
//									{
//										toDtl = todtl;
//										break;
//									}
...
//								DBAccess.RunSQL("DELETE FROM " + toDtl.getPTable() + " WHERE RefPK=" + dbStr + "RefPK", "RefPK", String.valueOf(this.getWorkID()));
//
//								// copy数量.
//								int deBugNumCopy = 0;
...
//												m2m_N.setMyPK(toDtl.getNo() + "_" + m2m.getM2MNo() + "_" + gedtl.toString() + "_" + m2m.getDtlObj());
...
//									}
//									DBAccess.RunSQL("UPDATE " + startDtl.getPTable() + " SET Rec='" + startUser + "',Checker='" + this.getExecer() + "' WHERE BatchID=" + this.getWorkID() + " AND Rec='" + this.getExecer() + "'");
//								}
//								///#endregion 如果启用了审核机制
//							}
//						}
...
//				}
			...
			{
				if (nd.getHisWhenNoWorker() == WhenNoWorker.Skip)
				{
					this.AddToTrack(ActionType.Skip, this.getExecer(), this.getExecerName(), nd.getNodeID(), nd.getName(), "自动跳转.(当没有找到处理人时)", getndFrom());
					setndFrom(nd);
					continue;
				}
				else
			...
			if (nd.getAutoJumpRole0())
			{
				...
				{
					/*如果发现了，当前人员包含处理人集合.*/
					this.AddToTrack(ActionType.Skip, this.getExecer(), this.getExecerName(), nd.getNodeID(), nd.getName(), "自动跳转,(处理人就是提交人)", getndFrom());
					setndFrom(nd);
					continue;
				}
			...
			if (nd.getAutoJumpRole1())
			{
				...
				if (isHave == true)
				{
					this.AddToTrack(ActionType.Skip, this.getExecer(), this.getExecerName(), nd.getNodeID(), nd.getName(), "自动跳转.(处理人已经出现过)", getndFrom());
					setndFrom(nd);
					continue;
				}
			...
			if (nd.getAutoJumpRole2())
			{
				...
				if (isHave == true)
				{
					...
					for (FrmImg img : imgs.ToJavaList())
					{
						...
						//如果设置了部门与岗位的集合进行拆分
						if (!StringHelper.isNullOrEmpty(img.getTag0()) && img.getTag0().contains("^") && img.getTag0().split("[^]", -1).length == 4)
						{
					...
					this.AddToTrack(ActionType.Skip, this.getExecer(), this.getExecerName(), nd.getNodeID(), nd.getName(), "自动跳转.(处理人与上一步相同)", getndFrom());
					setndFrom(nd);
					continue;
				}
	...
	*/
	public final void DealReturnOrderTeamup()
	{
		...
		this.addMsg(SendReturnMsgFlag.OverCurr, "@当前工作已经发送给退回人(" + gwl.getFK_Emp() + "," + gwl.getFK_EmpText() + ").", null, SendReturnMsgType.Info);
	...
	*/
	private Node NodeSend_GenerNextStepNode_Ext(Node currNode)
	{
		...
		for (Node nd : nds.ToJavaList())
		{
			...
			if (dcs.size() == 0)
			{
				throw new RuntimeException("@流程设计错误：从节点(" + currNode.getName() + ")到节点(" + nd.getName() + ")，没有设置方向条件，有分支的节点必须有方向条件。");
				// throw new Exception(string.Format(this.ToE("WN10", "@定义节点的方向条件错误:没有给从{0}节点到{1},定义转向条件."), this.HisNode.NodeID + this.HisNode.getName(), nd.NodeID + nd.getName()));
			}
		...
		if (myNodes.size() == 0)
		{
			throw new RuntimeException("@当前用户(" + this.getExecerName() + "),定义节点的方向条件错误:从{" + currNode.getNodeID() + currNode.getName() + "}节点到其它节点,定义的所有转向条件都不成立.");
		}
	...
	*/
	public final Nodes Func_GenerNextStepNodes()
	{
		...
		if (myNodes.size() == 0)
		{
			throw new RuntimeException(String.format("@定义节点的方向条件错误:没有给从%1$s节点到其它节点,定义转向条件或者您定义的所有转向条件都不成立.", this.getHisNode().getNodeID() + this.getHisNode().getName()));
		}
		return myNodes;
	}
	/** 
	...
	*/
	private void Func_CheckCompleteCondition()
	{
		...
		try
		{
			...
			{
				/* 如果流程完成 */
				String overMsg = this.getHisWorkFlow().DoFlowOver(ActionType.FlowOver, "符合流程完成条件", this.getHisNode(), this.rptGe);
			...
			if (this.getHisNode().getIsCCFlow() && this.getHisFlowCompleteConditions().getIsPass())
			{
				...
				/* 如果流程完成 */
				String overMsg = this.getHisWorkFlow().DoFlowOver(ActionType.FlowOver, "符合流程完成条件:" + stopMsg, this.getHisNode(), this.rptGe);
				this.setIsStopFlow(true);

				// string path = BP.Sys.Glo.Request.ApplicationPath;
				String mymsg = "@符合工作流程完成条件" + stopMsg + overMsg;
				String mymsgHtml = mymsg + "@查看<img src='" + getVirPath() + "WF/Img/Btn/PrintWorkRpt.gif' ><a href='" + getVirPath() + "WF/WFRpt.jsp?WorkID=" + this.getHisWork().getOID() + "&FID=" + this.getHisWork().getFID() + "&FK_Flow=" + this.getHisNode().getFK_Flow() + "' target='_self' >工作轨迹</a>";
				this.addMsg(SendReturnMsgFlag.FlowOver, mymsg, mymsgHtml, SendReturnMsgType.Info);
			}
		}
		catch (RuntimeException ex)
		{
			throw new RuntimeException(String.format("@判断流程{0}完成条件出现错误." + ex.getMessage(), this.getHisNode().getName()));
		}
		///#endregion
	}
	/** 
	...
	*/
	private String Func_DoSetThisWorkOver()
	{
		...
		this.rptGe.SetValByKey(GERptAttr.FlowDaySpan, DataType.GetSpanDays(rptGe.getFlowStartRDT(), DataType.getCurrentDataTime()));
	 ...
	 * @throws Exception 
	*/
	private void NodeSend_11(Node toND) throws RuntimeException
	{
		String sql = "";
		String errMsg = "";
		...
		if (this.getHisWork().getEnMap().getPhysicsTable() != toWK.getEnMap().getPhysicsTable())
		{
			...
			//为下一步骤初始化数据.
			GenerWorkerList gwl = (GenerWorkerList)((current_gwls.get(0) instanceof GenerWorkerList) ? current_gwls.get(0) : null);
        ...
        {
			String htmlInfo = String.format("@发送给%1$s如下%2$s位处理人,%3$s.", this.nextStationName, 
					String.valueOf(this.HisRememberMe.getNumOfObjs()), this.getHisGenerWorkFlow().getGuestNo() + "  " + this.getHisGenerWorkFlow().getGuestName());

			String textInfo = String.format("@发送给%1$s如下%2$s位处理人,%3$s.", this.nextStationName, 
		...
		if (this.HisWorkerLists.size() >= 2 && this.getHisNode().getIsTask())
		{
			if (WebUser.getIsWap())
			{
				this.addMsg(SendReturnMsgFlag.AllotTask, null, "<a href=\"" + this.getVirPath() + "WF/WorkOpt/AllotTask.jsp?WorkID=" + this.getWorkID() + "&NodeID=" + toND.getNodeID() + "&FK_Flow=" + toND.getFK_Flow() + "')\"><img src='" + getVirPath() + "WF/Img/AllotTask.gif' border=0/>指定特定的处理人处理</a>。", SendReturnMsgType.Info);
			}
			else
			{
				this.addMsg(SendReturnMsgFlag.AllotTask, null, "<a href=\"javascript:WinOpen('" + getVirPath() + "WF/WorkOpt/AllotTask.jsp?WorkID=" + this.getWorkID() + "&NodeID=" + toND.getNodeID() + "&FK_Flow=" + toND.getFK_Flow() + "')\"><img src='" + getVirPath() + "WF/Img/AllotTask.gif' border=0/>指定特定的处理人处理</a>。", SendReturnMsgType.Info);
			}
		}

		//if (WebUser.IsWap == false)
		//    this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href=\"javascript:WinOpen('" + VirPath + "WF/Msg/SMS.jsp?WorkID=" + this.WorkID + "&FK_Node=" + toND.NodeID + "');\" ><img src='" + VirPath + "WF/Img/SMS.gif' border=0 />发手机短信提醒他(们)</a>", SendReturnMsgType.Info);
		...
		if (this.getHisNode().getHisFormType() != NodeFormType.SDKForm || 1 == 1)
		{
			if (this.getHisNode().getIsStartNode())
			{
				if (WebUser.getIsWap())
				{
					this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='" + getVirPath() + "WF/Wap/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getHisWork().getOID() + "&FK_Flow=" + toND.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + getVirPath() + "WF/Wap/MyFlow.jsp?FK_Flow=" + toND.getFK_Flow() + "&FK_Node=" + toND.getFK_Flow() + "01'><img src='" + getVirPath() + "WF/Img/New.gif' border=0/>新建流程</a>。", SendReturnMsgType.Info);
				}
				else
				{
					this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='" + this.getVirPath() + this.getAppType() + "/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getHisWork().getOID() + "&FK_Flow=" + toND.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + getVirPath() + "WF/MyFlow.jsp?FK_Flow=" + toND.getFK_Flow() + "&FK_Node=" + toND.getFK_Flow() + "01'><img src='" + getVirPath() + "WF/Img/New.gif' border=0/>新建流程</a>。", SendReturnMsgType.Info);
				}
			}
			else
			{
				this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='" + this.getVirPath() + this.getAppType() + "/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getHisWork().getOID() + "&FK_Flow=" + toND.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>。", SendReturnMsgType.Info);
			}
		}
		...
		//ps.Add("NodeName", toND.getName());
		//ps.Add("WorkID", this.HisWork.getOID());
		//DBAccess.RunSQL(ps);

		if (this.getHisNode().getHisFormType() == NodeFormType.SDKForm || this.getHisNode().getHisFormType() == NodeFormType.SelfForm)
		{
		}
		else
		{
			this.addMsg(SendReturnMsgFlag.WorkRpt, null, "@<img src='" + getVirPath() + "WF/Img/Btn/PrintWorkRpt.gif' ><a href='" + getVirPath() + "WF/WFRpt.jsp?WorkID=" + this.getHisWork().getOID() + "&FID=" + this.getHisWork().getFID() + "&FK_Flow=" + toND.getFK_Flow() + "' target='_self' >工作轨迹</a>。");
		}
		this.addMsg(SendReturnMsgFlag.WorkStartNode, "@下一步[" + toND.getName() + "]工作成功启动.", "@下一步<font color=blue>[" + toND.getName() + "]</font>工作成功启动.");
		///#endregion
	...
	private void NodeSend_2X_GenerFH()
	{
		...
		if (this.getHisNode().getIsStartNode() || fh.getIsExits() == false)
		{
			...
			catch (RuntimeException ex)
			{
				...
				int i = attr.Retrieve(MapAttrAttr.FK_MapData, attr.getFK_MapData(), MapAttrAttr.KeyOfEn, attr.getKeyOfEn());
				if (i == 0)
				{
					attr.setKeyOfEn("Title");
				...
				fh.setTitle(this.getExecer() + "-" + this.getExecerName() + " @ " + DataType.getCurrentDataTime() + " ");
			}
	...
	*/
	private void NodeSend_24_UnSameSheet(Nodes toNDs) 
	{
		...
		FrmEleDBs eleDBs = new FrmEleDBs("ND" + this.getHisNode().getNodeID(), String.valueOf(this.getWorkID()));
//C# TO JAVA CONVERTER 
		///#endregionTODO TASK: There is no preprocessor in Java:
	...
	*/
	private void NodeSend_24_SameSheet(Node toNode)
	{
		...
		{
			this.getHisGenerWorkFlow().setTitle(WorkNode.GenerTitle(this.getHisFlow(), this.getHisWork()));
		}
		...
		current_gwls.Delete(GenerWorkerListAttr.FK_Node, this.getHisNode().getNodeID(), GenerWorkerListAttr.FID, this.getWorkID()); //首先清除.
		...
		*/
		...
		for (GenerWorkerList wl : current_gwls.ToJavaList())
		{
		  ...
		  //  mywk.Copy(this.HisWork);  // 复制过来信息。
			...
			if (IsHaveFH)
			{
				...
				else
				{
					...
					for (MapDtl dtl : dtlsFrom.ToJavaList())
					{
						...
						String sql = "SELECT * FROM " + dtl.getPTable() + " WHERE Rec='" + wl.getFK_Emp() + "' AND RefPK='" + this.getWorkID() + "'";
						...
						if (myDT.Rows.size() == 1)
						{
							...
							for (Attr attr : attrs)
							{
								...
								if (key.equals(GEDtlAttr.FID) || key.equals(GEDtlAttr.OID) || key.equals(GEDtlAttr.Rec) || key.equals(GEDtlAttr.RefPK))
								{
									continue;
								}
			...
			if (isHaveFLDtl == true)
			{
				...
				for (MapDtl dtl : this.getHisWork().getHisMapDtls().ToJavaList())
				{
					...
					for (GEDtl gedtl : gedtls.ToJavaList())
					{
						...
						if (StringHelper.isNullOrEmpty(dtl.getSubThreadGroupMark()) == false)
						{
			...
			if (isGroupMarkWorklist == false)
			{
				...
				if (athDBs.size() > 0)
				{
					...
					for (FrmAttachmentDB athDB : athDBs.ToJavaList())
					{
						...
						athDB_N.setFK_FrmAttachment(athDB_N.getFK_FrmAttachment().replace("ND" + this.getHisNode().getNodeID(),"ND" + toNode.getNodeID()));
					...
					for (FrmImg img : this.getHisNode().getMapData().getFrmImgs().ToJavaList())
					{
						...
						if (eleDBs.size() > 0)
						{
							...
							for (FrmEleDB eleDB : eleDBs.ToJavaList())
							{
								FrmEleDB eleDB_N = new FrmEleDB();
								eleDB_N.Copy(eleDB);
								eleDB_N.setFK_MapData(img.getEnPK().replace("ND" + this.getHisNode().getNodeID(), "ND" + toNode.getNodeID()));
				...
				if (this.getHisNode().getMapData().getFrmImgAths().size() > 0)
				{
					...
					if (frmImgAthDBs.size() > 0)
					{
						...
						for (FrmImgAthDB imgAthDB : frmImgAthDBs.ToJavaList())
						{
							...
							imgAthDB_N.setFK_FrmImgAth(imgAthDB_N.getFK_FrmImgAth().replace("ND" + this.getHisNode().getNodeID(), "ND" + toNode.getNodeID()));
							imgAthDB_N.Save();
						}
					}
				}
					...
					for (MapDtl dtl : dtlsFrom.ToJavaList())
					{
						...
						for (GEDtl gedtl : gedtls.ToJavaList())
						{
							if(gedtl.getRefPK().equals(String.valueOf(mywk.getOID())))
							{
								...
								{
									/*如果启用了多附件,就复制这条明细数据的附件信息。*/
									athDBs = new FrmAttachmentDBs(dtl.getNo(), gedtl.getOID());
									if (athDBs.size() > 0)
									{
										i = 0;
										for (FrmAttachmentDB athDB : athDBs.ToJavaList())
			...
			if (gwf.RetrieveFromDBSources() == 0)
			{
				...
				if (this.getHisNode().getIsStartNode())
				{
					gwf.setTitle(WorkNode.GenerTitle(this.getHisFlow(), this.getHisWork()) + "(" + wl.getFK_EmpText() + ")");
				}
				else
				{
				...
				gwf.setSysType(toNode.getHisFlow().getSysType());
		...
		///#region 处理消息提示
		this.addMsg("FenLiuInfo", "@分流节点:"+toNode.getName()+"已经发起,@发送给如下("
			+ this.HisRememberMe.getNumOfObjs() + ")个处理人"+this.HisRememberMe.getEmpsExt()+".");
		...
		if (this.getHisNode().getIsStartNode())
		{
			...
			{
				this.addMsg("AllotTask", "@<img src='" + getVirPath() + "WF/Img/AllotTask.gif' border=0 /><a href=\"" + getVirPath() + "WF/WorkOpt/AllotTask.jsp?WorkID=" + this.getWorkID() + "&FID=" + this.getWorkID() + "&NodeID=" + toNode.getNodeID() + "\" >修改接受对象</a>.");
			}
		}
		...
		if (this.getHisNode().getIsStartNode())
		{
			if (WebUser.getIsWap())
			{
				this.addMsg("UnDoNew", "@<a href='" + getVirPath() + "WF/Wap/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getWorkID() + "&FK_Flow=" + toNode.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + getVirPath() + "WF/Wap/MyFlow.jsp?FK_Flow=" + toNode.getFK_Flow() + "&FK_Node=" + toNode.getFK_Flow() + "01' ><img src='" + getVirPath() + "WF/Img/New.gif' border=0/>新建流程</a>。");
			}
			else
			{
				this.addMsg("UnDoNew", "@<a href='" + this.getVirPath() + this.getAppType() + "/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getWorkID() + "&FK_Flow=" + toNode.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>， <a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + toNode.getFK_Flow() + "&FK_Node=" + toNode.getFK_Flow() + "01' ><img src='" + getVirPath() + "WF/Img/New.gif' border=0/>新建流程</a>。");
			}
		}
		else
		{
			this.addMsg("UnDo", "@<a href='" + this.getVirPath() + this.getAppType() + "/MyFlowInfo.jsp?DoType=UnSend&WorkID=" + this.getWorkID() + "&FK_Flow=" + toNode.getFK_Flow() + "'><img src='" + getVirPath() + "WF/Img/Action/UnSend.png' border=0/>撤销本次发送</a>。");
		}

		this.addMsg("Rpt", "@<a href='" + getVirPath() + "WF/WFRpt.jsp?WorkID=" + this.getWorkID() + "&FID=" + wk.getFID() + "&FK_Flow=" + this.getHisNode().getFK_Flow() + "'target='_self' >工作轨迹</a>");
		///#endregion 处理消息提示
	}
	/** 
	 合流点到普通点发送
	...
	*/
	private void NodeSend_53_SameSheet_To_HeLiu(Node toNode)
	{
		...
		if (fid == 0)
		{
			...
			String strs = BP.DA.DBAccess.RunSQLReturnStringIsNull("SELECT FID FROM WF_GenerWorkFlow WHERE WorkID=" + this.getHisWork().getOID(), "0");
			if (strs.equals("0"))
			{
		...
		if (myfh.getFK_Node() == toNode.getNodeID())
		{
			...
			ps.Add("FID", this.getHisWork().getFID());
			java.math.BigDecimal all = new BigDecimal(DBAccess.RunSQLReturnValInt(ps));;
			if (all.compareTo(new BigDecimal(0)) == 0)
			{
				throw new RuntimeException("@获取总子线程数量出现错误,线程数量为0,执行的sql:" + ps.SQL + " FID=" + this.getHisWork().getFID());
			}
			...
			if (toNode.getPassRate().compareTo(passRate) <= 0)
			{
				...
				DBAccess.RunSQL("UPDATE WF_GenerWorkerList SET IsPass=0  WHERE FK_Node=" + dbStr + "FK_Node AND WorkID=" + dbStr + "WorkID", "FK_Node", toNode.getNodeID(), "WorkID", this.getHisWork().getFID());
			...
			if (myfh.getToEmpsMsg().contains("("))
			{
			   // 实际值的格式model:   (liyan,李言) . 要获取里面的编号值。
				...
				this.AddToTrack(ActionType.ForwardHL, myemps, myfh.getToEmpsMsg(), toNode.getNodeID(), toNode.getName(), null);
			...
			this.addMsg("ToHeLiuEmp", "@流程已经运行到合流节点[" + toNode.getName() + "]。@您的工作已经发送给如下人员[" + myfh.getToEmpsMsg() + "]。" + this.GenerWhySendToThem(this.getHisNode().getNodeID(), toNode.getNodeID()) + numStr);
		}
		else
		{
...
///#warning 对于多个分合流点可能会有问题。
			sql1 = "SELECT COUNT(distinct WorkID) AS Num FROM WF_GenerWorkerList WHERE IsEnable=1 AND FID=" + this.getHisWork().getFID() + " AND FK_Node IN (" + spanNodes + ")";
...
///#warning 为了不让其显示在途的工作需要， =3 不是正常的处理模式。
			...
			this.addMsg("InfoToHeLiu", "@流程已经运行到合流节点[" + toNode.getName() + "]。@您的工作已经发送给如下人员[" + toEmpsStr + "]，@您是第一个到达此节点的处理人.");
		}
	...
	*/
			...
			case FL: // 2: 分流节点向下发送的
				...
				if (toNDs.size() == 1)
				{
					...
					switch (toND2.getHisRunModel())
					{
						...
						//  throw new Exception("@流程设计错误:请检查流程获取详细信息, 分流点(" + this.HisNode.getName() + ")下面不能连接分流节点(" + toND2.getName() + ").");
						case HL: //2.3 分流点to合流点,分合流点.
						case FHL:
							this.NodeSend_11(toND2); // 按普通节点到普通节点处理.
							break;
						// throw new Exception("@流程设计错误:请检查流程获取详细信息, 分流点(" + this.HisNode.getName() + ")下面不能连接合流节点(" + toND2.getName() + ").");
					...
					for (Node nd : toNDs.ToJavaList())
					{
						switch (nd.getHisRunModel())
						{
							...
							case HL:
								NodeSend_11(nd); //按普通节点到普通节点处理.
								break;
							//    throw new Exception("@流程设计错误:请检查流程获取详细信息, 分流点(" + this.HisNode.getName() + ")下面不能连接分流节点(" + nd.getName() + ").");
							//case RunModel.FHL:
							//    throw new Exception("@流程设计错误:请检查流程获取详细信息, 分流点(" + this.HisNode.getName() + ")下面不能连接分合流节点(" + nd.getName() + ").");
							//case RunModel.HL:
							//    throw new Exception("@流程设计错误:请检查流程获取详细信息, 分流点(" + this.HisNode.getName() + ")下面不能连接合流节点(" + nd.getName() + ").");
							default:
								break;
						}
			...
			case HL: // 3: 合流节点向下发送
				...
				switch (toND3.getHisRunModel())
				{
					...
					case FL: //3.2 分流点
						this.NodeSend_31(toND3); // 让它与普通点点普通点一样的逻辑.
						break;
					//throw new Exception("@流程设计错误:请检查流程获取详细信息, 合流点(" + this.HisNode.getName() + ")下面不能连接分流节点(" + toND3.getName() + ").");
					case HL: //3.3 合流点
					case FHL:
						this.NodeSend_31(toND3); // 让它与普通点点普通点一样的逻辑.
						break;
					//throw new Exception("@流程设计错误:请检查流程获取详细信息, 合流点(" + this.HisNode.getName() + ")下面不能连接合流节点(" + toND3.getName() + ").");
					case SubThread: //3.4 子线程
						throw new RuntimeException("@流程设计错误:请检查流程获取详细信息, 合流点(" + this.getHisNode().getName() + ")下面不能连接子线程节点(" + toND3.getName() + ").");
					default:
			...
			case FHL: // 4: 分流节点向下发送的
				...
				switch (toND4.getHisRunModel())
				{
					case Ordinary: //4.1 普通工作节点
						...
						break;
					case FL: //4.2 分流点
						throw new RuntimeException("@流程设计错误:请检查流程获取详细信息, 合流点(" + this.getHisNode().getName() + ")下面不能连接分流节点(" + toND4.getName() + ").");
					case HL: //4.3 合流点
					case FHL:
						this.NodeSend_11(toND4); // 让它与普通点点普通点一样的逻辑.
						break;
					//  throw new Exception("@流程设计错误:请检查流程获取详细信息, 合流点(" + this.HisNode.getName() + ")下面不能连接合流节点(" + toND4.getName() + ").");
					case SubThread: //4.5 子线程
						...
						//    NodeSend_24_UnSameSheet(toNDs); /*可能是只发送1个异表单*/
						break;
					//throw new Exception("@流程设计错误:请检查流程获取详细信息, 合流点(" + this.HisNode.getName() + ")下面不能连接子线程节点(" + toND4.getName() + ").");
					default:
			...
			case SubThread: // 5: 子线程节点向下发送的
				...
				switch (toND5.getHisRunModel())
				{
					case Ordinary: //5.1 普通工作节点
						throw new RuntimeException("@流程设计错误:请检查流程获取详细信息, 子线程点(" + this.getHisNode().getName() + ")下面不能连接普通节点(" + toND5.getName() + ").");
					case FL: //5.2 分流点
						throw new RuntimeException("@流程设计错误:请检查流程获取详细信息, 子线程点(" + this.getHisNode().getName() + ")下面不能连接分流节点(" + toND5.getName() + ").");
					case HL: //5.3 合流点
					case FHL: //5.4 分合流点
					...
					case SubThread: //5.5 子线程
						...
						{
							throw new RuntimeException("@流程设计模式错误：连续两个子线程的子线程模式不一样，从节点(" + this.getHisNode().getName() + ")到节点(" + toND5.getName() + ")");
						}
						break;
					default:
	...
	public final void CopyData(Work toWK, Node toND, boolean isSamePTable)
	{
		...
		if (this.getHisNode().getMapData().getFrmAttachments().size() > 0)
		{
			...
			if (athDBs.size() > 0)
			{
				athDBs.Delete(FrmAttachmentDBAttr.FK_MapData, "ND" + toND.getNodeID(), FrmAttachmentDBAttr.RefPKVal, this.getWorkID());
				...
				for (FrmAttachmentDB athDB : athDBs.ToJavaList())
				{
					...
					athDB_N.setFK_FrmAttachment(athDB_N.getFK_FrmAttachment().replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID()));
		...
		if (this.getHisNode().getMapData().getFrmImgAths().size() > 0)
		{
			...
			if (athDBs.size() > 0)
			{
				...
				for (FrmImgAthDB athDB : athDBs.ToJavaList())
				{
					...
					athDB_N.setFK_FrmImgAth(athDB_N.getFK_FrmImgAth().replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID()));
					athDB_N.Save();
				}
			}
		}
		...
		if (this.getHisNode().getMapData().getFrmImgs().size() > 0)
		{
			for (FrmImg img : this.getHisNode().getMapData().getFrmImgs().ToJavaList())
			{
				...
				if (eleDBs.size() > 0)
				{
					eleDBs.Delete(FrmEleDBAttr.FK_MapData, img.getEnPK().replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID()), FrmEleDBAttr.EleID, this.getWorkID());
					...
					for (FrmEleDB eleDB : eleDBs.ToJavaList())
					{
						FrmEleDB eleDB_N = new FrmEleDB();
						eleDB_N.Copy(eleDB);
						eleDB_N.setFK_MapData(img.getEnPK().replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID()));
						eleDB_N.setRefPKVal(img.getEnPK().replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID()));
						eleDB_N.GenerPKVal();
						eleDB_N.Save();
					}
				}
		...
		if (this.getHisNode().getMapData().getMapM2Ms().size() > 0)
		{
			...
			if (m2ms.size() >= 1)
			{
				for (M2M item : m2ms.ToJavaList())
				{
					...
					m2.Copy(item);
					m2.setEnOID(this.getWorkID());
					m2.setFK_MapData(m2.getFK_MapData().replace("ND" + this.getHisNode().getNodeID(), "ND" + toND.getNodeID()));
					m2.InitMyPK();
					try
					{
						m2.DirectInsert();
		...
		// int deBugDtlCount=
		MapDtls dtls = this.getHisNode().getMapData().getMapDtls();
		String recDtlLog = "@记录测试明细表Copy过程,从节点ID:" + this.getHisNode().getNodeID() + " WorkID:" + this.getWorkID() + ", 到节点ID=" + toND.getNodeID();
		if (dtls.size() > 0)
		{
			...
			if (isEnablePass) // 如果有就建立它开始节点表数据
			{
				startDtls = new MapDtls("ND" + Integer.parseInt(toND.getFK_Flow()) + "01");
			}
			...
			for (MapDtl dtl : dtls.ToJavaList())
			{
				...
				for (MapDtl todtl : toDtls.ToJavaList())
				{
					...
					try
					{
						...
						toDtlName = todtl.getHisGEDtl().FK_MapDtl.substring(todtl.getHisGEDtl().FK_MapDtl.indexOf("Dtl"), todtl.getHisGEDtl().FK_MapDtl.length());
						dtlName = dtl.getHisGEDtl().FK_MapDtl.substring(dtl.getHisGEDtl().FK_MapDtl.indexOf("Dtl"), dtl.getHisGEDtl().FK_MapDtl.length());
					}
				...
				for (GEDtl gedtl : gedtls.ToJavaList())
				{
					...
					{
						/*如果启用了多附件,就复制这条明细数据的附件信息。*/
						FrmAttachmentDBs athDBs = new FrmAttachmentDBs(dtl.getNo(), gedtl.getOID());
						if (athDBs.size() > 0)
						{
							i = 0;
							for (FrmAttachmentDB athDB : athDBs.ToJavaList())
							{
								...
								athDB_N.Copy(athDB);
								athDB_N.setFK_MapData(toDtl.getNo());
								athDB_N.setMyPK(athDB.getMyPK() + "_" + dtCopy.getOID() + "_" + (new Integer(i)).toString());
								athDB_N.setFK_FrmAttachment(athDB_N.getFK_FrmAttachment().replace("ND" + this.getHisNode().getNodeID(),
												"ND" + toND.getNodeID()));
								athDB_N.setRefPKVal(String.valueOf(dtCopy.getOID()));
								try
								{
					...
					if (toDtl.getIsEnableM2M() || toDtl.getIsEnableM2MM())
					{
						...
						if (m2ms.size() > 0)
						{
							...
							for (M2M m2m : m2ms.ToJavaList())
							{
								...
								m2m_N.Copy(m2m);
								m2m_N.setFK_MapData(toDtl.getNo());
								m2m_N.setMyPK(m2m.getMyPK() + "_" + m2m.getM2MNo() + "_" + gedtl.toString() + "_" + m2m.getDtlObj());
								m2m_N.setEnOID(gedtl.getOID());
								m2m.InitMyPK();
								try
								{
...
///#warning 记录日志.
	...
	private void CallSubFlow() throws Exception
	{
		...
		for (String item : paras)
		{
			...
			for (String keyval : keyvals)
			{
				...
				String strs0 = strs[0];
				if (strs0.equals("FlowNo"))
				{
					FlowNo = strs[1];
				} else if (strs0.equals("EmpField"))
				{
					EmpField = strs[1];
				} else if (strs0.equals("DtlTable"))
				{
					DtlTable = strs[1];
				} else 
				{
	...
	*/
	public final void DoRefFunc_Listens()
	{
		...
		lts.RetrieveByLike(ListenAttr.Nodes, "%" + this.getHisNode().getNodeID() + "%");
		String info = "";
		for (Listen lt : lts.ToJavaList())
		{
			ps = new Paras();
			...
			{
				String FK_Emp = (String)((dr.getValue("FK_Emp") instanceof String) ? dr.getValue("FK_Emp") : null);

				Object tempVar = lt.getTitle();
				String title = (String)((tempVar instanceof String) ? tempVar : null);
				title = title.replace("@WebUser.getNo()", this.getExecer());
				title = title.replace("@WebUser.getName()", this.getExecerName());
				title = title.replace("@WebUser.getFK_Dept()", this.getExecerDeptNo());
				title = title.replace("@WebUser.getFK_DeptName()", this.getExecerDeptName());

				Object tempVar2 = lt.getDoc();
				String doc = (String)((tempVar2 instanceof String) ? tempVar2 : null);
				doc = doc.replace("@WebUser.getNo()", this.getExecer());
				doc = doc.replace("@WebUser.getName()", this.getExecerName());
				doc = doc.replace("@WebUser.getFK_Dept()", this.getExecerDeptNo());
				doc = doc.replace("@WebUser.getFK_DeptName()", this.getExecerDeptName());

				Attrs attrs = this.rptGe.getEnMap().getAttrs();
				for (Attr attr : attrs)
				{
					title = title.replace("@" + attr.getKey(), this.rptGe.GetValStrByKey(attr.getKey()));
					doc = doc.replace("@" + attr.getKey(), this.rptGe.GetValStrByKey(attr.getKey()));
				}

				if (this.town == null)
				{
					BP.WF.Dev2Interface.Port_SendMsg(FK_Emp, title, doc, "LS" + FK_Emp + "_" + this.getWorkID(), BP.WF.SMSMsgType.Self, this.getHisFlow().getNo(), this.getHisNode().getNodeID(), this.getWorkID(), 0);
				}
				else
				{
					BP.WF.Dev2Interface.Port_SendMsg(FK_Emp, title, doc, "LS" + FK_Emp + "_" + this.getWorkID(), BP.WF.SMSMsgType.Self, this.getHisFlow().getNo(), this.town.getHisNode().getNodeID(), this.getWorkID(), 0);
				}
	...
	public final void CheckFrm1ToN()
	{
		...
		for (FrmNode fn : frmNDs.ToJavaList())
		{
			...
			for (GenerWorkerList item : current_gwls.ToJavaList())
			{
				...
				//    {
				//        geEn.SetValByKey(attr.getKey(), BP.DA.DataType.getCurrentDataTime());
				//        continue;
				//    }

				//    if (attr.getDefaultValOfReal() == "@WebUser.getNo()")
				//    {
				//        geEn.SetValByKey(attr.getKey(), item.FK_Emp);
				//        continue;
				//    }

				//    if (attr.getDefaultValOfReal() == "@WebUser.getName()")
				//    {
				//        geEn.SetValByKey(attr.getKey(), item.FK_EmpText);
				//        continue;
				//    }

				//    if (attr.getDefaultValOfReal() == "@WebUser.getFK_Dept()")
				//    {
				//        Emp emp = new Emp(item.FK_Emp);
				//        geEn.SetValByKey(attr.getKey(), emp.FK_Dept);
				//        continue;
				//    }

				//    if (attr.getDefaultValOfReal() == "@WebUser.getFK_DeptName()")
				//    {
				//        Emp emp = new Emp(item.FK_Emp);
	...
	*/
	public final boolean CheckFrmIsNotNull()
	{
		...
		//增加节点表单的必填项判断.
		String err = "";
		if (this.getHisNode().getHisFormType() == NodeFormType.FreeForm || this.getHisNode().getHisFormType() == NodeFormType.FixForm)
		{
			...
			for (MapAttr attr : attrs.ToJavaList())
			{
				...
				String str = (String)((row.get(attr.getKeyOfEn().toLowerCase()) instanceof String) ? row.get(attr.getKeyOfEn().toLowerCase()) : null);
				/*如果是检查不能为空 */
				if (str == null || StringHelper.isNullOrEmpty(str) == true || str.trim().equals(""))
				{
		...
		for (FrmNode item : frmNDs.ToJavaList())
		{
			...
			for (FrmField ff : ffs.ToJavaList())
			{
				...
				if (ff.getIsNotNull() == true && Glo.getIsEnableCheckFrmTreeIsNull() == true)
				{
					/*如果是检查不能为空 */
					if (StringHelper.isNullOrEmpty(val) == true || val.trim().equals(""))
					{
				...
				if (ff.getIsWriteToFlowTable() == true)
				{
					...
					//this.rptGe.SetValByKey(ff.getKeyOfEn(), val);
				}
			}
		}
		if (!err.equals(""))
	...
	public final boolean DealOradeNode()
	{
		...
		for (GenerWorkerList gwl : gwls.ToJavaList())
		{
			if (gwl.getIsPassInt() > 10)
			{
				...
				//写入日志.
				this.AddToTrack(ActionType.Order, gwl.getFK_Emp(), gwl.getFK_EmpText(), this.getHisNode().getNodeID(), this.getHisNode().getName(), "队列发送");

				this.addMsg(SendReturnMsgFlag.VarAcceptersID, gwl.getFK_Emp(), gwl.getFK_Emp(), SendReturnMsgType.SystemMsg);
				this.addMsg(SendReturnMsgFlag.VarAcceptersName, gwl.getFK_EmpText(), gwl.getFK_EmpText(), SendReturnMsgType.SystemMsg);
				this.addMsg(SendReturnMsgFlag.OverCurr, "当前工作已经发送给(" + gwl.getFK_Emp() + "," + gwl.getFK_EmpText() + ").", null, SendReturnMsgType.Info);
	...
	*/
	private boolean CheckBlockModel()
	{
		...
		if (this.getHisNode().getBlockModel() == BlockModel.CurrNodeAll)
		{
			...
			{
				err += "@流程ID=" + gwf.getWorkID() + ",标题:" + gwf.getTitle() + ",当前执行人:" + gwf.getTodoEmps() + ",运行到节点:" + gwf.getNodeName();
			}
		...
		if (this.getHisNode().getBlockModel() == BlockModel.SpecSubFlow)
		{
			...
			for (String str : strs)
			{
				...
				for (GenerWorkFlow gwf : gwls.ToJavaList())
				{
					err += "@子流程ID=" + gwf.getWorkID() + ",子流程名称" + gwf.getFlowName() + ",标题:" + gwf.getTitle() + ",当前执行人:" + gwf.getTodoEmps() + ",运行到节点:" + gwf.getNodeName();
				}
			}
		...
		{
			/*按 sql 判断阻塞*/
			java.math.BigDecimal d = DBAccess.RunSQLReturnValDecimal(Glo.DealExp(this.getHisNode().getBlockExp(), this.rptGe, null), new BigDecimal(0), 1);
	...
	*/
	public final SendReturnObjs NodeSend(Node jumpToNode, String jumpToEmp)
	{
		...
		if (this.getHisNode().getIsGuestNode())
		{
			...
			{
				throw new RuntimeException("@当前节点（" + this.getHisNode().getName() + "）是客户执行节点，所以当前登录人员应当是Guest,现在是:" + this.getExecer());
			}
		}
	  ...
	  //      throw new Exception("@当前工作您已经处理完成，或者您(" + this.Execer + " " + this.ExecerName + ")没有处理当前工作的权限。");
		...
		String sendWhen = this.getHisFlow().DoFlowEventEntity(EventListOfNode.SendWhen, this.getHisNode(), this.getHisWork(), null, jumpToNode, JumpToEmp);
		...
		if (this.getHisNode().getIsStartNode())
		{
			...
			{
				throw new RuntimeException("@违反了流程发起限制条件:" + Glo.DealExp(this.getHisFlow().getStartLimitAlert(), this.getHisWork(), null));
			}
		}
		...
		if (this.getHisNode().getIsStartNode() == false && this.getHisGenerWorkFlow().getWFState() == WFState.Askfor)
		{
			...
			GenerWorkerLists gwls = new GenerWorkerLists();
			gwls.Retrieve(GenerWorkerListAttr.FK_Node, this.getHisNode().getNodeID(), GenerWorkerListAttr.WorkID, this.getWorkID());
			...
			for (GenerWorkerList item : gwls.ToJavaList())
			{
				if (item.getIsPassInt() == AskforHelpSta.AfterDealSend.getValue())
				{
					...
					DBAccess.RunSQL("UPDATE WF_GenerWorkerList SET IsPass=1 WHERE FK_Node=" + this.getHisNode().getNodeID() + " AND WorkID=" + this.getWorkID());

					//写入日志.
					this.AddToTrack(ActionType.ForwardAskfor, item.getFK_Emp(), item.getFK_EmpText(), this.getHisNode().getNodeID(), this.getHisNode().getName(), "加签后向下发送，直接发送给下一步处理人。");
				}
				...
				if (item.getIsPassInt() == AskforHelpSta.AfterDealSendByWorker.getValue())
				{
					...
					//让加签人，设置成工作未读。
					BP.WF.Dev2Interface.Node_SetWorkUnRead(this.getHisNode().getNodeID(), this.getWorkID(), item.getFK_Emp());
					...
					//    this.HisNode.NodeID, this.HisNode.getName(),
					//    "加签后向下发送，并转向加签人发起人（" + item.FK_Emp + "，" + item.FK_EmpText + "）。<br>意见:" + replyInfo);

					//写入日志.
					this.AddToTrack(ActionType.ForwardAskfor, item.getFK_Emp(), item.getFK_EmpText(), this.getHisNode().getNodeID(), this.getHisNode().getName(), "回复意见:" + replyInfo);

					//加入系统变量。
					this.addMsg(SendReturnMsgFlag.VarToNodeID, String.valueOf(this.getHisNode().getNodeID()), SendReturnMsgType.SystemMsg);
					this.addMsg(SendReturnMsgFlag.VarToNodeName, this.getHisNode().getName(), SendReturnMsgType.SystemMsg);
					...
					//加入提示信息.
					this.addMsg(SendReturnMsgFlag.SendSuccessMsg, "已经转给，加签的发起人(" + item.getFK_Emp() + "," + item.getFK_EmpText() + ")", SendReturnMsgType.Info);
		...
		if (this.getHisNode().getIsHL() || this.getHisNode().getHisRunModel() == RunModel.FHL)
		{
			...
			if (dtWL.Rows.size() != 0)
			{
				if (this.getHisNode().getThreadKillRole() == ThreadKillRole.None || this.getHisNode().getThreadKillRole() == ThreadKillRole.ByHand)
				{
					...
					{
						infoErr += "@操作员编号:" + dr.getValue("FK_Emp") + "," + dr.getValue("FK_EmpText") + ",停留节点:" + dr.getValue("FK_NodeText");
					}
				...
				if (this.getHisNode().getThreadKillRole() == ThreadKillRole.ByAuto)
				{
					...
					for (DataRow dr : dtWL.Rows)
					{
						BP.WF.Dev2Interface.Flow_DeleteSubThread(this.getHisFlow().getNo(), Long.parseLong(dr.getValue(0).toString()), "合流点发送时自动删除");
					}
				}
			}
		}
		...
		try
		{
			...
			if (this.getHisGenerWorkFlow().getWFState() == WFState.ReturnSta)
			{
				...
				if (mydt.Rows.size() != 0)
				{
					...
					if (this.getHisGenerWorkFlow().getTransferCustomType() == TransferCustomType.ByCCBPMDefine)
					{
						...
						if (this.JumpToNode.getTodolistModel() == TodolistModel.Order || this.JumpToNode.getTodolistModel() == TodolistModel.Teamup)
						{
							/*如果是多人处理节点.*/
					...
					if (this.getHisGenerWorkFlow().getTransferCustomType() == TransferCustomType.ByWorkerSet)
					{
						...
						if (this.getHisGenerWorkFlow().getTodolistModel() == TodolistModel.Order || this.getHisGenerWorkFlow().getTodolistModel() == TodolistModel.Teamup)
						{
							/*如果是多人处理节点.*/
							this.DealReturnOrderTeamup();
			...
			{
				throw new RuntimeException("@流程出现的错误,工作ID=" + this.getWorkID() + ",当前活动点(" + this.getHisGenerWorkFlow().getFK_Node() + this.getHisGenerWorkFlow().getNodeName() + ")与发送点(" + this.getHisNode().getNodeID() + this.getHisNode().getName() + ")不一致");
			}
			...
			if (jumpToNode == null && this.getHisGenerWorkFlow().getTransferCustomType() == TransferCustomType.ByWorkerSet)
			{
				...
				if (_transferCustom == null)
				{
					...
					String msg1 = this.getHisWorkFlow().DoFlowOver(ActionType.FlowOver, "流程已经按照设置的步骤成功结束。", this.getHisNode(), this.rptGe);
			...
			String mymsgHtml = "@查看<img src='" + getVirPath() + "WF/Img/Btn/PrintWorkRpt.gif' ><a href='" + getVirPath() + "WF/WFRpt.jsp?WorkID=" + this.getHisWork().getOID() + "&FID=" + this.getHisWork().getFID() + "&FK_Flow=" + this.getHisNode().getFK_Flow() + "' target='_self' >工作轨迹</a>";
			...
			else
			{
				...
				//计算从发送到现在的天数.
				//this.rptGe.setFlowDaySpan(DataType.GetSpanDays(this.getHisGenerWorkFlow().getRDT()),DateUtils.getCurrentDate());
				this.rptGe.Update();
			...
			///#region 生成单据
			if (this.getHisNode().getHisPrintDocEnable() == PrintDocEnable.PrintRTF && this.getHisNode().getBillTemplates().size() > 0)
			{
					...
					if (func.getHisBillFileType() != BillFileType.RuiLang)
					{
						...
						try
						{
							...
							if (func.getHisBillFileType() == BillFileType.PDF)
							{
								...
								billInfo += "<img src='" + getVirPath() + "WF/Img/FileType/PDF.gif' /><a href='" + billUrl + "' target=_blank >" + func.getName() + "</a>";
							}
							else
							{
								billInfo += "<img src='" + getVirPath() + "WF/Img/FileType/doc.gif' /><a href='" + billUrl + "' target=_blank >" + func.getName() + "</a>";
							}
			...
			if (this.getHisNode().getSFActiveFlows().length() > 2)
			{
				...
				{
					if (StringHelper.isNullOrEmpty(flowNo) == true)
					{
						continue;
					}
					...
					//检查是否有
					Conds cds = new Conds();
					cds.Retrieve(CondAttr.FK_Node, this.getHisNode().getNodeID(), CondAttr.NodeID, Integer.parseInt(flowNo));
					...
					//启动子流程.
				  SendReturnObjs sendObjs = BP.WF.Dev2Interface.Node_StartWork(fl.getNo(), this.getHisWork().getRow(), null, 0, null, this.getWorkID(), this.getHisFlow().getNo());
				  msg += "@子流程[" + fl.getName() + "]已经启动，已经发送给：" + sendObjs.getVarAcceptersName() + "，发送到：" + sendObjs.getVarToNodeName() + "。";
				}
				if (!msg.equals(""))
				{
			...
			try
			{
				...
				//执行发送.
				String sendSuccess = this.getHisFlow().DoFlowEventEntity(EventListOfNode.SendSuccess, this.getHisNode(), this.rptGe, null, this.HisMsgObjs);

				//string SendSuccess = this.HisNode.MapData.getFrmEvents().DoEventNode(EventListOfNode.SendSuccess, this.HisWork);
				if (sendSuccess != null)
				{
			...
			if (this.getHisNode().getHisTurnToDeal() == TurnToDeal.SpecMsg)
			{
				...
				if (msgOfSend.contains("@") == true)
				{
					...
					for (SendReturnObj item : this.HisMsgObjs.ToJavaList())
					{
...
///#warning 如果这里设置了自动跳转，现在去掉了. 2014-11-07.
			...
			if (this.getHisNode().getIsStartNode())
			{
				...
				{
					BP.WF.Dev2Interface.SetParentInfo(this.getHisFlow().getNo(), this.getWorkID(), this.rptGe.getPFlowNo(), this.rptGe.getPWorkID(), this.rptGe.getPNodeID(), this.rptGe.getPEmp());
					...
					Glo.AddToTrack(ActionType.StartChildenFlow, rptGe.getPFlowNo(), rptGe.getPWorkID(), fid, pND.getNodeID(), pND.getName(), WebUser.getNo(), WebUser.getName(), pND.getNodeID(), pND.getName(), WebUser.getNo(), WebUser.getName(), "<a href='/WF/WFRpt.jsp?FK_Flow=" + this.getHisFlow().getNo() + "&WorkID=" + this.getWorkID() + "' target=_blank >打开子流程</a>", paras);
				}
				else if (SystemConfig.getIsBSsystem() == true)
				{
					...
					if (StringHelper.isNullOrEmpty(pflowNo) == false)
					{
						...
						// 设置成父流程关系.
						BP.WF.Dev2Interface.SetParentInfo(this.getHisFlow().getNo(), this.getWorkID(), pflowNo, Long.parseLong(pWorkID), Integer.parseInt(pNodeID), pEmp);
						...
						String paras = "@CFlowNo=" + this.getHisFlow().getNo() + "@CWorkID=" + this.getWorkID();
						Glo.AddToTrack(ActionType.StartChildenFlow, pflowNo, Long.parseLong(pWorkID), Long.parseLong(String.valueOf(fid)), pND.getNodeID(), pND.getName(), WebUser.getNo(), WebUser.getName(), pND.getNodeID(), pND.getName(), WebUser.getNo(), WebUser.getName(), "<a href='/WF/WFRpt.jsp?FK_Flow=" + this.getHisFlow().getNo() + "&WorkID=" + this.getWorkID() + "' target=_blank >打开子流程</a>", paras);
					}
				}
			}
			///#endregion 设置流程的标记.
			...
			if (this.HisMsgObjs.getVarToNodeID() != WFState.Blank.getValue() && this.town != null && this.town.getHisNode().getWhoExeIt() != 0)
			{
		...
		catch (RuntimeException ex)
		{
			...
			throw new RuntimeException("Message:" + ex.getMessage() + " StackTrace:" + ex.getStackTrace(), ex);
		}
	}
	/** 
	 执行业务表数据同步.
	...
	*/
	private void WhenTranscactionRollbackError(RuntimeException ex)
	{
		...
		{
			//删除发生的日志.
			DBAccess.RunSQL("DELETE FROM ND" + Integer.parseInt(this.getHisFlow().getNo()) + "Track WHERE WorkID=" + this.getWorkID() + " AND NDFrom=" + this.getHisNode().getNodeID() + " AND ActionType=" + ActionType.Forward.getValue());
			...
			switch (startND.getHisNodeWorkType())
			{
				...
				default:
					 ...
					 */
					//// 把开始节点的装态设置回来。
					//DBAccess.RunSQL("UPDATE " + wk.getEnMap().getPhysicsTable() + " SET WFState=0 WHERE OID="+this.WorkID+" OR OID="+this);
					//wk.OID = this.WorkID;
			...
			this.getHisFlow().DoFlowEventEntity(EventListOfNode.SendError, this.getHisNode(), this.getHisWork(), null);
		...
		catch (RuntimeException ex1)
		{
			...
			throw new RuntimeException(ex.getMessage() + "@回滚发送失败数据出现错误：" + ex1.getStackTrace() + "@有可能系统已经自动修复错误，请您在重新执行一次。");
		}
	}
		///#endregion
	...
	*/
	public static String GenerTitle(Flow fl, Work wk, Emp emp, String rdt)
	{
		...
		if (StringHelper.isNullOrEmpty(titleRole))
		{
			...
			if (StringHelper.isNullOrEmpty(titleRole) || titleRole.contains("@") == false)
			{
	...
	private void InitStartWorkDataV2()
	{
		...
		if (SystemConfig.getIsBSsystem())
		{
			...
			if (StringHelper.isNullOrEmpty(fk_nodeFrom) == false)
			{
				...
				//记录父流程被调起。
				BP.WF.Dev2Interface.WriteTrack(this.getHisFlow().getNo(), this.getHisNode().getNodeID(), this.getWorkID(), 0, "被{" + ndFrom.getFlowName() + "}发起,发起人:" + this.getExecerName(), ActionType.CallChildenFlow, "@PWorkID=" + PWorkID + "@PFlowNo=" + ndFrom.getHisFlow().getNo(), "发起子流程:" + this.getHisFlow().getName(), null);
			}
		}
		...
		if (this.title == null)
		{
			...
			{
				/*如果是外部参数,*/
				gwf.setTitle(DBAccess.RunSQLReturnString("SELECT Title FROM " + this.getHisFlow().getPTable() + " WHERE OID=" + this.getWorkID()));
				if (StringHelper.isNullOrEmpty(gwf.getTitle()))
				{
					gwf.setTitle(this.getExecer() + "," + this.getExecerName() + "在:" + DataType.getCurrentDataTime() + "发起.");
				}
				//throw new Exception("您设置的流程标题生成规则为外部传来的参数，但是您岋料在创建空白工作时，流程标题值为空。");
			}
			else
			{
				gwf.setTitle(WorkNode.GenerTitle(this.getHisFlow(), this.getHisWork()));
			}
		}
		else
		{
		...
		if (StringHelper.isNullOrEmpty(rptGe.getBillNo()))
		{
			...
			{
				billNo = BP.WF.Glo.GenerBillNo(billNo, this.getWorkID(), this.rptGe, this.getHisFlow().getPTable());
		...
		if (this.getHisFlow().getHisTimelineRole() == TimelineRole.ByFlow)
		{
			...
			{
				Log.DefaultLogWriteLineError("可能是流程设计错误,获取开始节点{" + gwf.getTitle() + "}的整体流程应完成时间有错误,是否包含SysSDTOfFlow字段? 异常信息:" + ex.getMessage());
				/*获取开始节点的整体流程应完成时间有错误,是否包含SysSDTOfFlow字段? .*/
				if (this.getHisWork().getEnMap().getAttrs().Contains(WorkSysFieldAttr.SysSDTOfFlow) == false)
				{
					throw new RuntimeException("流程设计错误，您设置的流程时效属性是｛按开始节点表单SysSDTOfFlow字段计算｝，但是开始节点表单不包含字段 SysSDTOfFlow , 系统错误信息:" + ex.getMessage());
				}
	...
	*/
	public final void DoCopyCurrentWorkDataToRpt()
	{
		...
		//设置当前的流程所有的用时.
		rptGe.setFlowDaySpan(DataType.GetSpanDays(this.rptGe.GetValStringByKey(GERptAttr.FlowStartRDT), DataType.getCurrentDataTime()));
	...
	*/
	public final void DoCopyRptWork(Work fromWK)
	{
		...
		{
			String key = attr.getKey();
			if(key.equals(GERptAttr.FK_NY) || key.equals(GERptAttr.FK_Dept) || key.equals(GERptAttr.FlowDaySpan) || key.equals(GERptAttr.FlowEmps) || key.equals(GERptAttr.FlowEnder) || key.equals(GERptAttr.FlowEnderRDT) || key.equals(GERptAttr.FlowEndNode) || key.equals(GERptAttr.FlowStarter) || key.equals(GERptAttr.Title) || key.equals(GERptAttr.WFSta))
			{
				continue;
			}
	...
	*/
	...
	/** 
	 ...
	 @param msg 消息
	*/
	public final void AddToTrack(ActionType at, String toEmp, String toEmpName, int toNDid, String toNDName, String msg, Node ndFrom)
	{
		Track t = new Track();
		...
		switch (at)
		{
			...
			case ForwardHL:
				...
				if (this.getHisNode().getFocusField().length() > 1)
				{
					...
					{
						Log.DebugWriteError("@在节点(" + this.getHisNode().getNodeID() + " ， " + this.getHisNode().getName() + ")焦点字段被删除了,表达式为:" + this.getHisNode().getFocusField() + " 替换的结果为:" + t.getMsg());
					}
				}
				break;
			default:
	...
	*/
	public final void SendMsgToThem(GenerWorkerLists gwls)
	{
		...
		FrmEvent fEvent = this.getHisNode().getSendSuccess_FrmEvent();
		switch (fEvent.getMsgCtrl())
		{
			case BySet:
				//是否启动消息?
				isSendEmail = fEvent.getMailEnable();
				isSendSMS = fEvent.getSMSEnable();
			...
			case ByFrmIsSendMsg:
				...
				{
					/*从表单字段里取参数.*/
					if (this.getHisWork().getRow().containsKey("IsSendEmail") == true)
					{
				...
				{
					if (this.getHisWork().getRow().containsKey("IsSendEmail") == false || this.getHisWork().getRow().containsKey("IsSendSMS") == false)
					{
						throw new RuntimeException("没有在ccform里接收到IsSendEmail， IsSendSMS 参数.");
					}
				}
				break;
			case BySDK:
				try
				{
					if (this.getHisWork().GetValBooleanByKey("IsSendMsg") == false)
					{
						return;
					}
				}
				...
				{
					if (this.getHisWork().getRow().containsKey("IsSendMsg") == false)
					{
						throw new RuntimeException("没有接收到IsSendMsg参数.");
					}
				}
				break;
			default:
		...
		//邮件标题.
		String mailTitle = fEvent.getMailTitle();

		//邮件内容.
		String mailDoc = fEvent.getMailDoc();
		String mailEnd = "";

		//短消息内容.
		String msgTemp = fEvent.getSMSDoc();
		for (GenerWorkerList wl : gwls.ToJavaList())
		{
			...
			// 邮件标题.
			String title = "";
			if (StringHelper.isNullOrEmpty(mailTitle))
			{
				title = String.format("流程:%1$s.工作:%2$s,发送人:%3$s,标题:%4$s,需您处理.", this.getHisNode().getFlowName(), wl.getFK_NodeText(), this.getExecerName(), this.rptGe.getTitle());
			}
			else
			{
				title = Glo.DealExp(mailTitle, this.getHisWork(), null);
			}

			//邮件内容.
			String sid = wl.getFK_Emp() + "_" + wl.getWorkID() + "_" + wl.getFK_Node() + "_" + wl.getRDT();
			String url = hostUrl + "WF/Do.jsp?DoType=OF&SID=" + sid;
			url = url.replace("//", "/");
			url = url.replace("//", "/");
			mailDoc = Glo.DealExp(mailDoc, this.getHisWork(), null);
			mailDoc += "\t\n " + String.format(mailEnd.toString(), url);
			mailDoc = mailDoc.replace("{Url}", url);

			// 短信信息.
			if (StringHelper.isNullOrEmpty(msgTemp) == true)
			{
				msgTemp = "新工作:" + this.rptGe.getTitle() + "发送人:" + Glo.DealUserInfoShowModel(WebUser.getNo(), WebUser.getName()) + ",流程:" + this.getHisFlow().getName();
			}
			else
			{
				msgTemp = Glo.DealExp(msgTemp, this.getHisWork(), null);
			}

			BP.WF.Dev2Interface.Port_SendMsg(wl.getFK_Emp(), title, mailDoc, "WKAlt" + wl.getFK_Node() + "_" + wl.getWorkID(), BP.WF.SMSMsgType.ToDo, wl.getFK_Flow(), wl.getFK_Node(), wl.getWorkID(), wl.getFID());
		}
	}
	/** 
	 发送前的流程状态。
	...
	*/
	...
	{
		String sql = "SELECT FK_Emp,FK_EmpText FROM WF_GenerWorkerlist WHERE WorkID=" + this.getWorkID() + " AND IsEnable=1";
	...
	private void DealAutoRunEnable()
	{
		...
		/*如果是自动运行就要设置自动运行参数.*/
		Object tempVar = new String(this.getHisNode().getAutoRunParas());
	...
	private void CheckCompleteCondition()
	{
		...
		try
		{
			if (this.getHisNode().getHisToNodes().size() == 0 && this.getHisNode().getIsStartNode())
			{
				...
				this.setIsStopFlow(true);
				this.addMsg(SendReturnMsgFlag.OneNodeSheetver, "工作已经成功处理(一个流程的工作)。", "工作已经成功处理(一个流程的工作)。 @查看<img src='" + getVirPath() + "WF/Img/Btn/PrintWorkRpt.gif' ><a href='WFRpt.jsp?WorkID=" + this.getHisWork().getOID() + "&FID=" + this.getHisWork().getFID() + "&FK_Flow=" + this.getHisNode().getFK_Flow() + "' target='_self' >工作轨迹</a>", SendReturnMsgType.Info);
				return;
			}
			...
			if (this.getHisNode().getIsCCFlow() && this.getHisFlowCompleteConditions().getIsPass())
			{
				...
				// string path = BP.Sys.Glo.Request.ApplicationPath;
				this.addMsg(SendReturnMsgFlag.MacthFlowOver, "@符合工作流程完成条件" + stopMsg + overMsg, "@符合工作流程完成条件" + stopMsg + overMsg + " @查看<img src='" + getVirPath() + "WF/Img/Btn/PrintWorkRpt.gif' ><a href='WFRpt.jsp?WorkID=" + this.getHisWork().getOID() + "&FID=" + this.getHisWork().getFID() + "&FK_Flow=" + this.getHisNode().getFK_Flow() + "' target='_self' >工作轨迹</a>", SendReturnMsgType.Info);
				return;
			}
		}
		catch (RuntimeException ex)
	...
	*/
	...
	{
		return "";
		//return "@<a href='WhySendToThem.jsp?NodeID=" + fNodeID + "&ToNodeID=" + toNodeID + "&WorkID=" + this.WorkID + "' target=_blank >" + this.ToE("WN20", "为什么要发送给他们？") + "</a>";
	}
	/** 
	 工作流程ID
	*/
	...
	*/
	private void NodeSend_53_UnSameSheet_To_HeLiu(Node nd) 
	{
		...
		if (myfh.getFK_Node() == nd.getNodeID() && gwls.size() != 0)
		{
			...
			java.math.BigDecimal passRate = ok.divide(all, 2).multiply(new java.math.BigDecimal(100));
			...
			this.addMsg("ToHeLiuInfo", "@流程已经运行到合流节点[" + nd.getName() + "]，@您的工作已经发送给如下人员[" + myfh.getToEmpsMsg() + "]。" + this.GenerWhySendToThem(this.getHisNode().getNodeID(), nd.getNodeID()) + numStr);
		}
		else
		{
			// 说明第一次到达河流节点。
		...
		if (this.getHisNode().getMapData().getFrmAttachments().size() != 0)
		{
			...
			if (athDBs.size() > 0)
			{
				...
				for (FrmAttachmentDB athDB : athDBs.ToJavaList())
				{
					...
					athDB_N.Copy(athDB);
					athDB_N.setFK_MapData("ND" + nd.getNodeID());
					athDB_N.setMyPK(athDB_N.getMyPK().replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID()) + "_" + idx);
					athDB_N.setFK_FrmAttachment(athDB_N.getFK_FrmAttachment().replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID()));
		...
		if (this.getHisNode().getMapData().getFrmEles().size() != 0)
		{
			...
			if (eleDBs.size() > 0)
			{
				...
				for (FrmEleDB eleDB : eleDBs.ToJavaList())
				{
					...
					eleDB_N.Copy(eleDB);
					eleDB_N.setFK_MapData("ND" + nd.getNodeID());
					eleDB_N.setMyPK(eleDB_N.getMyPK().replace("ND" + this.getHisNode().getNodeID(), "ND" + nd.getNodeID()));
...
///#warning 对于多个分合流点可能会有问题。
		...
		ps.Add("FID", this.getHisWork().getFID());
		java.math.BigDecimal numAll1 = new java.math.BigDecimal(DBAccess.RunSQLReturnValInt(ps));
		java.math.BigDecimal passRate1 = numAll1.divide(numAll1).multiply(new java.math.BigDecimal(100));
...
///#warning 为了不让其显示在途的工作需要， =3 不是正常的处理模式。
		...
		//ps = new Paras();
		//ps.SQL = "UPDATE WF_GenerWorkFlow SET  WFState=" + (int)WFState.Runing + ", FK_Node=" + nd.NodeID + ",NodeName='" + nd.getName() + "' WHERE WorkID=" + this.HisWork.FID;
		...
		{
			this.addMsg("HeLiuInfo", "@当前工作已经完成，流程已经运行到合流节点[" + nd.getName() + "]。@您的工作已经发送给如下人员[" + toEmpsStr + "]。@您是第一个到达此节点的处理人." + info);
		}
		else
		{
	...
	*/
	public final WorkNode GetPreviousWorkNode()
	{
		...
		sql = "SELECT NDFrom FROM ND" + Integer.parseInt(this.getHisNode().getFK_Flow()) + "Track WHERE WorkID=" + this.getWorkID() + " AND NDTo='" + this.getHisNode().getNodeID() + "' AND ActionType=1 ORDER BY RDT DESC";
		...
		if (nodeid == 0)
		{
			switch (this.getHisNode().getHisRunModel())
			{
				case HL:
				case FHL:
					sql = "SELECT NDFrom FROM ND" + Integer.parseInt(this.getHisNode().getFK_Flow()) + "Track WHERE WorkID=" + this.getWorkID() + " ORDER BY RDT DESC";
					break;
				case SubThread:
					sql = "SELECT NDFrom FROM ND" + Integer.parseInt(this.getHisNode().getFK_Flow()) + "Track WHERE WorkID=" + this.getWorkID() + " AND NDTo=" + this.getHisNode().getNodeID() + " " + " AND ActionType=" + ActionType.SubFlowForward.getValue() + " ORDER BY RDT DESC";
					if (DBAccess.RunSQLReturnCOUNT(sql) == 0)
					{
						sql = "SELECT NDFrom FROM ND" + Integer.parseInt(this.getHisNode().getFK_Flow()) + "Track WHERE WorkID=" + this.getHisWork().getFID() + " AND NDTo=" + this.getHisNode().getNodeID() + " " + " AND ActionType=" + ActionType.SubFlowForward.getValue() + " ORDER BY RDT DESC";
					}
		...
		//                Work wk1 = (Work)pnd.HisWorks.getGetNewEntity();
		...
		//    Work wk = (Work)nd.HisWorks.getGetNewEntity();
		...
		//    wk.Rec = dt.Rows.get(0).getValue("EmpFrom").ToString();
		//    wk.RecText = dt.Rows.get(0).getValue("EmpFromT").ToString();
		...
		//{
		//    case 0:
		//        throw new Exception("没有找到他的上一步工作，系统错误，请通知管理员来处理，请上让上一步处理人撤消发送、或者用本区县管理员用户登录=》待办工作=》流程查询=》在关键字中输入Workid其它条件选择全部，查询到该流程删除它。 @WorkID=" + this.WorkID);
		//    case 1:
		//        return (WorkNode)wns.get(0);
		//    default:
		//        break;
		//}
		//Node nd1 = wns.get(0).HisNode;
