...
public class Bill extends EntityMyPK
{
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(BillAttr.FK_BillType, null, "单据类型", false, false, 0,
				300, 5);
		...
		map.AddTBString(BillAttr.FullPath, null, "FullPath", false, false, 0,
				2000, 5);
		
		map.AddDDLEntities(BillAttr.FK_Emp, null, "打印人", new Emps(), false);
		map.AddTBDateTime(BillAttr.RDT, "打印时间", true, true);
		
		map.AddDDLEntities(BillAttr.FK_Dept, null, "隶属部门", new BP.Port.Depts(),
				false);
		
		map.AddDDLEntities(BillAttr.FK_NY, null, "隶属年月", new BP.Pub.NYs(),
				false);
		
		map.AddTBString(BillAttr.Emps, null, "Emps", false, false, 0, 4000, 5);
		
		map.AddTBString(BillAttr.FK_Node, null, "节点", false, false, 0, 30, 5);
		map.AddTBString(BillAttr.FK_Bill, null, "FK_Bill", false, false, 0,
				500, 5);
		map.AddTBIntMyNum();
