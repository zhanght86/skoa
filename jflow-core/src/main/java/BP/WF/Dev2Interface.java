...
public class Dev2Interface {
	...
	public static int getTodolist_EmpWorks() {
		...
		case SpecFlows:
			if (BP.WF.Glo.getIsEnableTaskPool() == true) {
				ps.SQL = "SELECT count(WorkID) as Num FROM WF_EmpWorks WHERE (" + wfSql + ") AND FK_Emp=" + dbstr + "FK_Emp AND  FK_Flow IN "
						+ emp.getAuthorFlows() + " AND TaskSta!=0   ";
			} else {
				ps.SQL = "SELECT count(WorkID) as Num FROM WF_EmpWorks WHERE (" + wfSql + ") AND FK_Emp=" + dbstr + "FK_Emp AND  FK_Flow IN "
						+ emp.getAuthorFlows();
			}
	...
	public static int getTodolist_Runing() {
		...
		if (WebUser.getIsAuthorize()) {
			...
			sql = "SELECT count( distinct a.WorkID ) as Num FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp='"
					+ WebUser.getNo() + "' AND B.IsEnable=1 AND (B.IsPass=1 OR B.IsPass<0) AND A.FK_Flow IN " + emp.getAuthorFlows();
			return BP.DA.DBAccess.RunSQLReturnValInt(sql);
		} else {
			Paras ps = new Paras();
	...
	public static int getTodolist_Complete() {
		if (Glo.getIsDeleteGenerWorkFlow() == false) {
			...
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT count(WorkID) Num FROM WF_GenerWorkFlow WHERE Emps LIKE '%@" + WebUser.getNo() + "@%' AND WFState="
					+ WFState.Complete.getValue();
			return BP.DA.DBAccess.RunSQLReturnValInt(ps, 0);
		} else {
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT count(*) Num FROM V_FlowData WHERE FlowEmps LIKE '%@" + WebUser.getNo() + "%' AND FID=0 AND WFState="
					+ WFState.Complete.getValue();
			return BP.DA.DBAccess.RunSQLReturnValInt(ps, 0);
		}
	}
	...
	public static int getTodolist_Sharing() {
		...
		String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		String wfSql = "  (WFState=" + WFState.Askfor.getValue() + " OR WFState=" + WFState.Runing.getValue() + " OR WFState="
				+ WFState.Shift.getValue() + " OR WFState=" + WFState.ReturnSta.getValue() + ") AND TaskSta=" + TaskSta.Sharing.getValue();
		String sql;
		String realSql = null;
		if (WebUser.getIsAuthorize() == false) {
			/*不是授权状态*/
		...
		case All:
			...
			break;
		case SpecFlows:
			ps.SQL = "SELECT COUNT(WorkID) FROM WF_EmpWorks WHERE (" + wfSql + ") AND FK_Emp=" + dbstr + "FK_Emp AND  FK_Flow IN "
					+ emp.getAuthorFlows() + " ";
			ps.Add("FK_Emp", BP.Web.WebUser.getNo());
			break;
		case None:
	...
	public static String DTS_DealDeferredWork() throws Exception {
		...
		for (DataRow dr : dt.Rows) {
			...
			case AutoShiftToSpecUser: //移交给指定的人员.
				msg += BP.WF.Dev2Interface.Node_Shift(fk_flow, fk_node, workid, fid, nd.getDoOutTime(), "来自ccflow的自动消息:(" + BP.Web.WebUser.getName()
						+ ")工作未按时处理(" + nd.getName() + "),现在移交给您。");
				break;
			case SendMsgToSpecUser: //向指定的人员发消息.
				BP.WF.Dev2Interface.Port_SendMsg(nd.getDoOutTime(), "来自ccflow的自动消息:(" + BP.Web.WebUser.getName() + ")工作未按时处理(" + nd.getName() + ")",
	...
	/**
	 * 自动执行开始节点数据 说明:根据自动执行的流程设置，自动启动发起的流程。 比如：您根据ccflow的自动启动流程的设置，自动启动该流程，不使用ccflow的提供的服务程序，您需要按如下步骤去做。 1, 写一个自动调度的程序。 2，根据自己的时间需要调用这个接口。
	 * @param fl 流程实体,您可以 new Flow(flowNo); 来传入.
	 * @throws Exception
	 */
	public static void DTS_AutoStarterFlow(Flow fl) throws Exception {
		///#region 读取数据.
		BP.Sys.Frm.MapExt me = new BP.Sys.Frm.MapExt();
		...
		for (DataRow dr : dtMain.Rows) {
			...
			try {
				...
				BP.DA.Log.DefaultLogWriteLineWarning("处理发送信息"+ex.getMessage());
			}
		}
		///#endregion 处理流程发起.
	}
	...
	public static String GetFlowStarters(String fk_flow, String fk_dept) {
		...
		case ByBindEmp: //按人员
			...
			break;
		case ByDept: //按部门
			sql = "SELECT * FROM Port_Emp WHERE FK_Dept IN (SELECT FK_Dept FROM WF_NodeDept WHERE FK_Node=" + nd.getNodeID() + ") and fk_dept='"
					+ fk_dept + "' ";
			break;
		case ByStation: //按岗位
			sql = "SELECT * FROM Port_Emp WHERE No IN (SELECT FK_Emp FROM " + BP.WF.Glo.getEmpStation()
					+ " WHERE FK_Station IN ( SELECT FK_Station from WF_nodeStation where FK_Node=" + nd.getNodeID() + ")) and fk_dept='" + fk_dept
					+ "' ";
			break;
		default:
			throw new RuntimeException("@开始节点的人员访问规则错误,不允许在开始节点设置此访问类型:" + nd.getHisDeliveryWay());
	...
	public static DataTable DB_CCList(String FK_Emp) {
		return  DB_CCList(FK_Emp,"");
	}
	public static DataTable DB_CCList(String FK_Emp,String orderBy) {
		...
		Paras ps = new Paras();
		if (FK_Emp == null) {
			ps.SQL = "SELECT * FROM WF_CCList WHERE 1=1 "+orderByColumn;
		} else {
			ps.SQL = "SELECT * FROM WF_CCList WHERE CCTo=" + SystemConfig.getAppCenterDBVarStr() + "FK_Emp "+orderByColumn;
	...
	/**
	 * 获取指定人员的抄送列表(根据状态) 说明:可以根据这个列表生成指定用户的抄送数据.
	 * @param FK_Emp 人员编号,如果是null,则返回所有的.
	 * @return 返回该人员的所有抄送列表,结构同表WF_CCList.
	 */
	public static DataTable DB_CCList(String FK_Emp, CCSta sta) {
		return DB_CCList(FK_Emp,sta,null);
	}
	public static DataTable DB_CCList(String FK_Emp, CCSta sta,String orderBy) {
		String orderByColumn = "ORDER BY rdt desc";
		if (StringUtils.isNotBlank(orderBy)){
			orderByColumn = " ORDER BY "+orderBy+",rdt desc";
		}
		Paras ps = new Paras();
		if (FK_Emp == null) {
			ps.SQL = "SELECT * FROM WF_CCList WHERE Sta=" + SystemConfig.getAppCenterDBVarStr() + "Sta "+orderByColumn;
			ps.Add("Sta", sta.getValue());
		} else {
			ps.SQL = "SELECT * FROM WF_CCList WHERE CCTo=" + SystemConfig.getAppCenterDBVarStr() + "FK_Emp AND Sta="
					+ SystemConfig.getAppCenterDBVarStr() + "Sta "+orderByColumn;
			ps.Add("FK_Emp", FK_Emp);
			ps.Add("Sta", sta.getValue());
		}
		return DBAccess.RunSQLReturnTable(ps);
	 ...
	 */
	public static DataTable DB_CCList_UnRead(String FK_Emp,String orderBy) {
		return DB_CCList(FK_Emp, CCSta.UnRead,orderBy);
	}
	 ...
	 */
	public static DataTable DB_CCList_Read(String FK_Emp,String orderBy) {
		return DB_CCList(FK_Emp, CCSta.Read,orderBy);
	}
	 ...
	 */
	public static DataTable DB_CCList_Delete(String FK_Emp,String orderBy) {
		return DB_CCList(FK_Emp, CCSta.Del,orderBy);
	}
	 ...
	 * @return 返回该人员的已删除的抄送列表
	 */
	public static DataTable DB_CCList_CheckOver(String FK_Emp,String orderBy) {
		return DB_CCList(FK_Emp, CCSta.CheckOver,orderBy);
	}
	 ...
	 * 获取指定人员能够发起流程的集合 说明:利用此接口可以生成用户的发起的流程列表.
	 * @param userNo 操作员编号
	 * @return Datatable类型的数据集合,数据结构与表WF_Flow大致相同. 如何使用该方法形成发起工作列表,请参考:\WF\UC\Start.ascx
	 */
	...
	public static DataTable DB_GenerDraftDataTable(String flowNo) {
		...
		if (flowNo == null) {
			ps.SQL = "SELECT WorkID,Title,FK_Flow,FlowName,RDT,FlowNote FROM WF_GenerWorkFlow A WHERE WFState=1 AND Starter=" + dbStr
					+ "Starter ORDER BY RDT";
			ps.Add(GenerWorkFlowAttr.Starter, BP.Web.WebUser.getNo());
		} else {
			ps.SQL = "SELECT WorkID,Title,FK_Flow,FlowName,RDT,FlowNote FROM WF_GenerWorkFlow A WHERE WFState=1 AND Starter=" + dbStr
					+ "Starter AND FK_Flow=" + dbStr + "FK_Flow ORDER BY RDT";
	...
	public static DataTable DB_GenerEmpWorksOfDataTable(String userNo, WFState wfState, String fk_flow) {
		...
		if (WebUser.getIsAuthorize() == false) {
			...
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				...
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND FK_Flow=" + dbstr
		...
		case SpecFlows:
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					sql = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND  FK_Flow IN "
							+ emp.getAuthorFlows() + " AND TaskSta=0   ORDER BY FK_Flow,ADT DESC ";
				} else {
					sql = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND  FK_Flow IN "
							+ emp.getAuthorFlows() + "  ORDER BY FK_Flow,ADT DESC ";
				...
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					sql = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp  AND FK_Flow" + dbstr
							+ "FK_Flow AND FK_Flow IN " + emp.getAuthorFlows() + " AND TaskSta=0   ORDER BY FK_Flow,ADT DESC ";
				} else {
					sql = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp  AND FK_Flow" + dbstr
	...
	/**
	 * 根据状态获取当前操作员的共享工作 待办工作，适用于多系统
	 * @param wfState 流程状态
	 * @param fk_flow 流程编号  流程编号为空表示所有的流程
	 * @param sysType 系统类型  系统类型为空表示所有的流程
	 * @return 表结构与视图WF_EmpWorks一致
	 */
	public static DataTable DB_GenerEmpWorksSysType(String userNo, WFState wfState, String fk_flow,String sysType) {
		// 转化成编号.
		fk_flow = TurnFlowMarkToFlowNo(fk_flow);
		...
		if (WebUser.getIsAuthorize() == false) {
			/*不是授权状态*/
			if (StringHelper.isNullOrEmpty(fk_flow) && StringHelper.isNullOrEmpty(sysType)) {
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr
							+ "FK_Emp AND TaskSta=0   ORDER BY FK_Flow,ADT DESC ";
				} else {
				...
				ps.Add("WFState", wfState.getValue());
				ps.Add("FK_Emp", userNo);
			} else if (!StringHelper.isNullOrEmpty(fk_flow) && StringHelper.isNullOrEmpty(sysType))
			{
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND FK_Flow=" + dbstr
							+ "FK_Flow AND TaskSta=0  ORDER BY  ADT DESC ";
			...
			{
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND SysType=" + dbstr
							+ "SysType AND TaskSta=0  ORDER BY  ADT DESC ";
				} else {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND SysType=" + dbstr
							+ "SysType ORDER BY  ADT DESC ";
				}

				ps.Add("WFState", wfState.getValue());
				ps.Add("FK_Emp", userNo);
				ps.Add("SysType", sysType);
			} else 
			{
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND FK_Flow=" + dbstr
							+ "FK_Flow AND SysType=" + dbstr + "SysType AND TaskSta=0  ORDER BY  ADT DESC ";
				} else {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND FK_Flow=" + dbstr
							+ "FK_Flow AND SysType=" + dbstr + "SysType ORDER BY  ADT DESC ";
				}
		...
		case All:
			...
			}else if (StringHelper.isNullOrEmpty(fk_flow) && !StringHelper.isNullOrEmpty(sysType))
			{
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND SysType" + dbstr
							+ "SysType AND TaskSta=0  ORDER BY FK_Flow,ADT DESC ";
				} else {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND SysType" + dbstr
							+ "SysType ORDER BY FK_Flow,ADT DESC ";
				}
			...
			}else
			{
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND FK_Flow" + dbstr
							+ "FK_Flow AND SysType" + dbstr + "SysType AND TaskSta=0  ORDER BY FK_Flow,ADT DESC ";
				} else {
					ps.SQL = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp AND FK_Flow" + dbstr
							+ "FK_Flow AND SysType" + dbstr + "SysType ORDER BY FK_Flow,ADT DESC ";
				}
				ps.Add("WFState", wfState.getValue());
				ps.Add("FK_Emp", BP.Web.WebUser.getNo());
				ps.Add("FK_Flow", fk_flow);
		...
		case SpecFlows:
			...
			{
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					sql = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp  AND SysType" + dbstr
							+ "SysType AND FK_Flow IN " + emp.getAuthorFlows() + " AND TaskSta=0   ORDER BY FK_Flow,ADT DESC ";
				} else {
					sql = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp  AND SysType" + dbstr
							+ "SysType AND FK_Flow IN " + emp.getAuthorFlows() + "  ORDER BY FK_Flow,ADT DESC ";
				}
			...
			}else
			{
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					sql = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp  AND FK_Flow" + dbstr
							+ "FK_Flow AND FK_Flow IN " + emp.getAuthorFlows() + " AND SysType" + dbstr
							+ "SysType AND TaskSta=0   ORDER BY FK_Flow,ADT DESC ";
				} else {
					sql = "SELECT * FROM WF_EmpWorks WHERE WFState=" + dbstr + "WFState AND FK_Emp=" + dbstr + "FK_Emp  AND FK_Flow" + dbstr
							+ "FK_Flow AND FK_Flow IN " + emp.getAuthorFlows() + " AND SysType" + dbstr
	...
	public static DataTable DB_GenerEmpWorksOfDataTable() {
		return DB_GenerEmpWorksOfDataTable(null);
	}
	
	/**
	 * @Description: 查询待办信息  带参数 适用于judp平台
	 * @Title: DB_GenerEmpWorksOfDataTable  
	 * @author 裴孝峰
	 * @date 2016年6月15日
	 */
	public static DataTable DB_GenerEmpWorksOfDataTable(String orderBy) {
		Paras ps = new Paras();
		String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		String wfSql = "  WFState=" + WFState.Askfor.getValue() + " OR WFState=" + WFState.Runing.getValue() + "  OR WFState="
				+ WFState.AskForReplay.getValue() + " OR WFState=" + WFState.Shift.getValue() + " OR WFState=" + WFState.ReturnSta.getValue()
				+ " OR WFState=" + WFState.Fix.getValue();
//		String sql;
		String orderByColumn = " ORDER BY adt desc";
		if (StringUtils.isNotBlank(orderBy)){
			orderByColumn = " ORDER BY "+orderBy+" ,adt desc";
		}
		if (WebUser.getIsAuthorize() == false) {
			/*不是授权状态*/
			if (BP.WF.Glo.getIsEnableTaskPool() == true) {
				ps.SQL = "SELECT * FROM WF_EmpWorks WHERE (" + wfSql + ") AND FK_Emp=" + dbstr + "FK_Emp AND TaskSta!=1  "+orderByColumn;
			} else {
				ps.SQL = "SELECT * FROM WF_EmpWorks WHERE (" + wfSql + ") AND FK_Emp=" + dbstr + "FK_Emp "+orderByColumn;
			}
	...
	public static DataTable DB_FlowCompleteGroup(String userNo) {
		if (Glo.getIsDeleteGenerWorkFlow() == false) {
			...
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT FK_Flow as No,FlowName,COUNT(*) Num FROM WF_GenerWorkFlow WHERE Emps LIKE '%@" + userNo + "@%' AND FID=0 AND WFState="
					+ WFState.Complete.getValue() + " GROUP BY FK_Flow,FlowName";
			return BP.DA.DBAccess.RunSQLReturnTable(ps);
		} else {
	...
	public static DataTable DB_FlowComplete(String userNo, String flowNo, int pageSize, int pageIdx) {
		if (Glo.getIsDeleteGenerWorkFlow() == false) {
			...
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT *,FlowEndNode FK_Node FROM V_FlowData WHERE FlowEmps LIKE '%@" + userNo + "%' AND   FID=0 AND WFState="
					+ WFState.Complete.getValue();
	...
	public static DataTable DB_FlowComplete(String userNo, int pageCount, int pageSize, int pageIdx, String strFlow) {
		if (Glo.getIsDeleteGenerWorkFlow() == false) {
			...
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT *,FlowEndNode FK_Node FROM V_FlowData WHERE FK_Flow='" + strFlow + "' AND FlowEmps LIKE '%@" + userNo
	...
	public static DataTable DB_FlowComplete(String strFlow, int pageSize, int pageIdx) {
		if (Glo.getIsDeleteGenerWorkFlow() == false) {
			...
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT *,FlowEndNode FK_Node FROM V_FlowData WHERE FK_Flow='" + strFlow + "' AND FID=0 AND WFState="
					+ WFState.Complete.getValue();
			return BP.DA.DBAccess.RunSQLReturnTable(ps);
		}
	}
	...
	public static DataTable DB_TongJi_FlowComplete() {
		if (Glo.getIsDeleteGenerWorkFlow() == false) {
			...
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT T.FK_Flow, T.FlowName, COUNT(T.WorkID) as Num FROM WF_GenerWorkFlow T WHERE T.Emps LIKE '%@" + WebUser.getNo()
					+ "@%' AND T.FID=0 AND T.WFSta=" + WFSta.Complete.getValue() + " GROUP BY T.FK_Flow,T.FlowName";
			return BP.DA.DBAccess.RunSQLReturnTable(ps);
		} else {
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT T.FK_Flow, T.FlowName, COUNT(T.WorkID) as Num FROM V_FlowData T WHERE T.FlowEmps LIKE '%@" + WebUser.getNo()
					+ "@%'  AND T.FID=0 AND T.WFSta=" + WFSta.Complete.getValue() + "   GROUP BY T.FK_Flow,T.FlowName";
			return BP.DA.DBAccess.RunSQLReturnTable(ps);
		}
	}
	...
	public static DataTable DB_FlowComplete() {
		if (Glo.getIsDeleteGenerWorkFlow() == false) {
			...
			Paras ps = new Paras();
			String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
			ps.SQL = "SELECT 'RUNNING' AS Type, T.* FROM V_FlowData T WHERE T.FlowEmps LIKE '%@" + WebUser.getNo() + "@%' AND T.FID=0 AND T.WFState="
		...
		if (Glo.getIsDeleteGenerWorkFlow() == false) {
			...
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				if (StringHelper.isNullOrEmpty(title)) {
					ps.SQL = "SELECT 'RUNNING' AS Type,* FROM WF_GenerWorkFlow WHERE Emps LIKE '%@" + WebUser.getNo() + "@%' AND FID=0 AND WFState="
							+ WFState.Complete.getValue() + " and FK_Flow!='010' order by RDT desc";
				} else {
					ps.SQL = "SELECT 'RUNNING' AS Type,* FROM WF_GenerWorkFlow WHERE Emps LIKE '%@" + WebUser.getNo() + "@%' and Title Like '%"
							+ title + "%' AND FID=0 AND WFState=" + WFState.Complete.getValue() + " and FK_Flow!='010' order by RDT desc";
				}
			} else {
				if (StringHelper.isNullOrEmpty(title)) {
					ps.SQL = "SELECT 'RUNNING' AS Type,* FROM WF_GenerWorkFlow WHERE Emps LIKE '%@" + WebUser.getNo() + "@%' AND FID=0 AND WFState="
					...
					ps.SQL = "SELECT 'RUNNING' AS Type,* FROM WF_GenerWorkFlow WHERE Emps LIKE '%@" + WebUser.getNo() + "@%' and Title Like '%"
							+ title + "%' AND FID=0 AND WFState=" + WFState.Complete.getValue() + " and FK_Flow='" + fk_flow + "' order by RDT desc";
				}
			}
			return BP.DA.DBAccess.RunSQLReturnTable(ps);
		} else {
			...
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				if (StringHelper.isNullOrEmpty(title)) {
					ps.SQL = "SELECT 'RUNNING' AS Type,* FROM V_FlowData WHERE FlowEmps LIKE '%@" + WebUser.getNo() + "%' AND FID=0 AND WFState="
							+ WFState.Complete.getValue() + " and FK_Flow!='010' order by RDT desc";
				} else {
					ps.SQL = "SELECT 'RUNNING' AS Type,* FROM V_FlowData WHERE FlowEmps LIKE '%@" + WebUser.getNo() + "%' and Title Like '%" + title
							+ "%' AND FID=0 AND WFState=" + WFState.Complete.getValue() + " and FK_Flow!='010' order by RDT desc";
				}
			} else {
				if (StringHelper.isNullOrEmpty(title)) {
					ps.SQL = "SELECT 'RUNNING' AS Type,* FROM V_FlowData WHERE FlowEmps LIKE '%@" + WebUser.getNo() + "%' AND FID=0 AND WFState="
					...
					ps.SQL = "SELECT 'RUNNING' AS Type,* FROM V_FlowData WHERE FlowEmps LIKE '%@" + WebUser.getNo() + "%' and Title Like '%" + title
							+ "%' AND FID=0 AND WFState=" + WFState.Complete.getValue() + " and FK_Flow='" + fk_flow + "' order by RDT desc";
				}
			}
			return BP.DA.DBAccess.RunSQLReturnTable(ps);
		}
	...
	public static DataTable DB_MyFlow(String fk_flow, String title) {
		...
		if (StringHelper.isNullOrEmpty(fk_flow)) {
			if (StringHelper.isNullOrEmpty(title)) {
				...
				ps.SQL = "SELECT * FROM WF_GenerWorkFlow WHERE Starter='" + WebUser.getNo() + "' AND FK_Flow!='010' and Title Like '%" + title
						+ "%' ";
			}
		} else {
			if (StringHelper.isNullOrEmpty(title)) {
				ps.SQL = "SELECT * FROM WF_GenerWorkFlow WHERE Starter='" + WebUser.getNo() + "' AND FK_Flow='" + fk_flow + "' ";
			} else {
				ps.SQL = "SELECT * FROM WF_GenerWorkFlow WHERE Starter='" + WebUser.getNo() + "' AND FK_Flow='" + fk_flow + "' and Title Like '%"
						+ title + "%'";
			}
		}
		return BP.DA.DBAccess.RunSQLReturnTable(ps);
	...
	public static DataTable DB_TaskPoolOfMyApply() {
		...
		String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		String wfSql = "  (WFState=" + WFState.Askfor.getValue() + " OR WFState=" + WFState.Runing.getValue() + " OR WFState="
				+ WFState.Shift.getValue() + " OR WFState=" + WFState.ReturnSta.getValue() + ") AND TaskSta=" + TaskSta.Takeback.getValue();
		String sql;
		String realSql;
		if (WebUser.getIsAuthorize() == false) {
			/*不是授权状态*/
		...
		if (WebUser.getIsAuthorize()) {
			...
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				sql = "SELECT a.WorkID FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE  A.WFState=" + state
						+ " AND A.WorkID=B.WorkID AND B.FK_Emp='" + WebUser.getNo() + "' AND B.IsEnable=1 AND A.FK_Flow IN " + emp.getAuthorFlows();
			} else {
				sql = "SELECT a.WorkID FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE  A.FK_Flow='" + fk_flow + "' AND A.WFState=" + state
						+ " AND A.WorkID=B.WorkID AND B.FK_Emp='" + WebUser.getNo() + "' AND  B.IsPass=1 AND A.FK_Flow IN " + emp.getAuthorFlows();
			}
		} else {
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				sql = "SELECT a.WorkID FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE  A.WFState=" + state
	...
	public static DataTable DB_GenerWillReturnNodes(int fk_node, long workid, long fid) {
		...
		if (nd.getIsHL() || nd.getIsFLHL()) {
			...
			sql = "SELECT FK_Node AS No,FK_NodeText as Name, FK_Emp as Rec, FK_EmpText as RecName FROM WF_GenerWorkerlist WHERE FID=" + fid
					+ " AND WorkID=" + workid + " AND FK_Node!=" + fk_node + " AND IsPass=1 ORDER BY RDT  ";
		...
		case ReturnAnyNodes:
			if (nd.getTodolistModel() == TodolistModel.Order) {
				sql = "SELECT FK_Node as No,FK_NodeText as Name,FK_Emp as Rec,FK_EmpText as RecName FROM WF_GenerWorkerlist WHERE  (WorkID=" + workid
						+ " AND IsEnable=1 AND IsPass=1 AND FK_Node!=" + fk_node + ") OR (FK_Node=" + fk_node + " AND IsPass <0)  ORDER BY RDT";
			} else {
				sql = "SELECT FK_Node as No,FK_NodeText as Name,FK_Emp as Rec,FK_EmpText as RecName FROM WF_GenerWorkerlist WHERE  WorkID=" + workid
						+ " AND IsEnable=1 AND IsPass=1 AND FK_Node!=" + fk_node + " ORDER BY RDT";
			}
			return DBAccess.RunSQLReturnTable(sql);
		...
		case ByReturnLine: //按照流程图画的退回线执行退回.
			...
			for (Direction dir : dirs.ToJavaList()) {
				...
				sql = "SELECT FK_Emp,FK_EmpText FROM WF_GenerWorkerlist WHERE FK_Node=" + toNode.getNodeID() + " AND WorkID=" + workid
						+ " AND IsEnable=1 AND IsPass=1";
	...
	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
	//ORIGINAL LINE: public static DataTable DB_GenerRuning(string userNo, string fk_flow, bool isMyStarter=false)
	...
	public static DataTable DB_GenerRuning(String userNo, String fk_flow, boolean isMyStarter, String orderBy) {
		// 转化成编号.
		fk_flow = TurnFlowMarkToFlowNo(fk_flow);
		String dbStr = SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		String orderByColumn = "ORDER BY a.rdt desc ";
		if (StringUtils.isNotBlank(orderBy)){
			orderByColumn = " ORDER BY a."+orderBy+", a.rdt desc";
		}
		if (WebUser.getIsAuthorize()) {
			BP.WF.Port.WFEmp emp = new BP.WF.Port.WFEmp(userNo);
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				if (isMyStarter == true) {
					ps.SQL = "SELECT DISTINCT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND A.Starter=" + dbStr
							+ "Starter  AND B.FK_Emp=" + dbStr + "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < 0) AND A.FK_Flow IN "
							+ emp.getAuthorFlows()+orderByColumn;
					ps.Add("Starter", userNo);
					ps.Add("FK_Emp", userNo);
				} else {
					ps.SQL = "SELECT DISTINCT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr
							+ "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < 0) AND A.FK_Flow IN " + emp.getAuthorFlows()+orderByColumn;
					ps.Add("FK_Emp", userNo);
				}
			} else {
				if (isMyStarter == true) {
					ps.SQL = "SELECT DISTINCT a.WorkID FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE  A.FK_Flow=" + dbStr
							+ "FK_Flow  AND A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr
							+ "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < 0) AND  A.Starter=" + dbStr + "Starter AND A.FK_Flow IN "
							+ emp.getAuthorFlows()+orderByColumn;
					ps.Add("FK_Flow", fk_flow);
					ps.Add("FK_Emp", userNo);
					ps.Add("Starter", userNo);
			...
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				if (isMyStarter == true) {
					ps.SQL = "SELECT DISTINCT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr
							+ "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < 0) AND  A.Starter=" + dbStr + "Starter "+orderByColumn;
					ps.Add("FK_Emp", userNo);
					ps.Add("Starter", userNo);
				} else {
					ps.SQL = "SELECT DISTINCT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr
							+ "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < 0) "+orderByColumn;
					ps.Add("FK_Emp", userNo);
				}
			} else {
				if (isMyStarter == true) {
					ps.SQL = "SELECT DISTINCT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.FK_Flow=" + dbStr
							+ "FK_Flow  AND A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr
							+ "FK_Emp AND B.IsEnable=1 AND (B.IsPass=1 or B.IsPass < 0 ) AND  A.Starter=" + dbStr + "Starter  "+orderByColumn;
					ps.Add("FK_Flow", fk_flow);
					ps.Add("FK_Emp", userNo);
					...
					ps.SQL = "SELECT DISTINCT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.FK_Flow=" + dbStr
							+ "FK_Flow  AND A.WorkID=B.WorkID AND B.FK_Emp=" + dbStr + "FK_Emp AND B.IsEnable=1 AND (B.IsPass=1 or B.IsPass < 0 ) "+orderByColumn;
					ps.Add("FK_Flow", fk_flow);
					ps.Add("FK_Emp", userNo);
				}
			}
	...
	public static DataTable DB_TongJi_Runing() {
		...
		Paras ps = new Paras();
		if (WebUser.getIsAuthorize()) {
			WFEmp emp = new BP.WF.Port.WFEmp(BP.Web.WebUser.getNo());
			ps.SQL = "SELECT a.FK_Flow,a.FlowName, Count(a.WorkID) as Num FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp="
					...
					+ emp.getAuthorFlows()
					+ " GROUP BY A.FK_Flow, A.FlowName";
			ps.Add("FK_Emp", WebUser.getNo());
		} else {
			ps.SQL = "SELECT a.FK_Flow,a.FlowName, Count(a.WorkID) as Num FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp="
					+ dbStr + "FK_Emp AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < 0)  GROUP BY A.FK_Flow, A.FlowName";
			ps.Add("FK_Emp", WebUser.getNo());
		}
		return BP.DA.DBAccess.RunSQLReturnTable(ps);
	}
	...
	public static DataTable DB_GenerRuning2(String userNo, String fk_flow, String title) {
		...
		if (StringHelper.isNullOrEmpty(fk_flow)) {
			if (StringHelper.isNullOrEmpty(title)) {
				sql = "SELECT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp='" + userNo
						+ "' AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < 0) and A.FK_Flow!='010'";
			} else {
				sql = "SELECT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp='" + userNo
						+ "' AND B.IsEnable=1 AND  (B.IsPass=1 or B.IsPass < 0) and A.FK_Flow!='010' and A.Title Like '%" + title + "%'";
			}
		} else {
			if (StringHelper.isNullOrEmpty(title)) {
				sql = "SELECT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.FK_Flow='" + fk_flow
						+ "'  AND A.WorkID=B.WorkID AND B.FK_Emp='" + userNo + "' AND B.IsEnable=1 AND (B.IsPass=1 or B.IsPass < 0 )";
			} else {
				sql = "SELECT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.FK_Flow='" + fk_flow
	...
	public static DataTable DB_GenerRuningV2() {
		...
		int state = WFState.Runing.getValue();
		if (WebUser.getIsAuthorize()) {
			WFEmp emp = new BP.WF.Port.WFEmp(userNo);
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				sql = "SELECT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp='" + userNo
						+ "' AND B.IsEnable=1 AND B.IsPass=1 AND A.FK_Flow IN " + emp.getAuthorFlows();
			} else {
				sql = "SELECT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.FK_Flow='" + fk_flow
						+ "'  AND A.WorkID=B.WorkID AND B.FK_Emp='" + WebUser.getNo() + "' AND B.IsEnable=1 AND B.IsPass=1 AND A.FK_Flow IN "
						+ emp.getAuthorFlows();
			}
		} else {
			if (StringHelper.isNullOrEmpty(fk_flow)) {
				sql = "SELECT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.WorkID=B.WorkID AND B.FK_Emp='" + userNo
						+ "' AND B.IsEnable=1 AND B.IsPass=1 ";
			} else {
				sql = "SELECT a.* FROM WF_GenerWorkFlow A, WF_GenerWorkerlist B WHERE A.FK_Flow='" + fk_flow
	...
	public static DataTable DB_GenerRuningAndCC() {
		...
		//dt.getDefaultView.Sort = "RDT DESC";
		//return dt.DefaultView.ToTable();
		return dt;
	}
	...
	public static DataTable GetBatch(int FK_Node) {
		...
		String sql = "SELECT a.*, b.Starter,b.Title as STitle,b.ADT,b.WorkID FROM " + fromTable
				+ " a , WF_EmpWorks b WHERE a.OID=B.WorkID AND b.WFState Not IN (7) AND b.FK_Node=" + nd.getNodeID() + " AND b.FK_Emp='"
				+ WebUser.getNo() + "'";
	...
	public static boolean Port_CheckUserLogin(String UserNo, String SID) {
		...
		if (StringHelper.isNullOrEmpty(SID)) {
			return false;
		}

		//随后改成动态sid
		if (UserNo.equals("admin") && SID.toUpperCase().equals("33273D4A-35C8-4AE8-BDD9-0085EA648CCB")) {
			return true;
		}

		BP.Port.Emp emp = new BP.Port.Emp(UserNo);
		boolean bHave = emp.RetrieveByAttr(BP.Port.EmpAttr.No, UserNo);
		//用户编号不存在
		if (bHave == false) {
			return false;
		}
	 ...
	 * @param fid FID
	 */
	public static void Port_SendMsg(String userNo, String title, String msgDoc, String msgFlag, String msgType, String flowNo, long nodeID,
			long workID, long fid) {
		if (workID != 0) {
			String url = Glo.getCCFlowAppPath() + "WF/Do.jsp?SID=" + userNo + "_" + workID + "_" + nodeID;
			url = url.replace("//", "/");
			url = url.replace("//", "/");
			//msgDoc += " <hr>打开工作: " + url;
		}
	 ...
	 * @param workID 工作ID
	 * @param fid FID
	 * @param isEmail 是否发送邮件
	 * @param isSMS 是否发送短信
	 */
	public static void Port_SendMsg(String sendTo, String mailTitle, String mailDoc, String smsInfo, String msgFlag, String msgType, String flowNo,
			int nodeID, long workID, long fid, boolean isEmail, boolean isSMS) {
		SMS sms = new SMS();
	 ...
	 */
	public static void Port_SendMsg(String mobileNum, String mobileInfo, String emailAddress, String emilTitle, String emailBody, String msgFlag,
			String msgType) {
	...
	public static void Port_SendEmail(String mailAddress, String emilTitle, String emailBody, String msgType, String msgGroupFlag, String sender) {
		Port_SendEmail(mailAddress, emilTitle, emailBody, msgType, msgGroupFlag, sender, null);
	}

	public static void Port_SendEmail(String mailAddress, String emilTitle, String emailBody, String msgType, String msgGroupFlag) {
		Port_SendEmail(mailAddress, emilTitle, emailBody, msgType, msgGroupFlag, null, null);
	}

	public static void Port_SendEmail(String mailAddress, String emilTitle, String emailBody, String msgType) {
		Port_SendEmail(mailAddress, emilTitle, emailBody, msgType, null, null, null);
	}

	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
	//ORIGINAL LINE: public static void Port_SendEmail(string mailAddress, string emilTitle, string emailBody, string msgType, string msgGroupFlag=null, string sender = null, string msgPK = null)
	public static void Port_SendEmail(String mailAddress, String emilTitle, String emailBody, String msgType, String msgGroupFlag, String sender,
			String msgPK) {
		...
		/**
		 * 向其他系统推送消息，根据webservice接口实现  根据配置文件判断是否实现
		 */
		String insertOtherSysMsg=String.valueOf(SystemConfig.getAppSettings().get("InserOtherSysMsg"));
		if("true".equals(insertOtherSysMsg)){
			MessageClient mc=new MessageClient();
			mc.init();
			if(mc.userIsexit(WebUser.getNo())){
				Emp emp=getEmpByParam(null,null,mailAddress);
				if(emp!=null){
					mc.sendMsg(emilTitle, "0", "4", emailBody, "1", emp.getNo(), emp.getName(), (String[][])null, "4");
				}
	...
	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
	//ORIGINAL LINE: public static void Port_SendSMS(string tel, string smsDoc, string msgType, string msgGroupFlag, string sender = null, string msgPK = null, string sendEmpNo = null, string atParas=null)
	public static void Port_SendSMS(String tel, String smsDoc, String msgType, String msgGroupFlag, String sender, String msgPK, String sendEmpNo,
			String atParas) {
		...
		if("true".equals(insertOtherSysMsg)){
			...
			mc.init();
			if(mc.userIsexit(WebUser.getNo())){
				Emp emp=getEmpByParam(sendEmpNo, null,null);
				if(emp!=null){
					mc.sendMsg(smsDoc, "0", "4", smsDoc, "1", sendEmpNo, emp.getName(), (String[][])null, "3");
				}
			}
		}
	}
	...
	public static String TurnFlowMarkToFlowNo(String FlowMark) {
		...
		String s = DBAccess.RunSQLReturnStringIsNull("SELECT No FROM WF_Flow WHERE NO in " + FlowMark + " ", null);
		if (s == null) {
	...
	public static DataTable Port_GetNewMsg(String userNo, String dateLastTime) {
		Paras ps = new Paras();
		ps.SQL = "SELECT MsgType , Count(*) as Num FROM Sys_SMS WHERE SendTo=" + BP.Sys.SystemConfig.getAppCenterDBVarStr() + "SendTo AND RDT >"
	...
	public static DataTable Port_GetNewMsg(String userNo) {
		Paras ps = new Paras();
		ps.SQL = "SELECT MsgType , Count(*) as Num FROM Sys_SMS WHERE SendTo=" + BP.Sys.SystemConfig.getAppCenterDBVarStr()
	 ...
	 * @param msg 信息
	 */
	public static void WriteTrackInfo(String flowNo, int nodeFrom, long workid, long fid, String msg, String optionMsg) {
	...
	public static void WriteTrackWorkCheck(String flowNo, int currNodeID, long workid, long fid, String msg, String optionName) {
		...
		if (wn.getHisNode().getTodolistModel() == TodolistModel.QiangBan) {
			//先删除
			ps.SQL = "DELETE FROM ND" + Integer.parseInt(flowNo) + "Track WHERE  WorkID=" + dbStr + "WorkID  AND NDFrom=" + dbStr
					+ "NDFrom AND ActionType=" + ActionType.WorkCheck.getValue();
			...
			ps.SQL = "UPDATE  ND" + Integer.parseInt(flowNo) + "Track SET Msg=" + dbStr + "Msg WHERE  WorkID=" + dbStr + "WorkID  AND NDFrom="
					+ dbStr + "NDFrom AND EmpFrom=" + dbStr + "EmpFrom AND ActionType=" + ActionType.WorkCheck.getValue();
	...
	public static void WriteTrackDailyLog(String flowNo, int nodeFrom, long workid, long fid, String msg, String optionName) {
		...
		Paras ps = new Paras();
		ps.SQL = "UPDATE  ND" + Integer.parseInt(flowNo) + "Track SET Msg=" + dbStr + "Msg WHERE  RDT LIKE '" + today + "%' AND WorkID=" + dbStr
				+ "WorkID  AND NDFrom=" + dbStr + "NDFrom AND EmpFrom=" + dbStr + "EmpFrom AND ActionType=" + ActionType.WorkCheck.getValue();
		ps.Add(TrackAttr.Msg, msg);
		ps.Add(TrackAttr.WorkID, workid);
	...
	public static void WriteTrackWeekLog(String flowNo, int nodeFrom, long workid, long fid, String msg, String optionName) {
		...
		//		java.util.Date startWeek = dTime.plusDays(1 - Integer.parseInt(String.format("%d", dTime.getDayOfWeek()))); //本周第一天
		//
		//		Hashtable ht = new Hashtable(); //当前日期所属的week包含哪些日期
		//		for (int i = 1; i < 7; i++)
		//		{
		//			ht.put(i + 1, DateUtils.format(DateUtils.addDay(startWeek, i),"yyyy-MM-dd"));
		//		}
		//		ht.put(1, DateUtils.format(startWeek,"yyyy-MM-dd"));
		//
		...
		//		String sql = null;
		//
		//		for (Object de : ht.keySet())
		//		{
		//			sql = "SELECT * FROM ND" + Integer.parseInt(flowNo) + "Track  WHERE  RDT LIKE '" + de.toString() + "%' AND WorkID=" + workid + "  AND NDFrom='" + nodeFrom + "' AND EmpFrom='" + WebUser.getNo() + "' AND ActionType=" + ActionType.WorkCheck.getValue();
		//
		//			if (DBAccess.RunSQLReturnCOUNT(sql) != 0)
		//			{
		//				isExitWeek = true;
		//				insertDate = de.toString();
		//				break;
		//			}
		//		}
		//
	...
	public static void WriteTrackMonthLog(String flowNo, int nodeFrom, long workid, long fid, String msg, String optionName) {
		...
		//
		//		java.util.Date dTime = new java.util.Date();
		//		java.util.Date startDay = DateUtils.addDay(dTime, 1 - dTime.getDay()); //本月第一天
		//
		//		int days = java.util.Date.DaysInMonth(dTime.getYear(), dTime.getMonth());
		//		Hashtable ht = new Hashtable();
		//
		//		for (int i = 1; i < days; i++)
		//		{
		//			ht.put(i + 1,DateUtils.format(DateUtils.addDay(startDay, i), "yyyy-MM-dd"));
		//		}
		//		ht.put(1, DateUtils.format(startDay, "yyyy-MM-dd"));
		//
	...
	public static String GetCheckInfo(String flowNo, long workId, int nodeFrom) {
		String table = "ND" + Integer.parseInt(flowNo) + "Track";
		String sql = "SELECT Msg FROM " + table + " WHERE NDFrom=" + nodeFrom + " AND ActionType=" + ActionType.WorkCheck.getValue()
				+ " AND EmpFrom='" + WebUser.getNo() + "' AND WorkID=" + workId + " ORDER BY RDT DESC ";
		DataTable dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() == 0) {
	...
	public static String GetOrderCheckInfo(String flowNo, long workId, int nodeFrom) {
		String table = "ND" + Integer.parseInt(flowNo) + "Track";
		String sql = "SELECT Msg FROM " + table + " WHERE NDFrom=" + nodeFrom + " AND ActionType=" + ActionType.WorkCheck.getValue() + " AND WorkID="
				+ workId + " ORDER BY RDT DESC ";
		DataTable dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() == 0) {
			//BP.Sys.FrmWorkCheck fwc = new FrmWorkCheck(nodeFrom);
			//return fwc.FWCDefInfo;
	...
	public static void DeleteCheckInfo(String flowNo, long workId, int nodeFrom) {
		String table = "ND" + Integer.parseInt(flowNo) + "Track";
		String sql = "DELETE FROM " + table + " WHERE NDFrom=" + nodeFrom + " AND ActionType=" + ActionType.WorkCheck.getValue() + " AND EmpFrom='"
				+ WebUser.getNo() + "' AND WorkID=" + workId;
		BP.DA.DBAccess.RunSQL(sql);
	}

	public static String GetAskForHelpReInfo(String flowNo, long workId, int nodeFrom) {
		String table = "ND" + Integer.parseInt(flowNo) + "Track";
		String sql = "SELECT Msg FROM " + table + " WHERE NDFrom=" + nodeFrom + " AND ActionType=" + ActionType.AskforHelp.getValue()
				+ " AND EmpFrom='" + WebUser.getNo() + "' AND WorkID=" + workId + " ORDER BY RDT DESC ";
		DataTable dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() == 0) {
			return "";
	...
	public static void SetTrackInfo(String flowNo, long workId, int nodeFrom, int actionType, String strMsg) {
		...
		String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		ps.SQL = "UPDATE " + table + " SET Msg=" + dbstr + "Msg  WHERE ActionType=" + dbstr + "ActionType and WorkID=" + dbstr + "WorkID and NDFrom="
				+ dbstr + "NDFrom";
		ps.Add("Msg", strMsg);
	 ...
	 */
	public static void SetParentInfo(String subFlowNo, long subFlowWorkID, String parentFlowNo, long parentWorkID, int parentNodeID,
			String parentEmpNo) {
		if (parentWorkID == 0) {
		...
		String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		ps.SQL = "UPDATE WF_GenerWorkFlow SET PFlowNo=" + dbstr + "PFlowNo, PWorkID=" + dbstr + "PWorkID,PNodeID=" + dbstr + "PNodeID,PEmp=" + dbstr
				+ "PEmp WHERE WorkID=" + dbstr + "WorkID";
		...
		ps = new Paras();
		ps.SQL = "UPDATE " + fl.getPTable() + " SET PFlowNo=" + dbstr + "PFlowNo, PWorkID=" + dbstr + "PWorkID,PNodeID=" + dbstr + "PNodeID, PEmp="
				+ dbstr + "PEmp WHERE OID=" + dbstr + "OID";
	...
	/**
	 * 删除流程并写入日志 清除的内容如下: 1, 流程引擎中的数据. 2, 节点数据,NDxxRpt数据. 并作如下处理: 1, 保留track数据. 2, 写入流程删除记录表.
	 * @param flowNo 流程编号
	 * @param workID 工作ID
		...
		if (SystemConfig.getOSDBSrc() == OSDBSrc.Database) {
			...
			case ByStation:
				ps.SQL = "SELECT COUNT(A.FK_Node) as Num FROM WF_NodeStation A, " + BP.WF.Glo.getEmpStation()
						+ " B WHERE A.FK_Station= B.FK_Station AND  A.FK_Node=" + dbstr + "FK_Node AND B.FK_Emp=" + dbstr + "FK_Emp";
			...
			case ByDept:
				ps.SQL = "SELECT COUNT(A.FK_Node) as Num FROM WF_NodeDept A, " + BP.WF.Glo.getEmpDept()
						+ " B WHERE A.FK_Dept= B.FK_Dept AND  A.FK_Node=" + dbstr + "FK_Node AND B.FK_Emp=" + dbstr + "FK_Emp";
				ps.Add("FK_Node", nd.getNodeID());
				ps.Add("FK_Emp", userNo);
				num = DBAccess.RunSQLReturnValInt(ps);
				break;
	...
	public static int Flow_NumOfSubFlowRuning(long pWorkID, long currWorkID) {
		String sql = "SELECT COUNT(*) AS num FROM WF_GenerWorkFlow WHERE WFState!=" + WFState.Complete.getValue() + " AND WorkID!=" + currWorkID
				+ " AND PWorkID=" + pWorkID;
		return DBAccess.RunSQLReturnValInt(sql);
	}
	...
	public static boolean Flow_CheckAllSubFlowIsOver(int nodeID, long workID) {
		String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		ps.SQL = "SELECT COUNT(WorkID) FROM WF_GenerWorkFlow WHERE  PNodeID=" + dbstr + "PNodeID AND PWorkID=" + dbstr + "PWorkID AND WFState!="
				+ dbstr + "WFState ";
	...
	public static boolean Flow_IsCanDoCurrentWork(String fk_flow, int nodeID, long workID, String userNo) {
		...
		if (str.substring(len<0?0:len, len + 2).equals("01")) {
			return true;
		}
		///#endregion 判断是否是开始节点.
		...
		String sql = "SELECT c.RunModel, a.TaskSta, a.WFState, IsPass FROM WF_GenerWorkFlow a , WF_GenerWorkerlist b, WF_Node c WHERE a.FK_Node='"
				+ nodeID + "'  AND b.FK_Node=c.NodeID AND a.WorkID=b.WorkID AND a.FK_Node=b.FK_Node  AND b.FK_Emp='" + userNo
				+ "' AND b.IsEnable=1 AND a.workid='" + workID + "'";
		DataTable dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
		if (dt.Rows.size() == 0) {
			return false;
		}
		...
		//TaskSta TaskStai = (TaskSta)int.Parse(dt.Rows.get(0).getValue(1).ToString());
	...
	public static boolean Flow_IsCanDoCurrentWorkGuest(int nodeID, long workID, String userNo) {
		...
		//ps.Add("FK_Emp", userNo);
		//ps.Add("WorkID", workID);
		String sql = "SELECT c.RunModel, a.TaskSta FROM WF_GenerWorkFlow a , WF_GenerWorkerlist b, WF_Node c WHERE a.FK_Node='" + nodeID
				+ "'  AND b.FK_Node=c.NodeID AND a.WorkID=b.WorkID AND a.FK_Node=b.FK_Node  AND b.GuestNo='" + userNo
				+ "' AND b.IsEnable=1 AND a.workid='" + workID + "'";
	...
	public static boolean Flow_IsCanViewTruck(String flowNo, long workid, long fid) throws Exception {
		...
		String dbStr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		ps.SQL = "SELECT count(MyPK) as Num FROM ND" + Integer.parseInt(flowNo) + "Track WHERE (WorkID=" + dbStr + "WorkID OR FID=" + dbStr
				+ "FID) AND (EmpFrom=" + dbStr + "Emp1 OR EmpTo=" + dbStr + "Emp2)";
	...
	public static String Flow_DoPress(long workID, String msg, boolean isPressSubFlow) {
		...
		for (GenerWorkerList wl : wls.ToJavaList()) {
			...
			// 发消息.
			BP.WF.Dev2Interface.Port_SendMsg(wl.getFK_Emp(), mailTitle, msg, null, BP.WF.SMSMsgType.DoPress, gwf.getFK_Flow(), gwf.getFK_Node(),
	...
	public static boolean Flow_SetFlowTitle(String flowNo, long workid, String title) {
		return Flow_SetFlowTitle(flowNo,workid,title,null);
	}
	 ...
	 * @param flowNo 流程编号
	 * @param workid 工作ID
	 * @param runType 是否自动运行？ 如果自动运行，就按照流程设置的规则运行。 非自动运行，就按照用户自己定义的运转顺序计算。
	...
	public static void Flow_SetFlowTransferCustom(String flowNo, long workid, TransferCustomType runType, String paras) {
		...
		for (String str : strs) {
			...
			int todomodel = Integer.parseInt(vals[2]);
			...
			tc.setTodolistModel(TodolistModel.forValue(todomodel));
			tc.Save();
			idx++;
	...
	public static void Flow_Focus(long workid) {
		...
		if (paras.contains("F_" + WebUser.getNo() + "=0")) {
	...
	public static DataTable DB_AuthorEmpWorks(String empNo) {
		...
		if (!StringHelper.isNullOrEmpty(emp.getAuthor()) && emp.getAuthor().equals(WebUser.getNo())
			...
			case SpecFlows:
				if (BP.WF.Glo.getIsEnableTaskPool() == true) {
					sql = "SELECT * FROM WF_EmpWorks WHERE (" + wfSql + ") AND FK_Emp='" + emp.getNo() + "' AND  FK_Flow IN " + emp.getAuthorFlows()
	 ...
	 * @throws Exception
	 */
	public static SendReturnObjs Node_StartWork(String flowNo, Hashtable htWork, DataSet workDtls, int nextNodeID, String nextWorker,
			long parentWorkID, String parentFlowNo) {
		// 给全局变量赋值.
		...
		if (htWork != null) {
			for (Object str : htWork.keySet()) {
				String strOb = str.toString();
				if (strOb.equals(StartWorkAttr.OID)
						|| strOb.equals(StartWorkAttr.CDT)
						|| strOb.equals(StartWorkAttr.MD5)
						|| strOb.equals(StartWorkAttr.Emps)
						|| strOb.equals(StartWorkAttr.FID)
						|| strOb.equals(StartWorkAttr.FK_Dept)
						|| strOb.equals(StartWorkAttr.PRI)
						|| strOb.equals(StartWorkAttr.Rec)
						|| strOb.equals(StartWorkAttr.Title)) {
					continue;
				}
		...
		if (parentWorkID != 0) {
			DBAccess.RunSQL("UPDATE WF_GenerWorkFlow SET PWorkID=" + parentWorkID + ",PFlowNo='" + parentFlowNo + "' WHERE WorkID="
					+ objs.getVarWorkID());
		}
	...
	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
	//ORIGINAL LINE: public static Int64 Node_CreateBlankWork(string flowNo, Hashtable ht = null, DataSet workDtls = null, string starter = null, string title = null, Int64 parentWorkID = 0, Int64 parentFID = 0, string parentFlowNo = null, int parentNodeID = 0, string parentEmp = null, int jumpToNode = 0, string jumpToEmp = null)
	public static long Node_CreateBlankWork(String flowNo, Hashtable ht, DataSet workDtls, String starter, String title, long parentWorkID,
			long parentFID, String parentFlowNo, int parentNodeID, String parentEmp, int jumpToNode, String jumpToEmp) {
		...
		if (ht != null) {
			for (Object str : ht.keySet()) {
				String strOb = str.toString();
				if (strOb.equals(StartWorkAttr.OID) || strOb.equals(StartWorkAttr.CDT) || strOb.equals(StartWorkAttr.MD5) || strOb.equals(StartWorkAttr.Emps) || strOb.equals(StartWorkAttr.FID) || strOb.equals(StartWorkAttr.FK_Dept) || strOb.equals(StartWorkAttr.PRI) || strOb.equals(StartWorkAttr.Rec) || strOb.equals(StartWorkAttr.Title))
				{
					continue;
				}
		...
		if (StringHelper.isNullOrEmpty(title) == false) {
			...
			ps = new Paras();
			ps.SQL = "UPDATE " + fl.getPTable() + " SET PFlowNo=" + dbstr + "PFlowNo,PWorkID=" + dbstr + "PWorkID,WFState=" + dbstr
					+ "WFState,Title=" + dbstr + "Title WHERE OID=" + dbstr + "OID";
			...
			ps = new Paras();
			ps.SQL = "UPDATE " + fl.getPTable() + " SET PFlowNo=" + dbstr + "PFlowNo,PWorkID=" + dbstr + "PWorkID,WFState=" + dbstr
					+ "WFState,FK_Dept=" + dbstr + "FK_Dept,Title=" + dbstr + "Title WHERE OID=" + dbstr + "OID";
			ps.Add(GERptAttr.PFlowNo, parentFlowNo);
			ps.Add(GERptAttr.PWorkID, parentWorkID);
		...
		gwf.setNodeName(nd.getName());
		//默认启用草稿,如果写入待办则状态为运行
        if(DraftRole.SaveToTodolist == fl.getDraftRole())
        {
        	gwf.setWFState(WFState.Runing);
        } else if (DraftRole.SaveToDraftList == fl.getDraftRole())
        {
	 ...
	 * @param flowNo 流程编号
	 * @param ht 表单参数，可以为null。
	 * @param workDtls 表单明细表参数，可以为null。
	 * @param flowStarter 流程发起人，如果为null就是当前人员。
	 * @param title 创建工作时的标题，如果为null，就按设置的规则生成。
	 * @return 为开始节点创建工作后产生的WorkID.
	 */

	//	public static long Node_CreateStartNodeWork(String flowNo, Hashtable ht, DataSet workDtls, String flowStarter)
	//	{
	//		return Node_CreateStartNodeWork(flowNo, ht, workDtls, flowStarter, null);
	//	}
	//
	//	public static long Node_CreateStartNodeWork(String flowNo, Hashtable ht, DataSet workDtls)
	//	{
	//		return Node_CreateStartNodeWork(flowNo, ht, workDtls, null, null);
	//	}

	//	public static long Node_CreateStartNodeWork(String flowNo, Hashtable ht)
	//	{
	//		return Node_CreateStartNodeWork(flowNo, ht, null, null, null);
	//	}

	//	public static long Node_CreateStartNodeWork(String flowNo)
	//	{
	//		return Node_CreateStartNodeWork(flowNo, null, null, null, null);
	//	}

	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
	//ORIGINAL LINE: public static Int64 Node_CreateStartNodeWork(string flowNo, Hashtable ht = null, DataSet workDtls = null, string flowStarter = null, string title = null)
	//	public static long Node_CreateStartNodeWork(String flowNo, Hashtable ht, DataSet workDtls, String flowStarter, String title)
	//	{
	...
	public static long Node_CreateStartNodeWork(String flowNo, Hashtable htWork, DataSet workDtls, String flowStarter, String title,
			long parentWorkID, String parentFlowNo, int parentNDFrom) throws Exception {
		...
		String dbstr = SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		ps.SQL = "UPDATE " + fl.getPTable() + " SET WFState=" + dbstr + "WFState,Title=" + dbstr + "Title,FK_Dept=" + dbstr + "FK_Dept,PFlowNo="
				+ dbstr + "PFlowNo,PWorkID=" + dbstr + "PWorkID WHERE OID=" + dbstr + "OID";
		...
		////写入日志.
		//WorkNode wn = new WorkNode(wk, nd);
		//wn.AddToTrack(ActionType.CallSubFlow, flowStarter, emp.getName(), nd.NodeID, nd.Name, "来自" + WebUser.getNo() + "," + WebUser.getName()
		//    + "工作发起.");
	...
	public static SendReturnObjs Node_SendWork(String fk_flow, long workID, Hashtable htWork, int toNodeID, String nextWorkers) throws Exception {
		...
		return Node_SendWork(fk_flow, workID, htWork, null, toNodeID, nextWorkers, WebUser.getNo(), WebUser.getName(), WebUser.getFK_Dept(),
				WebUser.getFK_DeptName(), null);
	}
	 ...
	 * @param nextWorkers 下一步的接受人，如果多个人员用逗号分开，比如:zhangsan,lisi, 如果为空，则标识让ccflow按照节点访问规则自动寻找。
	 * @return 执行信息
	 * @throws Exception
	 */
	public static SendReturnObjs Node_SendWork(String fk_flow, long workID, Hashtable htWork, DataSet workDtls, int toNodeID, String nextWorkers) {
		return Node_SendWork(fk_flow, workID, htWork, workDtls, toNodeID, nextWorkers, WebUser.getNo(), WebUser.getName(), WebUser.getFK_Dept(),
				WebUser.getFK_DeptName(), null);
	}
	 ...
	 */
	public static SendReturnObjs Node_SendWork(String fk_flow, long workID, Hashtable htWork, DataSet workDtls, int toNodeID, String nextWorkers,
			String execUserNo, String execUserName, String execUserDeptNo, String execUserDeptName, String title) {
		...
		if (htWork != null) {
			...
			for (Object key : htWork.keySet()) {
				...
				if (keyOb.equals(WorkSysFieldAttr.SysSDTOfFlow))
				{
					ps = new Paras();
					...
					ps.Add(GenerWorkerListAttr.WorkID, workID);
					DBAccess.RunSQL(ps);
				} else if (keyOb.equals(WorkSysFieldAttr.SysSDTOfNode))
				{
					ps = new Paras();
	...
	public static void Node_InsertOrderEmp(String flowNo, int nodeID, long workid, long fid, String empNo, String empName) {
		...
		String sql = "SELECT No,Name FROM Port_Emp WHERE No IN (SELECT FK_Emp FROM WF_GenerWorkerList WHERE WorkID=" + workid + " AND FK_Node="
				+ nodeID + " AND IsPass >=100 ) ORDER BY IDX desc";
		DataTable dt = BP.DA.DBAccess.RunSQLReturnTable(sql);
		int idx = 100;
		for (DataRow dr : dt.Rows) {
			idx++;
	...
	public static void Node_CC_SetSta(int nodeID, long workid, String empNo, CCSta sta) {
		String dbstr = SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		ps.SQL = "UPDATE WF_CCList   SET Sta=" + dbstr + "Sta,CDT=" + dbstr + "CDT WHERE WorkID=" + dbstr + "WorkID AND FK_Node=" + dbstr
	...
	public static void Node_CC_SetRead(String mypk) {
		...
		Paras ps = new Paras();
		ps.SQL = "UPDATE WF_CCList SET Sta=" + SystemConfig.getAppCenterDBVarStr() + "Sta  WHERE MyPK=" + SystemConfig.getAppCenterDBVarStr()
				+ "MyPK";
	...
	public static void Node_CC_SetRead(int nodeID, long workid, String empNo) {
		Paras ps = new Paras();
		ps.SQL = "UPDATE WF_CCList SET Sta=" + SystemConfig.getAppCenterDBVarStr() + "Sta  WHERE WorkID=" + SystemConfig.getAppCenterDBVarStr()
				+ "WorkID AND FK_Node=" + SystemConfig.getAppCenterDBVarStr() + "FK_Node AND CCTo=" + SystemConfig.getAppCenterDBVarStr() + "CCTo";
		ps.Add(CCListAttr.Sta, CCSta.Read.getValue());
		ps.Add(CCListAttr.WorkID, workid);
		ps.Add(CCListAttr.FK_Node, nodeID);
		ps.Add(CCListAttr.CCTo, empNo);
		...
		ps = new Paras();
		ps.SQL = "UPDATE WF_GenerWorkerlist SET IsRead=1 WHERE WorkID=" + SystemConfig.getAppCenterDBVarStr() + "WorkID AND FK_Node="
				+ SystemConfig.getAppCenterDBVarStr() + "FK_Node AND FK_Emp=" + SystemConfig.getAppCenterDBVarStr() + "FK_Emp";
		ps.Add(GenerWorkerListAttr.WorkID, workid);
		ps.Add(GenerWorkerListAttr.FK_Node, nodeID);
	 ...
	 * @return
	 */
	public static String Node_CC(String fk_flow, int fk_node, long workID, String toEmpNo, String toEmpName, String msgTitle, String msgDoc,
			String pFlowNo, long pWorkID) {
		Flow fl = new Flow(fk_flow);
		Node nd = new Node(fk_node);
		...
		GenerWorkFlow gwf = new GenerWorkFlow(workID);
		//记录日志.
		Glo.AddToTrack(ActionType.CC, fk_flow, workID, 0, nd.getNodeID(), nd.getName(), WebUser.getNo(), WebUser.getName(), nd.getNodeID(),
				nd.getName(), toEmpNo, toEmpName, msgTitle, null);

		//发送邮件.
		BP.WF.Dev2Interface.Port_SendMsg(toEmpNo, WebUser.getName() + "把工作:" + gwf.getTitle(), "抄送:" + msgTitle, "CC" + nd.getNodeID() + "_" + workID
				+ "_", BP.WF.SMSMsgType.CC, gwf.getFK_Flow(), gwf.getFK_Node(), gwf.getWorkID(), gwf.getFID());
		...
		try {
			...
			if (htWork != null) {
				...
				if (StringHelper.isNullOrEmpty(paras) == false) {
					String dbstr = SystemConfig.getAppCenterDBVarStr();
					Paras ps = new Paras();
					ps.SQL = "UPDATE WF_GenerWorkerlist SET AtPara=" + dbstr + "Paras WHERE WorkID=" + dbstr + "WorkID AND FK_Node=" + dbstr
							+ "FK_Node";
					ps.Add(GenerWorkerListAttr.Paras, paras);
					ps.Add(GenerWorkerListAttr.WorkID, workID);
					ps.Add(GenerWorkerListAttr.FK_Node, nd.getNodeID());
	...
	public static boolean Node_TaskPoolTakebackOne(long workid) {
		...
		Paras ps = new Paras();
		//设置已经被取走的状态。
		ps.SQL = "UPDATE WF_GenerWorkerlist SET IsEnable=-1 WHERE IsEnable=1 AND WorkID=" + dbstr + "WorkID AND FK_Node=" + dbstr
		...
		BP.WF.Dev2Interface.WriteTrackInfo(gwf.getFK_Flow(), gwf.getFK_Node(), workid, 0, "任务被" + WebUser.getName() + "从任务池取走.", "获取");
		if (i == 1) {
			return true;
		} else {
			return false;
	...
	public static void Node_TaskPoolPutOne(long workid) {
		...
		BP.WF.Dev2Interface.WriteTrackInfo(gwf.getFK_Flow(), gwf.getFK_Node(), workid, 0, "任务被" + WebUser.getName() + "放入了任务池.", "放入");
	}
	...
	public static void Node_AddNextStepAccepters(long workID, int formNodeID, String emps) {
		Node_AddNextStepAccepters(workID, formNodeID, emps, true);
	}

	//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
	//ORIGINAL LINE: public static void Node_AddNextStepAccepters(Int64 workID, int formNodeID, string emps, bool Del_Selected = true)
	public static void Node_AddNextStepAccepters(long workID, int formNodeID, String emps, boolean Del_Selected) {
		SelectAccper sa = new SelectAccper();
		//删除历史选择
	 ...
	 * @param emp 接收人
	 * @param tag 分组维度，可以为空.是为了分流节点向下发送时候，可能有一个工作人员两个或者两个以上的子线程的情况出现。 tag 是个维度，这个维度可能是一个类别，一个批次，一个标记，总之它是一个字符串。详细:
	...
	public static Date Node_GetHungUpTimeSpan(String flowNo, int nodeID, long workid) {
		...
		Paras ps = new Paras();
		ps.SQL = "SELECT * FROM ND" + Integer.parseInt(flowNo) + "Track WHERE WorkID=" + dbstr + "WorkID AND " + TrackAttr.ActionType + " in ("
				+ instr + ")  and  NDFrom=" + dbstr + "NDFrom ";
		ps.Add(TrackAttr.WorkID, workid);
	...
	public static String Node_Shift(String flowNo, int nodeID, long workID, long fid, String toEmp, String msg) {
		...
		if (nd.getTodolistModel() == TodolistModel.Order || nd.getTodolistModel() == TodolistModel.Teamup) {
			/*如果是队列模式，或者是协作模式. */
			try {
				String sql = "UPDATE WF_GenerWorkerlist SET FK_Emp='" + emp.getNo() + "', FK_EmpText='" + emp.getName() + "' WHERE FK_Emp='"
						+ WebUser.getNo() + "' AND FK_Node=" + nodeID + " AND WorkID=" + workID;
				BP.DA.DBAccess.RunSQL(sql);
			} catch (java.lang.Exception e) {
			...
			//记录日志.
			Glo.AddToTrack(ActionType.Shift, nd.getFK_Flow(), workID, fid, nd.getNodeID(), nd.getName(), WebUser.getNo(), WebUser.getName(),
					nd.getNodeID(), nd.getName(), toEmp, emp.getName(), msg, null);
			...
			info += "@<a href='" + Glo.getCCFlowAppPath() + "WF/MyFlowInfo.jsp?DoType=UnShift&FK_Flow=" + nd.getFK_Flow() + "&WorkID=" + workID
					+ "&FK_Node=" + nodeID + "&FID=" + fid + "' ><img src='Img/Action/UnSend.png' border=0 />撤消工作移交</a>.";
			return info;
		...
		if (gwf.RetrieveFromDBSources() == 0) {
			...
			ps = new Paras();
			ps.SQL = "UPDATE WF_GenerWorkerlist SET IsEnable=1  WHERE WorkID=" + dbStr + "WorkID AND FK_Node=" + dbStr + "FK_Node AND FK_Emp="
					+ dbStr + "FK_Emp";
			ps.Add(GenerWorkerListAttr.WorkID, workID);
			ps.Add(GenerWorkerListAttr.FK_Node, nodeID);
			...
			if (i == 0) {
				...
				if (wls.size() == 0) {
					...
					//   mygwfl.FK_DeptT = BP.Web.WebUser.getFK_Dept()Name;
					wl.setWorkID(workID);
					wl.setFID(0);
			...
			ps = new Paras();
			ps.SQL = "UPDATE WF_GenerWorkFlow SET WFState=" + WFState.Shift.getValue() + ", WFSta=" + WFSta.Runing.getValue() + "   WHERE WorkID="
					+ dbStr + "WorkID ";
			ps.Add(GenerWorkerListAttr.WorkID, workID);
			DBAccess.RunSQL(ps);
		}
		...
		sf.Insert();
		//记录日志.
		Glo.AddToTrack(ActionType.Shift, nd.getFK_Flow(), workID, gwf.getFID(), nd.getNodeID(), nd.getName(), WebUser.getNo(), WebUser.getName(),
				nd.getNodeID(), nd.getName(), toEmp, emp.getName(), msg, null);

		//发送邮件.
		BP.WF.Dev2Interface.Port_SendMsg(emp.getNo(), WebUser.getName() + "向您移交了工作" + gwf.getTitle(), "移交信息:" + msg,
				"SF" + workID + "_" + sf.getFK_Node(), BP.WF.SMSMsgType.Shift, gwf.getFK_Flow(), gwf.getFK_Node(), gwf.getWorkID(), gwf.getFID());
		...
		inf1o += "@<a href='" + Glo.getCCFlowAppPath() + "WF/MyFlowInfo.jsp?DoType=UnShift&FK_Flow=" + nd.getFK_Flow() + "&WorkID=" + workID
				+ "&FK_Node=" + nodeID + "&FID=" + fid + "' ><img src='Img/Action/UnSend.png' border=0 />撤消工作移交</a>.";
		return inf1o;
	}
	...
	public static String Node_ReturnWork(String fk_flow, long workID, long fid, int currentNodeID, int returnToNodeID, String returnToEmp,
			String msg, boolean isBackToThisNode) {
		// 转化成编号.
		fk_flow = TurnFlowMarkToFlowNo(fk_flow);
	 ...
	 */
	public static String Node_ReturnWork(String fk_flow, long workID, long fid, int currentNodeID, int returnToNodeID, String msg,
			boolean isBackToThisNode) {
	...
	public static int Node_GetCurrentNodeID(String fk_flow, long workid) {
		...
		int nodeID = BP.DA.DBAccess.RunSQLReturnValInt("SELECT FK_Node FROM WF_GenerWorkFlow WHERE WorkID=" + workid + " AND FK_Flow='" + fk_flow
				+ "'", 0);
		if (nodeID == 0) {
	...
	public static void Node_SetWorkRead(int nodeID, long workid, String empNo) {
		...
		String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		ps.SQL = "UPDATE WF_GenerWorkerList SET IsRead=1 WHERE WorkID=" + dbstr + "WorkID AND FK_Node=" + dbstr + "FK_Node AND FK_Emp=" + dbstr
				+ "FK_Emp";
		ps.Add("WorkID", workid);
		ps.Add("FK_Node", nodeID);
		ps.Add("FK_Emp", empNo);
		if (DBAccess.RunSQL(ps) == 0) {
			throw new RuntimeException("@设置的工作不存在，或者当前的登录人员已经改变。");
		}
		...
		if (isSend == true) {
			...
			// 获取流程的发送人.
			ps = new Paras();
			ps.SQL = "SELECT " + GenerWorkerListAttr.Sender + " FROM WF_GenerWorkerlist WHERE WorkID=" + dbstr + "WorkID AND FK_Node=" + dbstr
					+ "FK_Node ";
			ps.Add("WorkID", workid);
			ps.Add("FK_Node", nodeID);
			dt = DBAccess.RunSQLReturnTable(ps);
			...
			//发送已读回执。
			BP.WF.Dev2Interface.Port_SendMsg(sender, "已读回执:" + title, "您发送的工作已经被" + WebUser.getName() + "在" + DataType.getCurrentDataTimeCNOfShort()
					+ " 打开.", "RP" + workid + "_" + nodeID, BP.WF.SMSMsgType.Self, nd.getFK_Flow(), nd.getNodeID(), workid, 0);
		}
	...
	public static void Node_SetWorkUnRead(int nodeID, long workid, String userNo) {
		String dbstr = BP.Sys.SystemConfig.getAppCenterDBVarStr();
		Paras ps = new Paras();
		ps.SQL = "UPDATE WF_GenerWorkerList SET IsRead=0 WHERE WorkID=" + dbstr + "WorkID AND FK_Node=" + dbstr + "FK_Node AND FK_Emp=" + dbstr
				+ "FK_Emp";
		ps.Add("WorkID", workid);
		ps.Add("FK_Node", nodeID);
		ps.Add("FK_Emp", userNo);
	...
	public static void Node_SetWorkUnRead(int nodeID, long workid) {
		Node_SetWorkUnRead(nodeID, workid, BP.Web.WebUser.getNo());
	}
	...
	public static void UI_Window_AskForHelp(String fk_flow, int fk_node, long workid, long fid) throws IOException {
		...
		String urlr3 = Glo.getCCFlowAppPath() + "WF/WorkOpt/Askfor.jsp?FK_Node=" + fk_node + "&FID=" + fid + "&WorkID=" + workid + "&FK_Flow="
				+ fk_flow + "&s=" + tKey;
	...
	public static void UI_Window_JumpWay(String fk_flow, int fk_node, long workid, long fid) throws IOException {
		...
		PubClass.WinOpen(ContextHolderUtils.getResponse(), Glo.getCCFlowAppPath() + "WF/JumpWaySmallSingle.aspx?FK_Flow=" + fk_flow + "&FK_Node="
				+ fk_node + "&WorkID=" + workid + "&FID=" + fid, 500, 400);
	}
	...
	public static void UI_Window_FlowChartTruck(String fk_flow, int nodeID, long workid, long fid) throws IOException {
		// 转化成编号.
		fk_flow = TurnFlowMarkToFlowNo(fk_flow);
		PubClass.WinOpen(ContextHolderUtils.getResponse(), Glo.getCCFlowAppPath() + "WF/WorkOpt/OneWork/ChartTrack.aspx?FK_Flow=" + fk_flow
				+ "&WorkID=" + workid + "&FID=" + fid, 500, 400);
	}
	...
	public static void UI_Window_ThreadInfo(String fk_flow, int fk_node, long workid, long fid) throws IOException {
		...
		String url = Glo.getCCFlowAppPath() + "WF/WorkOpt/threaddtl.jsp?FK_Node=" + fk_node + "&FID=" + fid + "&WorkID=" + workid + "&FK_Flow=" + fk_flow
				+ "&s=" + key;
	...
	public static int WorkOpt_ToNodes_GetLasterSelectNodeID(String flowNo, int nodeID) {
		...
		case MSSQL:
		case Access:
			sql = "SELECT TOP 1 NDTo FROM ND" + Integer.parseInt(flowNo) + "Track WHERE EmpFrom='" + BP.Web.WebUser.getNo() + "' AND NDFrom="
					+ nodeID + " AND (ActionType=" + ActionType.Forward.getValue() + " OR ActionType=" + ActionType.ForwardFL.getValue()
					+ " OR ActionType=" + ActionType.SubFlowForward.getValue() + ")  ORDER BY RDT DESC";
			break;
		case Oracle:
			sql = "SELECT NDTo FROM ND" + Integer.parseInt(flowNo) + "Track WHERE  RowNum=1 AND EmpFrom='" + BP.Web.WebUser.getNo() + "' AND NDFrom="
					+ nodeID + " AND (ActionType=" + ActionType.Forward.getValue() + " OR ActionType=" + ActionType.ForwardFL.getValue()
					+ " OR ActionType=" + ActionType.SubFlowForward.getValue() + ")  ORDER BY RDT DESC";
			break;
		case MySQL:
			sql = "SELECT NDTo FROM ND" + Integer.parseInt(flowNo) + "Track WHERE  limit 0,1  AND EmpFrom='" + BP.Web.WebUser.getNo()
					+ "' AND NDFrom=" + nodeID + " AND (ActionType=" + ActionType.Forward.getValue() + " OR ActionType="
					+ ActionType.ForwardFL.getValue() + " OR ActionType=" + ActionType.SubFlowForward.getValue() + ")  ORDER BY RDT DESC";
			break;
		case Informix:
			sql = "SELECT first 1 NDTo FROM ND" + Integer.parseInt(flowNo) + "Track WHERE EmpFrom='" + BP.Web.WebUser.getNo() + "' AND NDFrom="
					+ nodeID + " AND (ActionType=" + ActionType.Forward.getValue() + " OR ActionType=" + ActionType.ForwardFL.getValue()
					+ " OR ActionType=" + ActionType.SubFlowForward.getValue() + ")  ORDER BY RDT DESC";
			break;
		default:
	 ...
	 * @throws Exception
	 */
	public static SendReturnObjs WorkOpt_SendToEmps(String flowNo, int nodeID, long workid, long fid, int toNodeID, String toEmps,
			boolean isRememberMe) throws Exception {
	...
	public static String SaveImageAsFile(byte[] img, String pkval, String fk_Frm_Ele) {
		...
		//			Bitmap bitmap = new Bitmap(newImage, new Size(fe.getWOfInt(), fe.getHOfInt()));
		//
		//			if ((new java.io.File(fe.getHandSigantureSavePath() + "\\" + fe.getFK_MapData() + "\\")).isDirectory() == false)
		//			{
		//				(new java.io.File(fe.getHandSigantureSavePath() + "\\" + fe.getFK_MapData() + "\\")).mkdirs();
		//			}
		//
		//			String saveTo = fe.getHandSigantureSavePath() + "\\" + fe.getFK_MapData() + "\\" + pkval + ".jpg";
		//			bitmap.Save(saveTo, ImageFormat.Jpeg);
		//
		//			String pathFile = BP.Sys.Glo.Request.ApplicationPath + fe.getHandSiganture_UrlPath() + fe.getFK_MapData() + "/" + pkval + ".jpg";
		//			FrmEleDB ele = new FrmEleDB();
		//			ele.setFK_MapData(fe.getFK_MapData());
		//			ele.setEleID(fe.getEleID());
		//			ele.setRefPKVal(pkval);
		//			ele.setTag1(pathFile.replace("\\\\", "\\"));
		//			ele.setTag1(pathFile.replace("////", "//"));
		//
		//			ele.setTag2(saveTo.replace("\\\\", "\\"));
		//			ele.setTag2(saveTo.replace("////", "//"));
		//
		//			ele.GenerPKVal();
		//			ele.Save();
	...
	///#region 调度相关的操作.
	/**
	 * 更新时间状态, 交付给 huangzhimin. 作用：按照当前的时间，每天两次更新WF_GenerWorkFlow 的 TodoSta 状态字段。 该字段： 0=正常, 1=预警, 2=逾期, 3=按时完成 , 4=逾期完成.
	...
	public static void DTS_GenerWorkFlowTodoSta() throws Exception {
		...
		sql += " WHERE WorkID IN (SELECT WorkID FROM WF_GenerWorkerlist A WHERE a.DTOfWarning >'" + timeDT + "' AND a.SDT <'" + timeDT
				+ "' AND A.IsPass=0 ) ";
	 ...
	 * @date 2016年7月16日
	 */
	public static Emp getEmpByParam(String userNo,String tel,String mialAddr){
		Emp emp=new Emp();
		userNo=userNo!=null?userNo:"";
		tel=tel!=null?tel:"";
		mialAddr=mialAddr!=null?mialAddr:"";
		
		//用户编号、电话、邮件地址全不为空进行查询
		if(!"".equals(userNo) && !"".equals(tel) &&!"".equals(mialAddr) ){
			emp.Retrieve(EmpAttr.No, userNo,EmpAttr.Tel,tel,EmpAttr.Email,mialAddr);
		}
		//用户编号、电话不为空进行查询
		else if(!"".equals(userNo) && !"".equals(tel)){
			emp.Retrieve(EmpAttr.No, userNo,EmpAttr.Tel,tel);
		}
		//用户编号、邮件地址不为空进行查询
		else if(!"".equals(userNo) && !"".equals(mialAddr)){
			emp.Retrieve(EmpAttr.No, userNo,EmpAttr.Email,mialAddr);
		}
		//电话、邮件地址不为空进行查询
		else if(!"".equals(tel) && !"".equals(mialAddr)){
			emp.Retrieve(EmpAttr.Tel, tel,EmpAttr.Email,mialAddr);
		}
		//用户编号不为空进行查询
		else if(!"".equals(userNo)){
			emp.Retrieve(EmpAttr.No, userNo);
		}
		//电话不为空进行查询
		else if(!"".equals(tel)){
			emp.Retrieve(EmpAttr.Tel, tel);
		}
		//邮件地址不为空进行查询
		else if(!"".equals(mialAddr)){
			emp.Retrieve(EmpAttr.Email, mialAddr);
		}
		return emp;
	}
}