...
public class WorkUnSend
{
	...
	public final String getAppType()
	{
		if (_AppType == null)
		{
			...
			else
			{
				...
				{
					//boolean b = BP.Sys.Glo.Request.RawUrl.toLowerCase().contains("oneflow");
					boolean b = BP.Sys.Glo.getRequest().getRemoteAddr().toLowerCase().contains("oneflow");
					if (b)
					{
						_AppType = "WF/OneFlow";
					}
	...
	*/
    ...
    /** 
    * @Title: DoThreadUnSend 
    * @Description: 执行子线程的撤销. 
    * @return
    * @author dongliang
    * @date 2016年8月16日
    */ 
    private String DoThreadUnSend(){
        ...
        if (nd.getHisCancelRole() == CancelRole.SpecNodes){
            ...
            if (ncs.size() == 0)
				try {
					throw new Exception("@流程设计错误, 您设置了当前节点(" + wn.getHisNode().getName() + ")可以让指定的节点人员撤销，但是您没有设置指定的节点.");
				} catch (Exception e) {		
					e.printStackTrace();
				}
        ...
        if (nd.getHisCancelRole() == CancelRole.OnlyNextStep){
            ...
            int num = wl.Retrieve(GenerWorkerListAttr.FK_Emp, BP.Web.WebUser.getNo(),
        ...
        if (cancelToNode.getIsStartNode() == false && cancelToNode.getIsEnableTaskPool() == false){
            ...
            String[] empStrs = rm.getObjs().split("@");
        ...
        if (nd.getIsStartNode()) {
            ...
            if (gwfs.size() > 0) {
                for(GenerWorkFlow item: gwfs.Tolist()){
                    /*删除每个子线程.*/
        ...
        if (wnOfCancelTo.getHisNode().getIsStartNode())
            return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.aspx?FK_Flow=" + this.FlowNo + "&FID=" + this.FID + "&WorkID=" + this.WorkID + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A> , <a href='" + this.getVirPath() + this.getAppType() + "/Do.aspx?ActionType=DeleteFlow&WorkID=" + wn.getHisWork().getOID() + "&FK_Flow=" + this.FlowNo + "' /><img src='" + getVirPath() + "WF/Img/Btn/Delete.gif' border=0/>此流程已经完成(删除它)</a>。" + msg;
        else
            return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.aspx?FK_Flow=" + this.FlowNo + "&FID=" + this.FID + "&WorkID=" + this.WorkID + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A>  . " + msg;
	...
	public final String DoUnSend()
	{
		...
		if (wnOfCancelTo.getHisNode().getIsStartNode())
		{
			if (BP.Web.WebUser.getIsWap())
			{
				...
				{
					return "@撤消发送执行成功，您可以点这里<a href='" + getVirPath() + "WF/Wap/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A> , <a href='" + getVirPath() + "WF/Wap/MyFlowInfo.jsp?DoType=DeleteFlow&WorkID=" + wn.getHisWork().getOID() + "&FK_Flow=" + this.FlowNo + "' /><img src='" + getVirPath() + "WF/Img/Btn/Delete.gif' border=0/>此流程已经完成(删除它)</a>。" + msg;
				}
				else
				{
					return "@撤销成功." + msg;
			...
			else
			{
				switch (wnOfCancelTo.getHisNode().getHisFormType())
				{
					case FixForm:
					case FreeForm:
						return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A> , <a href='" + this.getVirPath() + this.getAppType() + "/Do.jsp?ActionType=DeleteFlow&WorkID=" + wn.getHisWork().getOID() + "&FK_Flow=" + this.FlowNo + "' /><img src='" + getVirPath() + "WF/Img/Btn/Delete.gif' border=0/>此流程已经完成(删除它)</a>。" + msg;
					default:
						return "撤销成功" + msg;
				}
			}
		...
		else
		{
			...
			switch (wnOfCancelTo.getHisNode().getHisFormType())
			{
				case FixForm:
				case FreeForm:
					return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A>  . " + msg;
				default:
					return "撤销成功:" + msg;
			}
		}
	...
	private String DoUnSendFeiLiu(GenerWorkFlow gwf)
	{
		...
		for (Node ndNext : nd.getHisToNodes().ToJavaList())
		{
			...
			if (ndNext.getHisRunModel() == RunModel.SubThread)
			{
				...
				{
					BP.WF.Dev2Interface.Flow_DeleteSubThread(gwf.getFK_Flow(), en.getWorkID(), "撤销发送删除");
				}
				continue;
			}
		...
		if (cNode.getIsStartNode())
		{
			if (BP.Web.WebUser.getIsWap())
			{
				return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FID=0&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A> , <a href='MyFlowInfo.jsp?DoType=DeleteFlow&WorkID=" + cWork.getOID() + "&FK_Flow=" + this.FlowNo + "' /><img src='" + getVirPath() + "WF/Img/Btn/Delete.gif' border=0/>此流程已经完成(删除它)</a>。" + msg;
			}
			else
			{
				...
				{
					return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FID=0&FK_Node=" + gwf.getFK_Node() + "' ><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A> , <a href='Do.jsp?ActionType=DeleteFlow&WorkID=" + cWork.getOID() + "&FK_Flow=" + this.FlowNo + "' /><img src='" + getVirPath() + "WF/Img/Btn/Delete.gif' border=0/>此流程已经完成(删除它)</a>。" + msg;
				}
			}
		}
		else
		{
			...
			if (BP.Web.WebUser.getIsWap() == false)
			{
				if (!this.getHisFlow().getFK_FlowSort().equals("00"))
				{
					return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FID=0&FK_Node=" + gwf.getFK_Node() + "' ><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A>。" + msg;
				}
				else
				{
					return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FID=0&FK_Node=" + gwf.getFK_Node() + "' ><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A>。" + msg;
	...
	public final String DoUnSendHeiLiu_Main(GenerWorkFlow gwf)
	{
		...
		if (wnPri.getHisNode().getIsStartNode())
		{
			if (BP.Web.WebUser.getIsWap())
			{
				...
				{
					return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A> , <a href='" + this.getVirPath() + this.getAppType() + "/MyFlowInfo.jsp?DoType=DeleteFlow&WorkID=" + wn.getHisWork().getOID() + "&FK_Flow=" + this.FlowNo + "' /><img src='" + getVirPath() + "WF/Img/Btn/Delete.gif' border=0/>此流程已经完成(删除它)</a>。";
				}
				else
				{
					return "@撤销成功.";
...
///#warning
			...
			if (BP.Web.WebUser.getIsWap() == false)
			{
				return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A>。";
			}
			else
			{
				return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A>。";
	...
	public final String DoUnSendSubFlow(GenerWorkFlow gwf)
	{
		...
		if (wnPri.getHisNode().getIsStartNode())
		{
			if (BP.Web.WebUser.getIsWap())
			{
				return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FID=" + gwf.getFID() + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A> , <a href='" + getVirPath() + "WF/MyFlowInfo.jsp?DoType=DeleteFlow&WorkID=" + wn.getHisWork().getOID() + "&FK_Flow=" + this.FlowNo + "' /><img src='" + getVirPath() + "WF/Img/Btn/Delete.gif' border=0/>此流程已经完成(删除它)</a>。" + msg;
			}
			else
			{
				if (!this.getHisFlow().getFK_FlowSort().equals("00"))
				...
				{
					return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FID=" + gwf.getFID() + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A> , <a href='" + getVirPath() + "WF/Do.jsp?ActionType=DeleteFlow&WorkID=" + wn.getHisWork().getOID() + "&FK_Flow=" + this.FlowNo + "' /><img src='" + getVirPath() + "WF/Img/Btn/Delete.gif' border=0/>此流程已经完成(删除它)</a>。" + msg;
				}
			}
		}
		else
		{
			...
			if (BP.Web.WebUser.getIsWap() == false)
			{
				if (!this.getHisFlow().getFK_FlowSort().equals("00"))
				{
					return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FID=" + gwf.getFID() + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A>。" + msg;
				}
				else
				{
					return "@撤消执行成功，您可以点这里<a href='" + this.getVirPath() + this.getAppType() + "/MyFlow.jsp?FK_Flow=" + this.FlowNo + "&WorkID=" + this.WorkID + "&FID=" + gwf.getFID() + "&FK_Node=" + gwf.getFK_Node() + "'><img src='" + getVirPath() + "WF/Img/Btn/Do.gif' border=0/>执行工作</A>。" + msg;
