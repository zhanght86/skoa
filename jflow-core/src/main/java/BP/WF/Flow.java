...
import BP.WF.Template.CFlowWay;
...
public class Flow extends BP.En.EntityNoName
{
	...
	*/
	public final String getFlowJson() {
		try {
			return this.GetBigTextFromDB("FlowJson","UTF-8");
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
...
/**************************************************************************/
	...
	*/
	...
	{
		return TimelineRole
				.forValue(this.GetValIntByKey(FlowAttr.TimelineRole));
	}
	...
	public final Work NewWork(Emp emp, Hashtable paras)
	{
		...
		try
		{
			...
			if (this.getIsGuestFlow() == true && StringHelper.isNullOrEmpty(GuestUser.getNo()) == false)
			{
				...
				if (dt.Rows.size() > 0 && IsNewWorkID == false)
				{
					...
					if (nodeID != this.getStartNodeID())
					{
						...
						Log.DefaultLogWriteLineError("NewWork "+error);
			...
			if (wk.getOID() == 0 || IsNewWorkID == true)
			{
				...
				wk.SetValByKey(GERptAttr.FK_Dept, emp.getName());
				wk.setFID(0);
				wk.DirectInsert();
				...
				if (this.getPTable().equals(wk.getEnMap().getPhysicsTable()))
				{
					...
					//WebUser.getNo() + "," + BP.Web.WebUser.getName() + "在" + DataType.CurrentDataCNOfShort + "发起.";
				...
				else
				{
					...
					rpt.setTitle(WorkNode.GenerTitle(this, wk));
					// rpt.Title = WebUser.getNo() + "," + BP.Web.WebUser.getName() + "在" + DataType.CurrentDataCNOfShort + "发起.";
		...
		if (StringHelper.isNullOrEmpty(PNodeIDStr) == false && StringHelper.isNullOrEmpty(PWorkIDStr) == false)
		{
			...
			if (CopyFormWorkID != null)
			{
				...
				rpt.SetValByKey(GERptAttr.FK_Dept, emp.getName());
				...
				//生成单据编号.
				Object tempVar2 = this.getBillNoFormat();
				String billNoFormat = (String)((tempVar2 instanceof String) ? tempVar2 : null);
				if (StringHelper.isNullOrEmpty(billNoFormat) == false)
				{
					rpt.SetValByKey(GERptAttr.BillNo, BP.WF.Glo.GenerBillNo(billNoFormat, rpt.getOID(), rpt, this.getPTable()));
				}
			}
			...
			if (dtls.size() > 0)
			{
				...
				if (dtlsFrom.size() == dtls.size())
				{
					for (MapDtl dtl : dtls.ToJavaList())
					{
						...
						MapDtl dtlFrom = (MapDtl)((dtlsFrom.getItem(idx) instanceof MapDtl) ? dtlsFrom.getItem(idx) : null);
		...
		if (mygwf.RetrieveFromDBSources() == 0)
		{
			...
			mygwf.setDeptName(BP.Web.WebUser.getFK_Dept());
...
///#warning 不应该出现的工作丢失.
		...
		if (nd.getIsStartNode())
		{
			try
			{
				...
				{
					/*让他自动生成编号*/
					wk.SetValByKey(NDXRptBaseAttr.BillNo, BP.WF.Glo.GenerBillNo(nd.getHisFlow().getBillNoFormat(), wk.getOID(), wk, nd.getHisFlow().getPTable()));
				}
			}
...
//
//
//			String mysql = "\t\n SELECT '" + fl.getFK_FlowSort() + "' AS FK_FlowSort,'" + fl.getNo() + "' AS FK_Flow,OID,FID,Title,WFState,CDT,FlowStarter,FlowStartRDT,FK_Dept,FK_NY,FlowDaySpan,FlowEmps,FlowEnder,FlowEnderRDT,FlowEndNode,1 as MyNum,PWorkID,PFlowNo,BillNo,PrjNo FROM " + fl.getPTable() + " WHERE WFState >1 ";
//			try
//			{
...
//		return null;
	...
	*/
	public final String DoCheck()
	{
			...
			for (Node nd : nds.ToJavaList())
			{
				...
				switch (nd.getHisDeliveryWay())
				{
					...
					case ByDeptAndStation: //按部门与岗位的交集计算.
						...
						{
							mysql = "SELECT No FROM Port_Emp WHERE No IN (SELECT FK_Emp FROM Port_EmpDept WHERE FK_Dept IN ( SELECT FK_Dept FROM WF_NodeDept WHERE FK_Node=" + nd.getNodeID() + "))AND No IN (SELECT FK_Emp FROM " + BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN ( SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + nd.getNodeID() + " )) ORDER BY No ";
						}
						else
						{
		...
		{
			throw new RuntimeException("@检查流程错误:" + ex.getMessage(), ex);
		}
	}
	/** 
	...
	*/
	...
	{
		String name = this.getNo() + "_" + this.getName(), path = PathFlowDesc
				+ name + "/";
		this.DoExpFlowXmlTemplete(path);
		return path + this.getName() + "_" + this.getVer().replace(":", "_")
				+ ".xml";
...
//			String name = this.getName();
//			name = BP.Tools.StringExpressionCalculate.ReplaceBadCharOfFileName(name);
//			name = name + "." + this.getVer().replace(":", "_") + ".xml";
...
//						(new java.io.File(path)).mkdirs();
//					}
//					else if ((new java.io.File(xmlName)).isFile())
//					{
//						java.util.Date time = File.GetLastWriteTime(xmlName);
//						String xmlNameOld = path + "Flow" + time.toString("@yyyyMMddHHmmss") + ".xml";
//
//						isXmlLocked = true;
//						if ((new java.io.File(xmlNameOld)).isFile())
//						{
//							(new java.io.File(xmlNameOld)).delete();
//						}
//						File.Move(xmlName, xmlNameOld);
//					}
//				}
//
//				if (!tangible.StringHelper.isNullOrEmpty(xmlName))
//				{
//					ds.WriteXml(xmlName);
//					isXmlLocked = false;
//				}
...
//				rpt.setFlowDaySpan(ts.Days);
	...
	*/
	private void CheckRptViewDel(Nodes nds)
	{
		...
		sql += "/* CCFlow Auto Create :" + this.getName() + " Date:" + BP.Tools.DateUtils.getCurrentDate() + " */ ";
	...
	*/
	private void DoCheck_CheckRpt(Nodes nds)
	{
		...
		{
			case Oracle:
				sql = "SELECT MyPK, KeyOfEn FROM Sys_MapAttr WHERE FK_MapData IN ( SELECT 'ND' " + SystemConfig.getAppCenterDBAddStringStr() + " cast(NodeID as varchar(20)) FROM WF_Node WHERE FK_Flow='" + this.getNo() + "')";
				break;
			case MySQL:
				sql = "SELECT MyPK, KeyOfEn FROM Sys_MapAttr WHERE FK_MapData IN (SELECT X.No FROM ( SELECT CONCAT('ND',NodeID) AS No FROM WF_Node WHERE FK_Flow='" + this.getNo() + "') AS X )";
				break;
			default:
				sql = "SELECT MyPK, KeyOfEn FROM Sys_MapAttr WHERE FK_MapData IN ( SELECT 'ND' " + SystemConfig.getAppCenterDBAddStringStr() + " cast(NodeID as varchar(20)) FROM WF_Node WHERE FK_Flow='" + this.getNo() + "')";
				break;
		...
		if (!attrs.Contains(md.getNo() + "_" + GERptAttr.OID))
		{
			/* WorkID */
		...
		if (!attrs.Contains(md.getNo() + "_" + GERptAttr.PrjName))
		{
			/* 项目名称 */
			MapAttr attr = new MapAttr();
			attr.setFK_MapData(md.getNo());
	...
	*/
	...
	public final String DoFlowEventEntity(String doType, Node currNode, Entity en, String atPara, SendReturnObjs objs, Node jumpToNode)
	{
		return DoFlowEventEntity(doType, currNode, en, atPara, objs, jumpToNode, null);
	}
...
//ORIGINAL LINE: public string DoFlowEventEntity(string doType, Node currNode, Entity en, string atPara, SendReturnObjs objs, Node jumpToNode=null, string jumpToEmps=null)
	public final String DoFlowEventEntity(String doType, Node currNode, Entity en, String atPara, SendReturnObjs objs, Node jumpToNode, String jumpToEmps)
	{
		...
		if (isSendEmail)
		{
			...
			mailTitleTmp = mailTitleTmp.replace("@WebUser.getNo()", WebUser.getNo());
			mailTitleTmp = mailTitleTmp.replace("@WebUser.getName()", WebUser.getName());
		...
		for (String emp : emps)
		{
			...
			//发送消息.
			BP.WF.Dev2Interface.Port_SendMsg(emp, mailTitleTmp, mailDocReal, smsDocTmp, en.getPKVal().toString(), doType, this.getNo(), currNode.getNodeID(), Long.parseLong(en.getPKVal().toString()), 0, isSendEmail, isSendSMS);
		}
		return str;
	}
	...
	{
		return DoFlowEventEntity(doType, currNode, en, atPara, jumpToNode, null);
	}
...
//C# TO JAVA CONVERTER NOTE: Java does not support optional parameters. Overloaded method(s) are created above:
//ORIGINAL LINE: public string DoFlowEventEntity(string doType, Node currNode, Entity en, string atPara, Node jumpToNode=null, string jumpToEmp=null)
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(FlowAttr.SysType, null, "系统类别", true, false, 0, 5, 10);
	...
	*/
	...
	{
		this.DoCheck();
		return Glo.getCCFlowAppPath() + "WF/Admin/Exp.aspx?CondType=0&FK_Flow=" + this.getNo();
		//PubClass.WinOpen(ContextHolderUtils.getResponse(),Glo.CCFlowAppPath + "WF/Admin/Exp.aspx?CondType=0&FK_Flow=" + this.getNo(), "单据", "cdsn", 800, 500, 210, 300);
		//return null;
	}
	/** 
	 定义报表
	...
	*/
	...
	public final String DoDelData()
	{
		...
		//DBAccess.RunSQL("DELETE FROM WF_WorkList WHERE FK_Flow='" + this.getNo() + "'");
	...
	*/
	...
	public static Flow DoLoadFlowTemplate(String fk_flowSort, String path, ImpFlowTempleteModel model, int SpecialFlowNo)
	{
		...
		//{
		//    string FlowMark = dtFlow.Rows.get(0).getValue("FlowMark").ToString();
		...
		if (SpecialFlowNo >= 0){
			//更新流程图
			fl.setFlowJson(fjson.getFlowJson());
		}
		///#endregion 处理流程表数据
		...
		if (mydtGF != null)
		{
			//throw new Exception("@" + fl.No + fl.getName() + ", 缺少：Sys_GroupField");
		...
		for (DataTable dt : ds.Tables)
		{
			...
			if(tableName.equals("WF_Flow")) //模版文件。
			{
				continue;
			} else if (tableName.equals("WF_FlowFormTree")) //独立表单目录 add 2013-12-03
			{
				...
				//    cd.Insert();
				//}
			} else if (tableName.equals("WF_FlowForm")) //独立表单。 add 2013-12-03
			{
				//foreach (DataRow dr in dt.Rows)
				//{
				//    FlowForm cd = new FlowForm();
				...
				//    cd.Insert();
				//}
			} else if (tableName.equals("WF_NodeForm")) //节点表单权限。 2013-12-03
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						...
						columnName = dc.ColumnName.toLowerCase();
						if (columnName.equals("tonodeid")
								|| columnName.equals("fk_node")
								|| columnName.equals("nodeid")) {
							if (val.length() == 3)
							{
						...
						} else if (columnName.equals("fk_flow"))
						{
							val = fl.getNo();
						} else
						{
			...
			} else if (tableName.equals("Sys_FrmSln")) //表单字段权限。 2013-12-03
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("WF_NodeToolbar")) //工具栏。
			{
				for (DataRow dr : dt.Rows)
				{
					NodeToolbar cd = new NodeToolbar();
			...
			} else if (tableName.equals("WF_BillTemplate")) 
			{
...
//					try
//					{
//						File.Copy(info.getParent() + "\\" + no + ".rtf", BP.Sys.SystemConfig.getPathOfWebApp() + "\\DataUser\\CyclostyleFile\\" + bt.getNo() + ".rtf", true);
//					}
//					catch (RuntimeException ex)
//					{
...
//					bt.Insert();
//				}
			} else if (tableName.equals("WF_FrmNode"))  //Conds.xml。
			{
			...
			} else if (tableName.equals("WF_FindWorkerRole")) //找人规则
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						...
						columnName = dc.ColumnName.toLowerCase();
						if (columnName.equals("fk_node") || columnName.equals("nodeid")) {
							if (val.length() == 3)
							{
								val = flowID + val.substring(1);
							}
						...
						} else if (columnName.equals("fk_node")) {
							val = fl.getNo();
						} else {
							val = val.replace("ND" + oldFlowID, "ND" + flowID);
						}
			...
			} else if (tableName.equals("WF_Cond")) //Conds.xml。
			{
				for (DataRow dr : dt.Rows)
				{
					Cond cd = new Cond();
			...
			} else if (tableName.equals("WF_CCDept")) //抄送到部门。
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("WF_NodeReturn")) //可退回的节点。
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						...
						columnName = dc.ColumnName.toLowerCase();
						if (columnName.equals("fk_node") || columnName.equals("returnn"))
						{
							if (val.length() == 3)
							{
								val = flowID + val.substring(1);
			...
			} else if (tableName.equals("WF_Direction")) //方向。
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						...
						columnName = dc.ColumnName.toLowerCase();
						if (columnName.equals("node") || columnName.equals("tonode"))
						{
							if (val.length() == 3)
							{
								val = flowID + val.substring(1);
			...
			} else if (tableName.equals("WF_TurnTo")) //转向规则.
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("WF_FAppSet")) //FAppSets.xml。
			{
				continue;
			} else if (tableName.equals("WF_LabNote")) //LabNotes.xml。
			{
				idx = 0;
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("WF_NodeDept")) //FAppSets.xml。
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("WF_Node")) //导入节点信息.
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						...
						//  NodeAttr.NodeFrmID
						columnName = dc.ColumnName.toLowerCase();
						if (columnName.equals("nodefrmid")){
							if (val.length() == 5)
							{
						...
						} else if (columnName.equals("nodeid")){
							if (val.length() == 3)
							{
								val = flowID + val.substring(1);
							}
							...
							else if (val.length() == 5)
							{
								...
								|| columnName.equals("fk_flowsort")) {
							continue;
						} else if (columnName.equals("showsheets")
								|| columnName.equals("histonds")
								|| columnName.equals("histonds")) {
							String key = "@" + flowID;
							val = val.replace(key, "");
						}
			...
			} else if (tableName.equals("WF_NodeStation")) //FAppSets.xml。
			{
			...
			} else if (tableName.equals("WF_Listen"))  // 信息侦听。
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						...
						} else if (columnName.equals("nodes")){
			...
			} else if (tableName.equals("Sys_Enum")) //RptEmps.xml。
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_EnumMain")) //RptEmps.xml。
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_MapAttr")) //RptEmps.xml。
			{
				for (DataRow dr : dt.Rows)
				{
					...
					for (DataColumn dc : dt.Columns)
					{
						String val = (String)((dr.getValue(dc.ColumnName) instanceof String) ? dr.getValue(dc.ColumnName) : null);
						columnName = dc.ColumnName.toLowerCase();
						if (columnName.equals("fk_mapdata")
								|| columnName.equals("keyofen")
								|| columnName.equals("autofulldoc")) {
							if (val == null)
							{
								continue;
							}
			...
			} else if (tableName.equals("Sys_MapData")) //RptEmps.xml。
			{
				for (DataRow dr : dt.Rows)
				{
					MapData md = new MapData();
			...
			} else if (tableName.equals("Sys_MapDtl")) //RptEmps.xml。
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_MapExt")) 
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_FrmLine")) 
			{
				idx = 0;
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_FrmEle")) 
			{
				idx = 0;
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_FrmImg")) 
			{
				idx = 0;
			...
			} else if (tableName.equals("Sys_FrmLab")) 
			{
				idx = 0;
				timeKey = DateUtils.getCurrentDate("yyyyMMddHHmmss");
				for (DataRow dr : dt.Rows)
			...
			} else if (tableName.equals("Sys_FrmLink")) 
			{
				idx = 0;
				for (DataRow dr : dt.Rows)
				{
					...
					//en.setMyPK(Guid.NewGuid().toString());
					en.setMyPK(UUID.randomUUID().toString());
					//en.setMyPK("LK" + timeKey + "_" + idx);
					en.Insert();
				}
			} else if (tableName.equals("Sys_FrmAttachment")) 
			{
				idx = 0;
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_FrmEvent"))  //事件.
			{
				idx = 0;
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_MapM2M"))  //Sys_MapM2M.
			{
				idx = 0;
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_FrmRB"))  //Sys_FrmRB.
			{
				idx = 0;
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("WF_NodeEmp"))  //FAppSets.xml。
			{
				for (DataRow dr : dt.Rows)
				{
			...
			} else if (tableName.equals("Sys_GroupField")) 
			{
				for (DataRow dr : dt.Rows)
				{
					GroupField gf = new GroupField();
					for (DataColumn dc : dt.Columns)
					{
					  ...
					  DBAccess.RunSQL("UPDATE Sys_MapAttr SET GroupID=" + gf.getOID() + " WHERE FK_MapData='" + gf.getEnName() + "' AND GroupID=" + gf.getOID());
					try {
						gf.InsertAsOID(oid);
					} catch (Exception e) {
						e.printStackTrace();
			...
			} else if (tableName.equals("WF_CCEmp"))  // 抄送.
			{
				for (DataRow dr : dt.Rows)
				{
					CCEmp ne = new CCEmp();
			...
			} else if (tableName.equals("WF_CCStation"))  // 抄送.
			{
	...
	*/
	public final String DoNewFlow(String flowSort, String flowName, DataStoreModel model, String pTable, String FlowMark) throws Exception
	{
		try
		{
			...
			FlowSort fs = new FlowSort(flowSort);
			if ("99".equals(fs.getParentNo()) || "0".equals(fs.getParentNo()))
			{
				this.setSysType(flowSort);
			}
			else
			{
		...
		catch (RuntimeException ex)
		{
			...
			*/
			...
			//提示错误.
			throw new RuntimeException("创建流程错误:" + ex.getMessage(), ex);
		}
