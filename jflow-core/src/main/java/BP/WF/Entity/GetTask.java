...
public class GetTask extends BP.En.Entity
{
	...
	public final boolean Can_I_Do_It()
	{
		...
		switch (this.getHisDeliveryWay())
		{
			...
			case ByStation:
				...
				for (Entity obj : NodeStations.convertEntities(ndeStas))
				{
					NodeStation ndS = (NodeStation) obj;
	...
	public Map getEnMap()
	{
		...
		map.setPhysicsTable("WF_Node"); // 要物理表。
		map.setEnDesc("取回任务");
		...
		map.AddTBString(GetTaskAttr.CheckNodes, null, "工作节点s", true, false, 0,
				800, 100);
