...
public class ReturnWork extends EntityMyPK
{
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(ReturnWorkAttr.ReturnNodeName, null, "退回节点名称", true,
				true, 0, 200, 10);
		
		map.AddTBString(ReturnWorkAttr.Returner, null, "退回人", true, true, 0,
				20, 10);
		map.AddTBString(ReturnWorkAttr.ReturnerName, null, "退回人名称", true, true,
				0, 200, 10);
		
		map.AddTBInt(ReturnWorkAttr.ReturnToNode, 0, "ReturnToNode", true, true);
		map.AddTBString(ReturnWorkAttr.ReturnToEmp, null, "退回给", true, true, 0,
				4000, 10);
		
		map.AddTBString(ReturnWorkAttr.Note, "", "退回原因", true, true, 0, 4000,
				10);
