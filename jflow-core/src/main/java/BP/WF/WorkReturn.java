...
///#warning 没有考虑两种流程数据存储模式。
		...
		if (Glo.getIsEnableSysMessage() == true)
		{
			...
			BP.WF.Dev2Interface.Port_SendMsg(returnToEmp.getNo(), title, Msg, "RE" + this.HisNode.getNodeID() + this.WorkID, BP.WF.SMSMsgType.ReturnWork, ReturnToNode.getFK_Flow(), ReturnToNode.getNodeID(), this.WorkID, this.FID);
		}
	...
	*/
	private String ExeReturn5_2()
	{
		...
		BP.WF.Dev2Interface.Port_SendMsg(toEmp, gwf.getTitle(), Msg, "RE" + this.HisNode.getNodeID() + this.WorkID, BP.WF.SMSMsgType.ReturnWork, ReturnToNode.getFK_Flow(), ReturnToNode.getNodeID(), this.WorkID, this.FID);
	...
	*/
	private String ExeReturn1_1()
	{
		...
		if (this.HisNode.getTodolistModel() == TodolistModel.Order || this.HisNode.getTodolistModel() == TodolistModel.Sharing || this.HisNode.getTodolistModel() == TodolistModel.Teamup)
		{
...
///#warning 没有考虑两种流程数据存储模式。
		...
		//        this.HisNode.FlowName, this.ReturnToNode.getName(), WebUser.getName());
		...
		//把退回原因加入特殊变量里.
		this.HisWork.getRow().put("ReturnMsg", Msg);
		String text = this.HisNode.getHisFlow().DoFlowEventEntity(EventListOfNode.ReturnAfter, this.HisNode, this.HisWork, atPara, null, gwl.getFK_Emp());
	...
	*/
	public final void DeleteToNodesData(Nodes nds)
	{
		...
		for (Node nd : nds.ToJavaList())
		{
			...
			// 删除表单附件信息。
			BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmAttachmentDB WHERE RefPKVal=" + dbStr + "WorkID AND FK_MapData=" + dbStr + "FK_MapData ", "WorkID", String.valueOf(this.WorkID), "FK_MapData", "ND" + nd.getNodeID());
			// 删除签名信息。
			BP.DA.DBAccess.RunSQL("DELETE FROM Sys_FrmEleDB WHERE RefPKVal=" + dbStr + "WorkID AND FK_MapData=" + dbStr + "FK_MapData ", "WorkID", String.valueOf(this.WorkID), "FK_MapData", "ND" + nd.getNodeID());
			///#endregion 删除当前节点数据。
	...
	private WorkNode DoReturnSubFlow(int backtoNodeID, String msg, boolean isHiden)
	{
		...
		if (Glo.getIsEnableSysMessage())
		{
			...
			BP.WF.Dev2Interface.Port_SendMsg(wn.getHisWork().getRec(), title, msg, "RESub" + backtoNodeID + "_" + this.WorkID, BP.WF.SMSMsgType.ToDo, nd.getFK_Flow(), nd.getNodeID(), this.WorkID, this.FID);
		}
		return wn;
	}
}