...
public class FullSA
{
	...
	public FullSA(WorkNode currWorkNode)
	{
		...
		for (Node item : nds.ToJavaList())
		{
			...
			if (item.getHisDeliveryWay() == DeliveryWay.ByStation)
			{
				...
				// sa.Delete(SelectAccperAttr.FK_Node,item.getNo()deID, SelectAccperAttr.WorkID, workid); //删除已经存在的数据.
				sa.setFK_Emp(no);
				sa.setEmpName(name);
			...
			if (item.getHisDeliveryWay() == DeliveryWay.ByStation)
			{
				...
				if (this.HisCurrWorkNode.getHisFlow().getFlowAppType() == FlowAppType.Normal)
				{
					switch (BP.Sys.SystemConfig.getAppCenterDBType())
					{
						case MySQL:
						case MSSQL:
							sql = "select No from Port_Emp x inner join (select FK_Emp from " + BP.WF.Glo.getEmpStation() + " a inner join WF_NodeStation b ";
	...
	*/
	private void InitDT(SelectAccper sa, Node nd)
	{
		...
		dtOfShould = Glo.AddDayHoursSpan(this.LastTimeDot, nd.getTSpanDay(), nd.getTSpanMinues());
