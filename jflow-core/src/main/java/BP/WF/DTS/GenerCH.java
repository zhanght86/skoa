		...
		try
		{
			...
			for (Node nd : nds.ToJavaList())
			{
				...
				for (DataRow dr : dt.Rows)
				{
					...
					//当前的人员，如果不是就让其登录.
					String fk_emp = (String)((dr.getValue(BP.WF.TrackAttr.EmpFrom) instanceof String) ? dr.getValue(BP.WF.TrackAttr.EmpFrom) : null);
					...
					//调用他.
					Glo.InitCH(nd.getHisFlow(), nd, workid, fid, title, priRDT, sdt, DataType.ParseSysDate2DateTime(dr.getValue(TrackAttr.RDT).toString()));
