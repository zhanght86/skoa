...
public class GenerSiganture extends Method
{
	...
	public GenerSiganture()
	{
		...
		this.Help = "此功能需要用户对 " + BP.Sys.SystemConfig.getPathOfDataUser()
				+ "\\Siganture\\ 有读写权限，否则会执行失败。";
	}
	...
	public Object Do()
	{
		try
		{
			...
			String empOKs = "";
			String empErrs = "";
			for (Emp emp : Emps.convertEmps(emps))
			{
				...
				/*
				 * warning if (System.IO.File.Exists(pathMe)) { continue; }
				 * 
				 * File.Copy(BP.Sys.SystemConfig.PathOfDataUser +
				 * "\\Siganture\\Templete.JPG", path, true);
				 * 
				 * System.Drawing.Image img =
				 * System.Drawing.Image.FromFile(path); Font font = new
				 * Font(fontName, 15); Graphics g = Graphics.FromImage(img);
				 * System.Drawing.SolidBrush drawBrush = new
				 * System.Drawing.SolidBrush(System.Drawing.Color.Black);
				 * System.Drawing.StringFormat drawFormat = new
				 * System.Drawing.StringFormat
				 * (StringFormatFlags.DirectionVertical); //文本
				 * g.DrawString(emp.getName(), font, drawBrush, 3, 3);
				 * img.Save(pathMe); img.dispose(); g.dispose();
				 * 
				 * File.Copy(pathMe, BP.Sys.SystemConfig.getPathOfDataUser() +
				 * "\\Siganture\\" + emp.getName() + ".JPG", true);
				 */
			}
			return "执行成功...";
		} catch (RuntimeException ex)
		{
			return "执行失败，请确认对 " + BP.Sys.SystemConfig.getPathOfDataUser()
					+ "\\Siganture\\ 目录有访问权限？异常信息:" + ex.getMessage();
		}
	}
