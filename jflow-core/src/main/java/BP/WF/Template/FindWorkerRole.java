...
public class FindWorkerRole extends EntityOIDName
{
	...
	*/
	public final DataTable GenerWorkerOfDataTable()
	{
		...
		// 首先判断第一类别
		String sortVal = this.getSortVal0();
		if (sortVal.equals("ByDept"))
		{
			return this.GenerByDept();
		} else if (sortVal.equals("Leader") || sortVal.equals("SpecEmps"))
		{
	...
	private DataTable GenerByDept()
	{
		...
		{
			sql = "SELECT B.No,B.Name FROM Port_DeptEmp A, Port_Emp B WHERE A.FK_Dept='"
					+ deptNo
					+ "'  AND A.FK_Duty='"
					+ objVal
					+ "' AND B.No=A.FK_Emp";
		...
		{
			sql = "SELECT B.No,B.Name FROM Port_DeptEmpStation A, Port_Emp B WHERE A.FK_Dept='"
					+ deptNo
					+ "'  AND A.FK_Station='"
					+ objVal
					+ "' AND B.No=A.FK_Emp";
