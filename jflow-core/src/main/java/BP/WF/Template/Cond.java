	...
	*/
	public boolean getIsPassed()
	{
		...
		if (this.getHisDataFrom() == ConnDataFrom.Url)
		{
			...
			{
					/*是bs系统，并且是url参数执行类型.*/
				String myurl = BP.Sys.Glo.getRequest().getRequestURL().toString();
				if (myurl.indexOf('?') != -1)
				{
...
//				String text = DataType.ReadURLContext(url, 8000, encode);
		...
		try
		{
			...
			{
				throw new RuntimeException("判断条件方向出现错误：实体：" + nd.getEnDesc() + " 属性" + this.getAttrKey() + "已经不存在.");
			}
			...
			{
				throw new RuntimeException("@没有找到操作符号(" + StringUtils.trim(this.getFK_Operator().toLowerCase()) + ").");
			}
		}
		catch (RuntimeException ex)
		{
