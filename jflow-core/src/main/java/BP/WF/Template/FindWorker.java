...
public class FindWorker
{
	...
	*/
	...
	private DataTable FindByWorkFlowModel()
	{
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySQL || town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySQLAsSubThreadEmpsAndData)
		{
			...
			if (sql.contains("@"))
			{
				if (Glo.getSendHTOfTemp() != null)
				{
					...
					{
						sql = sql.replace("@" + key, Glo.getSendHTOfTemp().get(key).toString());
					}
				}
			}

			dt = DBAccess.RunSQLReturnTable(sql);
			if (dt.Rows.size() == 0 && town.getHisNode().getHisWhenNoWorker() != WhenNoWorker.Skip)
			{
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.ByDtlAsSubThreadEmps)
		{
			...
			for (MapDtl dtl : dtls.ToJavaList())
			{
				try
				{
					...
					if (StringHelper.isNullOrEmpty(empFild))
					{
						empFild = " UserNo ";
					}

					ps = new Paras();
					ps.SQL = "SELECT " + empFild + ", t.* FROM " + dtl.getPTable() + " t WHERE RefPK=" + dbStr + "OID ORDER BY OID";
		...
		///#region 按照选择的人员处理。
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySelected || town.getHisNode().getHisDeliveryWay() == DeliveryWay.ByFEE)
		{
			ps = new Paras();
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.ByPreviousNodeFormEmpsField)
		{
			...
			{
				throw new RuntimeException("@您设置的当前节点按照指定的人员，决定下一步的接受人员，但是你没有在节点表单中设置该表单" + specEmpFields + "字段。");
			}

			//获取接受人并格式化接受人, 
			String emps = this.currWn.getHisWork().GetValStringByKey(specEmpFields);
			emps = emps.replace(" ", "");
		...
		if (town.getHisNode().getHisDeliveryWay() == DeliveryWay.BySpecNodeEmpStation)
		{
			...
			if (DataType.IsNumStr(para) == true)
			{
				...
				if (dt.Rows.size() != 1)
				{
					throw new RuntimeException("@流程设计错误，到达的节点（" + town.getHisNode().getName() + "）在指定的节点中没有数据，无法找到工作的人员。");
				}
		...
		/* 如果执行节点 与 接受节点岗位集合一致 */
		if (this.currWn.getHisNode().getGroupStaNDs().equals(town.getHisNode().getGroupStaNDs()))
		{
			/* 说明，就把当前人员做为下一个节点处理人。*/
			DataRow dr = dt.NewRow();
		...
		if (!this.currWn.getHisNode().getGroupStaNDs().equals(town.getHisNode().getGroupStaNDs()))
		{
			...
			if (flowAppType == FlowAppType.Normal)
			{
//				if (SystemConfig.OSDBSrc == Sys.OSDBSrc.Database)
//				{
...
//				if (SystemConfig.OSDBSrc == Sys.OSDBSrc.WebServices)
//				{
...
//				}
			...
			if (flowAppType == FlowAppType.PRJ)
			{
				...
				{
					/* 如果项目组里没有工作人员就提交到公共部门里去找。 */
					sql = "SELECT No FROM Port_Emp WHERE NO IN " + "(SELECT  FK_Emp  FROM " + BP.WF.Glo.getEmpStation() + " WHERE FK_Station IN (SELECT FK_Station FROM WF_NodeStation WHERE FK_Node=" + dbStr + "FK_Node))" + " AND  NO IN " + "(SELECT FK_Emp FROM Port_EmpDept WHERE FK_Dept =" + dbStr + "FK_Dept)";
	...
	*/
	public final DataTable DoIt(Flow fl, WorkNode currWn, WorkNode toWn)
	{
		...
		if (toWn.getHisNode().getHisDeliveryWay() != DeliveryWay.ByCCFlowBPM)
		{
				...
				for (String reWay : reWays)
				{
					...
					if (specName.equals("SpecStations")) //按岗位
					{
						...
						for (String station : stations)
						{
							...
							{
								//排除为空编号
								if (empRow.getValue(0) == null || StringHelper.isNullOrEmpty(empRow.getValue(0).toString()))
								{
									continue;
								}
			...
			//本节点接收人不允许包含上一步发送人 。
			if (this.town.getHisNode().getIsExpSender() == true && re_dt.Columns.size() <= 2)
			{
				/*
