...
public class Bill extends EntityMyPK
{
	...
	*/
	...
	public Map getEnMap()
	{
		...
		rm.Icon = SystemConfig.getCCFlowWebPath() + "WF/Img/FileType/pdf.gif";
		map.AddRefMethod(rm);
	...
	*/
	...
	{
		//String path = BP.Sys.Glo.Request.MapPath(this.getUrl());
		//String url = BP.WF.Glo.getCCFlowAppPath();
		//url = url.substring(0, url.length() - 1);
		//String path = url + this.getUrl();
		String path =  this.getFullPath();
		try {
			PubClass.OpenWordDocV2(path, this.getFK_EmpT() + "打印的" + this.getFK_BillTypeT() + ".doc");
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
	...
	*/
	public final String DoOpenPDF()
	{
		...
		String path = url + this.getUrl();
		try {
			PubClass.OpenWordDocV2(path, this.getFK_EmpT() + "打印的" + this.getFK_BillTypeT() + ".pdf");
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
