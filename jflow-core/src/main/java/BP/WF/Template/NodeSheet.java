...
//	}
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.SetHelperUrl(NodeAttr.SubFlowStartWay, "http://ccbpm.mydoc.io/?v=5404&t=17946"); //增加帮助
			...
			//待办处理模式.
		map.AddDDLSysEnum(NodeAttr.TodolistModel, TodolistModel.QiangBan.getValue(), "待办处理模式", true, true, NodeAttr.TodolistModel, "@0=抢办模式@1=协作模式@2=队列模式@3=共享模式");
		...
		{
			map.getAttrsOfOneVSM().Add(new BP.WF.Template.NodeStations(), new BP.Port.Stations(), BP.WF.Template.NodeStationAttr.FK_Node, BP.WF.Template.NodeStationAttr.FK_Station, DeptAttr.Name, DeptAttr.No, "节点绑定岗位");
			...
			{
				map.getAttrsOfOneVSM().Add(new BP.WF.Template.NodeDepts(), new BP.Port.Depts(), NodeDeptAttr.FK_Node, NodeDeptAttr.FK_Dept, DeptAttr.Name, DeptAttr.No, "节点绑定部门");
			}
		}
		else
		{
				//节点岗位.
			map.getAttrsOfOneVSM().Add(new BP.WF.Template.NodeStations(), new BP.GPM.Stations(), NodeStationAttr.FK_Node, NodeStationAttr.FK_Station, DeptAttr.Name, DeptAttr.No, "节点绑定岗位");
				//判断是否为集团使用，集团时打开新页面以树形展示
			if (Glo.getIsUnit() == true)
			{
				RefMethod rmDept = new RefMethod();
			...
			{
					//节点部门.
				map.getAttrsOfOneVSM().Add(new BP.WF.Template.NodeDepts(), new BP.GPM.Depts(), NodeDeptAttr.FK_Node, NodeDeptAttr.FK_Dept, DeptAttr.Name, DeptAttr.No, "节点绑定部门");
			}
		}


		map.getAttrsOfOneVSM().Add(new BP.WF.Template.NodeEmps(), new BP.Port.Emps(), NodeEmpAttr.FK_Node, NodeEmpAttr.FK_Emp, DeptAttr.Name, DeptAttr.No, "节点绑定接受人");
	...
	*/
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Admin/AttrNode/SortingMapAttrs.jsp?FK_Flow=" + this.getFK_Flow() + "&FK_MapData=ND" + this.getNodeID() + "&t=" + DataType.getCurrentDataTime();
	}
	/** 
	 集团部门树
	...
	*/
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Admin/FlowFormTree.jsp?s=d34&FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getNodeID() + "&RefNo=" + DataType.getCurrentDataTime();
	}
	/** 
	 制度
	...
	*/
	public final String DoCCRole()
	{
		...
		//PubClass.WinOpen(ContextHolderUtils.getResponse(),"./RefFunc/UIEn.jsp?EnName=BP.WF.CC&PK=" + this.NodeID, "抄送规则", "Bill", 800, 500, 200, 300);
		//return null;
	}
	/** 
	 个性化接受人窗口
	...
	*/
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Admin/Listen.jsp?CondType=0&FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getNodeID() + "&FK_Attr=&DirType=&ToNodeID=";
	}
	...
	*/
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Admin/FeatureSetUI.jsp?CondType=0&FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getNodeID() + "&FK_Attr=&DirType=&ToNodeID=";
	}
	public final String DoShowSheets()
	{
		return Glo.getCCFlowAppPath() + "WF/Admin/ShowSheets.jsp?CondType=0&FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getNodeID() + "&FK_Attr=&DirType=&ToNodeID=";
	}
	public final String DoCond()
	{
		return Glo.getCCFlowAppPath() + "WF/Admin/Condition.jsp?CondType=" + CondType.Flow.getValue() + "&FK_Flow=" + this.getFK_Flow() + "&FK_MainNode=" + this.getNodeID() + "&FK_Node=" + this.getNodeID() + "&FK_Attr=&DirType=&ToNodeID=" + this.getNodeID();
	}
	/** 
	 设计傻瓜表单
	...
	*/
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Admin/BindFrms.jsp?ShowType=FlowFrms&FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + this.getNodeID() + "&Lang=CH";
	}
	...
	public final String DoMapData()
	{
		...
		switch (type)
		{
			...
			case FixForm:
			try {
				PubClass.WinOpen(ContextHolderUtils.getResponse(),Glo.getCCFlowAppPath() + "WF/MapDef/MapDef.jsp?PK=ND" + this.getNodeID(), "设计表单", "sheet", 800, 500, 210, 300);
			} catch (IOException e) {
				e.printStackTrace();
			}
				break;
	...
	{
		return Glo.getCCFlowAppPath() + "WF/Admin/Action.jsp?NodeID=" + this.getNodeID() + "&FK_Flow=" + this.getFK_Flow() + "&tk=" + (new Random()).nextDouble();
	}
	/** 
	 单据打印
