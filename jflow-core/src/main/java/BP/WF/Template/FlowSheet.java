...
public class FlowSheet extends EntityNoName
{
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddDDLSysEnum(FlowAttr.TimelineRole, TimelineRole.ByNodeSet.getValue(), "时效性规则", true, true, FlowAttr.TimelineRole, "@0=按节点(由节点属性来定义)@1=按发起人(开始节点SysSDTOfFlow字段计算)");
		...
		///#region 延续流程。
			//延续流程.
		map.AddDDLSysEnum(FlowAttr.CFlowWay, CFlowWay.None.getValue(), "延续流程", true, true, FlowAttr.CFlowWay, "@0=无:非延续类流程@1=按照参数@2=按照字段配置");
		...
		// rm.Icon = "../Img/Btn/View.gif";
		// rm.ClassMethodName = this.ToString() + ".DoOpenRpt()";
		// //rm.Icon = "../Img/Btn/Table.gif";
		// map.AddRefMethod(rm);
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/AttrFlow/DTSBTable.jsp?s=d34&ShowType=FlowFrms&FK_Node=" + Integer.parseInt(this.getNo()) + "01&FK_Flow=" + this.getNo() + "&ExtType=StartFlow&RefNo=" + DataType.getCurrentDataTime();
	}
	/** 
	 批量修改节点属性.
	...
	*/
	...
	{
		return SystemConfig.getCCFlowWebPath() + "WF/Admin/FeatureSetUI.jsp?s=d34&ShowType=FlowFrms&FK_Node=" + Integer.parseInt(this.getNo()) + "01&FK_Flow=" + this.getNo() + "&ExtType=StartFlow&RefNo=" + DataType.getCurrentDataTime();
	}
	public final String DoBindFlowSheet()
	{
		try {
			PubClass.WinOpen(ContextHolderUtils.getResponse(),SystemConfig.getCCFlowWebPath() + "WF/Admin/BindFrms.jsp?s=d34&ShowType=FlowFrms&FK_Node=0&FK_Flow=" + this.getNo() + "&ExtType=StartFlow&RefNo=" + DataType.getCurrentDataTime(), 700, 500);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
	...
	*/
	...
	{
		try {
			PubClass.WinOpen(ContextHolderUtils.getResponse(),SystemConfig.getCCFlowWebPath() + "WF/Admin/AttrFlow/BatchStartFields.jsp?s=d34&FK_Flow=" + this.getNo() + "&ExtType=StartFlow&RefNo=" + DataType.getCurrentDataTime(), 700, 500);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
...
//			gwf.setSDTOfFlow(dttime.toString("yyyy-MM-dd HH:mm:ss"));
			...
			// 向接受人发送一条消息.
			BP.WF.Dev2Interface.Port_SendMsg(currWl.getFK_Emp(), "工作恢复:" + gwf.getTitle(), "工作被:" + WebUser.getNo() + " 恢复." + note, "ReBack" + workid, BP.WF.SMSMsgType.ToDo, this.getNo(), Integer.parseInt(this.getNo() + "01"), workid, 0);
	...
	*/
	...
	{
		//PubClass.WinOpen(ContextHolderUtils.getResponse(),Glo.CCFlowAppPath + "WF/Rpt/OneFlow.jsp?FK_Flow=" + this.getNo() + "&ExtType=StartFlow&RefNo=", 700, 500);
		try {
			PubClass.WinOpen(ContextHolderUtils.getResponse(),Glo.getCCFlowAppPath() + "WF/Comm/Search.jsp?s=d34&EnsName=BP.WF.Data.GenerWorkFlowViews&FK_Flow=" + this.getNo() + "&ExtType=StartFlow&RefNo=", 700, 500);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
	...
	*/
	...
	{
		try {
			PubClass.WinOpen(ContextHolderUtils.getResponse(),Glo.getCCFlowAppPath() + "WF/Admin/FlowFormTree.jsp?s=d34&FK_Flow=" + this.getNo() + "&ExtType=StartFlow&RefNo=" + DataType.getCurrentDataTime(), 700, 500);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
	...
	*/
	public final String DoSetStartFlowDataSources()
	{
		...
		return Glo.getCCFlowAppPath() + "WF/MapDef/MapExt.jsp?s=d34&FK_MapData=ND" + flowID + "&ExtType=StartFlow&RefNo=";
	}
