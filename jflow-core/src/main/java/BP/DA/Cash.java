...
public class Cash
{
	...
	public static void EnsDataSet(String clName, Entities obj)
	{
		if (obj.size() == 0)
		{
			...
			// obj.GetNewEntity.EnMap.PhysicsTable);
		}
	...
	public static Object GetObj(String key, Depositary where)
	{
		...
		{
			throw new RuntimeException("您没有把(" + key
					+ ")放到session or application 里面不能找出他们.");
		}
		if (SystemConfig.getIsBSsystem())
		{
	...
	public static void AddObj(String key, Depositary where, Object obj)
	{
		if (key == null)
		{
			throw new RuntimeException("您需要为obj=" + obj.toString()
					+ ",设置为主键值。key");
		}
		...
		{
			throw new RuntimeException("您没有把(" + key
					+ ")放到 session or application 里面设置他们.");
		}
		// if (Cash.IsExits(key, where))
		// return;
	...
	{
		String val = (String) ((getBill_Cash().get(cfile) instanceof String) ? getBill_Cash()
				.get(cfile) : null);
		if (isCheckCash == true)
		{
			val = null;
		...
		if (StringHelper.isNullOrEmpty(val))
		{
			...
			{
				file = SystemConfig.getPathOfDataUser() + "CyclostyleFile/"
						+ cfile;
			}
			...
			{
				val = ConvertTools
						.StreamReaderToStringConvert(file, "us-ascii");
			} catch (Exception ex)
			{
				throw new RuntimeException("@读取单据模板时出现错误。cfile=" + cfile
						+ " @Ex=" + ex.getMessage());
			}
	...
	public static String[] GetBillParas(String cfile, String ensStrs,
			Entities ens)
	{
		String[] paras = (String[]) ((getBill_Cash().get(cfile + "Para") instanceof String[]) ? getBill_Cash()
				.get(cfile + "Para") : null);
		if (paras != null)
		{
			return paras;
	...
	public static String[] GetBillParas_Gener(String cfile, Attrs attrs)
	{
		...
		if (haveError)
		{
			String myfile = SystemConfig.getPathOfDataUser()
		...
		{
			// String s =
			// "@帮助信息:用记事本打开它模板,找到红色的字体. 把尖括号内部的非法字符去了,例如:《|f0|fs20RDT.NYR|lang1033|kerning2》，修改后事例：《RDT.NYR》。@注意把双引号代替单引号，竖线代替反斜线。";
			// throw new
