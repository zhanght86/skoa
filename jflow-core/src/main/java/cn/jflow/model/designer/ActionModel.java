					...
					if (myEvent != null && myEvent.equals(xml.getNo())) {
						...
						this.Pub1
								.append(BaseModel.AddLi(String
										.format("<div><a href='"
												+ basePath
												+ "WF/Admin/Action.jsp?NodeID=%1$s&Event=%2$s&FK_Flow=%3$s&tk=%5$s&FK_MapData=%6$s'><span class='nav'><img src='"+Glo.getCCFlowAppPath()+"WF/Img/Event.png' border=0/>" + msg + "%4$s</span></a></div>%5$s",
					...
					if (myEvent != null && myEvent.equals(xml.getNo())) {
						...
						this.Pub1
								.append(BaseModel.AddLi(String
										.format("<div><a href='"
												+ basePath
												+ "WF/Admin/Action.jsp?NodeID=%1$s&Event=%2$s&FK_Flow=%3$s&MyPK=%4$s&tk=%7$s'><span class='nav'><img src='"+Glo.getCCFlowAppPath()+"WF/Img/Event.png' border=0/>" + msg + "%5$s</span></a></div>%6$s",
												this.getNodeID(), xml.getNo(),
												this.getFK_Flow(), nde
														.getMyPK(), xml
														.getName(), "\r\n",
