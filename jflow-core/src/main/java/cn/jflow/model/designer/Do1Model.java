...
import BP.Sys.Frm.FieldGroupXml;
import BP.Sys.Frm.FieldGroupXmls;
...
//				}
			...
			} else if (doType.equals("AddFG")) // 执行一个插入列组的命令.
			{
				...
				WinClose();
			} else if (doType.equals("FrameDoUp")) {
				MapFrame frame1 = new MapFrame(getMyPK());
				if (frame1.getRowIdx() > 0) {
					frame1.setRowIdx(frame1.getRowIdx() - 1);
					frame1.Update();
				}
				WinClose();
				return;
			} else if (doType.equals("FrameDoDown")) {
				MapFrame frame2 = new MapFrame(getMyPK());
				if (frame2.getRowIdx() < 10) {
					frame2.setRowIdx(frame2.getRowIdx() + 1);
					frame2.Update();
				}
				WinClose();
				return;
