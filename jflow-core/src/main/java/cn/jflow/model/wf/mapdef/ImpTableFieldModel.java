	...
	public void loadPage() {
		...
		if (this.getStep() == 1) {
			...
			for (SFDBSrc item : ens.ToJavaList()) {
				...
				Pub1.append(AddLi("<div><a href='ImpTableField.jsp?Step=2&FK_MapData="
						+ this.getFK_MapData()
						+ "&FK_SFDBSrc="
						+ item.getNo()
						+ "'><span class='nav'>"
		...
		if (this.getStep() == 2) {
			...
			this.Pub1
					.append("<div class='easyui-layout' style='height:800px' data-options=\"fit:true\">");
			...
			for (DataRow dr : dt.Rows) {
				String url = "ImpTableField.jsp?Step=" + this.getStep() + "&FK_MapData="
						+ this.getFK_MapData() + "&FK_SFDBSrc="
						+ this.getFK_SFDBSrc() + "&STable="
						+ dr.getValue("No").toString();
				if (dr.getValue("No").toString().equals(this.getSTable())) {
					this.Pub1.append("<li><font color=red><b>"
							+ dr.getValue("Name") + "</font></b></li>");
				} else {
					this.Pub1.append("<li><a href='" + url + "' >"
							+ dr.getValue("Name") + "</a></li>");
				}
			}
			...
			this.Pub1.append(AddTDGroupTitle(
					"style='width:40px;text-align:center'", cb.toString()));
			this.Pub1.append(AddTDGroupTitle("序"));
			this.Pub1.append(AddTDGroupTitle("字段名"));
			this.Pub1.append(AddTDGroupTitle("中文描述"));
			this.Pub1.append(AddTDGroupTitle(
					"style='width:80px;text-align:center'", "类型"));
			this.Pub1.append(AddTDGroupTitle(
					"style='width:50px;text-align:center'", "最大长度"));
...
//			boolean isHave = false;
			...
			if(tableColumns!=null && tableColumns.Rows.size()>0){
				for (DataRow dr : tableColumns.Rows) {
					cb = new CheckBox();
					cb.setId("CB_Col_" + dr.getValue("name"));
					if (StringHelper.isNullOrEmpty(this.getSColumns())) {
						cb.setChecked(false);
					} else {
						cb.setChecked(this.getSColumns().contains(dr.getValue("name") + ","));
					}

					// 如果已经有该字段，就放弃.
					if (attrs.Contains(MapAttrAttr.KeyOfEn, dr.getValue("name").toString())) {
						continue;
					}
					...
					this.Pub1.append(AddTD(dr.getValue("Desc").toString()));
					this.Pub1.append(AddTD(dr.getValue("type").toString()));
					this.Pub1.append(AddTD(dr.getValue("length").toString()));
					this.Pub1.append(AddTREnd());
				}
			}
			...
			this.Pub1.append(String.format("<a href='%1$s?Step=1&FK_MapData=%2$s' class='easyui-linkbutton'>上一步</a>",
									Glo.getCCFlowAppPath()+"WF/MapDef/ImpTableField.jsp",
									this.getFK_MapData()));
			this.Pub1.append(AddBR());
			this.Pub1.append(AddBR());
