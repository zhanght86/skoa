...
//			qo.addRightBracket();
	...
	public static void DataPanelDtl(StringBuilder sb, Entities ens, int pageIdx) {
		...
		try {
			...
			tableStyle = UIRowStyleGlo.forValue(ens
					.GetEnsAppCfgByKeyInt("UIRowStyleGlo")); // 界面风格。

			 WinCardH = ens.GetEnsAppCfgByKeyInt("WinCardH", 500); // 弹出窗口高度
		     WinCardW = ens.GetEnsAppCfgByKeyInt("WinCardW", 820); // 弹出窗口宽度
