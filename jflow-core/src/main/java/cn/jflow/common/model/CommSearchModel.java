...
public class CommSearchModel extends BaseModel {
	...
	public void init() {
		...
		if (!getHisEn().getHisUAC().IsInsert)
			isEdit = false;
		if (isEdit) {
			int h = BP.Sys.EnsAppCfgs.GetValInt(getEnsName(), "WinCardH");
			if (h == 0) {
				h = 500;
			}
			int w = BP.Sys.EnsAppCfgs.GetValInt(getEnsName(), "WinCardW");
			if (w == 0) {
				w = 800;
			}
			/*toolBar1.AddLab(
			...
			String js = "javascript:ShowEn('./RefFunc/UIEn.jsp?EnsName=" + this.getEnsName() + "','cd','" + h + "' , '" +w + "');";
