...
*/
			...
			case ReturnSta:
				...
				if (rws.size() != 0) {
					...
					for(int i=0;i<rws.size();i++){
						...
						msgInfo += "<fieldset width='100%' ><legend>&nbsp; 来自节点:" + rw.getReturnNodeName() + " 退回人:" + rw.getReturnerName() + "  " + rw.getRDT() + "&nbsp;<a href='" + Glo.getCCFlowAppPath() + "DataUser/ReturnLog/" + fk_flow + "/" + rw.getMyPK() + ".htm' target=_blank>工作日志</a></legend>";
						msgInfo += rw.getNoteHtml();
						msgInfo += "</fieldset>";
					}
		...
		if (WebUser.getUserWorkDev() == UserWorkDev.Mobile) {
			try {
				response.sendRedirect("/CCMobile/MyFlow.jsp?WorkID=" + workId + "&FK_Flow=" + fk_flow + "&FK_Node=" + fk_node + "&FID=" +fid);
			} catch (IOException e) {
				e.printStackTrace();
			}
			return;
	...
	public final void BindWork(Node nd, Work wk)
	{
		...
		switch (nd.getHisNodeWorkType())
		{
			...
			case WorkHL:
				if (this.getFID() != 0 && this.getFID() != this.getWorkID())
				{
					...
					this.FlowMsg.append(rw.getNoteHtml());
		...
		switch (nd.getHisFormType())
		{
			...
			case FixForm:
				...
				else if (nd.getHisFormType().getValue() == NodeFormType.FixForm.getValue())
				{
					/* 仅仅只有节点表单的情况。 */
					/*傻瓜表单*/
