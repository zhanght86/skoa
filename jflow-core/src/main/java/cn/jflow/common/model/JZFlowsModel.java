...
//             	 continue;
			...
			//顶级目录跳过展示,要求流程设计器中流程类别,顶级类别下不要新建流程;若可以再顶级类别下建流程,则需要注解掉下面代码.
			//if("0".equals(en.getParentNo())||"".equals(en.getParentNo())||"".equals(en.getNo()))
			//	 continue;
			//如果该目录下流程数量为空,跳过.
...
//            		 continue;
				...
				if (fl.getIsBatchStart()){
					extUrl = "<a  class='btnList' href=\"javascript:winOpen('"+basePath+"/WF/BatchStart.jsp?FK_Flow="+fl.getNo()+"','批量发起');\" >批量发起</a>|<a  class='btnList' href=\"javascript:winOpen('"+basePath+"/WF/Rpt/Search.jsp?RptNo=ND" + Integer.parseInt(fl.getNo()) + "MyRpt&FK_Flow=" + fl.getNo() + "','查询')\">查询</a>|<a  class='btnList' href=\"javascript:winOpen('"+basePath+"/WF/Admin/CCFlowDesigner/Truck.html?FK_Flow=" + fl.getNo() + "&DoType=Chart&T=" + timeKey + "','流程图');\"  >图</a>";
				}else{
					extUrl = "<a  class='btnList' href=\"javascript:winOpen('"+basePath+"/WF/Rpt/Search.jsp?RptNo=ND" + Integer.parseInt(fl.getNo()) + "MyRpt&FK_Flow=" + fl.getNo() + "','查询');\">查询</a>|<a  class='btnList' href=\"javascript:winOpen('"+basePath+"/WF/Admin/CCFlowDesigner/Truck.html?FK_Flow=" + fl.getNo() + "&DoType=Chart&T=" + timeKey + "','流程图');\" >图</a>";
				}

				if (isHaveIt){
					if (Glo.getIsWinOpenStartWork() == 1)
						this.ui.append(BaseModel.AddLi("<a  class='btnList' href=\"javascript:winOpen('"+basePath+"/WF/MyFlow.jsp?FK_Flow=" + fl.getNo() + "&FK_Node=" + Integer.parseInt(fl.getNo()) + "01&T=" + timeKey + "','" + fl.getName() + "');\" >" + fl.getName() + "</a> - " + extUrl));
					else if (Glo.getIsWinOpenStartWork() == 2)
						this.ui.append(BaseModel.AddLi("<a  class='btnList' href=\"javascript:winOpen('"+basePath+"/WF/OneFlow/MyFlow.jsp?FK_Flow=" + fl.getNo() + "&FK_Node=" + Integer.parseInt(fl.getNo()) + "01&T=" + timeKey + "','" + fl.getName() + "');\" >" + fl.getName() + "</a> - " + extUrl));
					else
						this.ui.append(BaseModel.AddLi("<a  class='btnList' href=\"javascript:winOpen('"+basePath+"/WF/MyFlow.jsp?FK_Flow=" + fl.getNo() + "&FK_Node=" + Integer.parseInt(fl.getNo()) + "01','" + fl.getName() + "')\" >" + fl.getName() + "</a> - " + extUrl));
				}else{
