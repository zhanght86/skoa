	...
	public final void loadPage() throws IOException
	{
		...
		if (this.getRefNo() != null)
		{
			...
			if (getParameter("Type") != null)
			{
				...
				{
					ContextHolderUtils.getResponse().sendRedirect("../WAP/MyFlow.jsp?FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + Integer.parseInt(this.getFK_Flow()) + "01");
				}
				else
				{
					ContextHolderUtils.getResponse().sendRedirect("../MyFlow.jsp?FK_Flow=" + this.getFK_Flow() + "&FK_Node=" + Integer.parseInt(this.getFK_Flow()) + "01");
				}
			}
			else
			{
		...
		try
		{
			switch (nd.getHisDeliveryWay())
			{
				...
				case ByDeptAndStation:
					...
					if (BP.WF.Glo.getOSModel() == OSModel.OneOne){
						sql.append("(SELECT FK_Emp FROM Port_EmpDept WHERE FK_Dept IN ");
...
//			appendPub(AddTD("<a href='./../Port.jsp?DoWhat=StartLigerUI&UserNo=" + emp.getNo() + "&FK_Flow=" + this.getFK_Flow() + "&Lang=" +  WebUser.getSysLang() + "&Type=" + getParameter("Type") + "'  ><img src='"+Glo.getCCFlowAppPath()+"WF/Img/ie.gif' border=0 />LigerUI模式</a>"));
			//appendPub(AddTD("<a href='./../Port.aspx?DoWhat=Start&UserNo=" + emp.No + "&FK_Flow=" + this.FK_Flow + "&Lang=" + BP.Web.WebUser.SysLang + "&Type=" + getParameter(["Type"] + "'  ><img src='"+Glo.getCCFlowAppPath()+"WF/Img/ie.gif' border=0 />Blog模式</a>");                
			//appendPub(AddTD("<a href='./../Port.aspx?DoWhat=StartSmallSingle&UserNo=" + emp.No + "&FK_Flow=" + this.FK_Flow + "&Lang=" + BP.Web.WebUser.SysLang + "&Type=" + getParameter(["Type"] + "'  ><img src='"+Glo.getCCFlowAppPath()+"WF/Img/ie.gif' border=0 />Internet Explorer</a>");
			//appendPub(AddTD("<a href=\"javascript:WinOpen('TestFlow.aspx?RefNo=" + emp.No + "&FK_Flow=" + this.FK_Flow + "&Lang=" + BP.Web.WebUser.SysLang + "&Type=" + getParameter(["Type"] + "&IsWap=1','470px','600px','" + emp.No + "');\"  ><img src='./../Img/Mobile.gif' border=0 width=25px height=18px />Mobile</a> ");
