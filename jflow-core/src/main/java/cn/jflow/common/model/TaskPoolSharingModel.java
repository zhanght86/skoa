...
public class TaskPoolSharingModel extends BaseModel {
	...
	public TaskPoolSharingModel(HttpServletRequest request,
...
//		FK_Flow = getRequest().getParameter("FK_Flow");
	...
	private String BindList(){
        ...
        rtnHtml.append(AddTable("align='left' width='100%'"));
        rtnHtml.append(AddCaption("任务池-<a href='TaskPoolApply.jsp' >申请下来工作</a>"));
        ...
        rtnHtml.append(AddTDTitle1("ID"));
        rtnHtml.append(AddTDTitle1("标题"));

        if (!"FlowName".equals(GroupBy)){
        	rtnHtml.append(AddTDTitle1("<a href='" + getPageID() + ".jsp?GroupBy=FlowName" + extStr + "&T=" + timeKey + "' >流程</a>"));
        }
        if (!"NodeName".equals(GroupBy)){
        	rtnHtml.append(AddTDTitle1("<a href='" + getPageID() + ".jsp?GroupBy=NodeName" + extStr + "&T=" + timeKey + "' >节点</a>"));
        }
        if (!"StarterName".equals(GroupBy)){
        	rtnHtml.append(AddTDTitle1("<a href='" + getPageID() + ".jsp?GroupBy=StarterName" + extStr + "&T=" + timeKey + "' >发起人</a>"));
        }

        if (isPRI){
        	rtnHtml.append(AddTDTitle1("<a href='" + getPageID() + ".jsp?GroupBy=PRI" + extStr + "&T=" + timeKey + "' >优先级</a>"));
        }

        rtnHtml.append(AddTDTitle1("发起日期"));
        rtnHtml.append(AddTDTitle1("接受日期"));
        rtnHtml.append(AddTDTitle1("期限"));
        rtnHtml.append(AddTDTitle1("状态"));
        rtnHtml.append(AddTDTitle1("备注"));
        ...
        for(String gVal : gVals) {
            ...
            if ("Rec".equals(GroupBy)){
            	rtnHtml.append(AddTD1("colspan='" + colspan + "' class='TRSum' onclick=\"GroupBarClick('" + appPath + "','" + gIdx + "')\" ", "<div style='text-align:left; float:left' ><img src='"+appPath+"WF/Img/Min.gif' alert='Min' id='Img" + gIdx + "' border='0' />&nbsp;<b>" + gVal.replace(",", "") + "</b>"));
            }else{
            	rtnHtml.append(AddTD1("colspan='" + colspan + "' class='TRSum' onclick=\"GroupBarClick('" + appPath + "','" + gIdx + "')\" ", "<div style='text-align:left; float:left' ><img src='"+appPath+"WF/Img/Min.gif' alert='Min' id='Img" + gIdx + "' border='0' />&nbsp;<b>" + gVal.replace(",", "") + "</b>"));
            }
            rtnHtml.append(AddTREnd());
            ...
            for(int drNum=0;drNum<dt.Rows.size();drNum++){
            	...
            	if(Glo.getIsWinOpenEmpWorks()){
            		if (isRead == 0){
            			rtnHtml.append(AddTD1("Class='TTD' onclick=\"SetImg('" + appPath + "','I" + gIdx + "_" + i + "')\"", "<a href=\"javascript:WinOpenIt('" + appPath + "WF/MyFlow.jsp?FK_Flow=" + FK_Flow + "&FK_Node=" + FK_Node + "&FID=" + FID + "&WorkID=" + WorkID + "&IsRead=0&T=" + timeKey + "','" + WorkID + "');\" ><img class=Icon src='"+appPath+"WF/Img/Mail_UnRead.png' id='I" + gIdx + "_" + i + "' />" + Title + "</a>"));
            		}else{
            			rtnHtml.append(AddTD("<a href=\"javascript:WinOpenIt('"+appPath+"WF/MyFlow.jsp?FK_Flow=" + FK_Flow + "&FK_Node=" + FK_Node + "&FID=" + FID + "&WorkID=" + WorkID + "','" + WorkID + "');\"  ><img src='"+appPath+"WF/Img/Mail_Read.png' id='I" + gIdx + "_" + i + "' class=Icon />" + Title + "</a>"));
            		}
            	}else{
            		if (isRead == 0){
            			rtnHtml.append(AddTD1("Class=TTD onclick=\"SetImg('" + appPath + "','I" + gIdx + "_" + i + "')\" ", "<a href=\"MyFlow" + getPageSmall() + ".jsp?FK_Flow=" + FK_Flow + "&FK_Node=" + FK_Node + "&FID=" + FID + "&WorkID=" + WorkID + "&IsRead=0&T=" + timeKey + "\" ><img class=Icon src='"+appPath+"WF/Img/Mail_UnRead.png' id='I" + gIdx + "_" + i + "' />" + Title + "</a>"));
            		}else{
            			rtnHtml.append(AddTD1("Class=TTD ", "<a href=\"MyFlow" + getPageSmall() + ".jsp?FK_Flow=" + FK_Flow + "&FK_Node=" + FK_Node + "&FID=" + FID + "&WorkID=" + WorkID + "&T=" + timeKey + "\" ><img class=Icon src='"+appPath+"WF/Img/Mail_Read.png' id='I" + gIdx + "_" + i + "' />" + Title + "</a>"));
            		}
            	}
            	if (!"FlowName".equals(GroupBy)){
	...
	private String BindList_PRI(){
        ...
        for(Object obj : ses.toArray()) {
            ...
            gIdx++;
            rtnHtml.append(AddTR());
            rtnHtml.append(AddTD("colspan='" + colspan + "' class='TRSum' onclick=\"GroupBarClick('" + appPath + "','" + gIdx + "')\" ", "<div style='text-align:left; float:left' ><img src='"+appPath+"WF/Style/Min.gif' alert='Min' id='Img" + gIdx + "' />&nbsp;<img src='"+appPath+"WF/Img/PRI/" + se.getIntKey() + ".png'  class='Icon' />"));
            rtnHtml.append(AddTREnd());

            String pri = String.valueOf(se.getIntKey());
            for(int drNum=0;drNum<dt.Rows.size();drNum++){
            	DataRow dr = dt.Rows.get(drNum);
            	if (!pri.equals(dr.get("pri").toString())){
                    continue;
            	}
            	String sdt = dr.get("sdt").toString();
            	rtnHtml.append(AddTR("ID='" + gIdx + "_" + i + "'"));
            	i++;
            	rtnHtml.append(AddTDIdx(i));
            	if(Glo.getIsWinOpenEmpWorks()){
            		rtnHtml.append(AddTD("<a href=\"javascript:WinOpenIt('"+appPath+"WF/MyFlow.jsp?FK_Flow=" + dr.get("fk_flow") + "&FK_Node=" + dr.get("fk_node") + "&FID=" + dr.get("fid") + "&WorkID=" + dr.get("workid") + "&T=" + timeKey + "','" + dr.get("workid") + "');\"  >" + dr.get("title").toString() + "</a>"));
            	}else{
