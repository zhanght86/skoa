	...
	public void run(){
		new Thread(new Runnable() {
			...
			public void run() {
				...
				ISAutoNodeDTS = SystemConfig.GetValByKeyBoolen("ISAutoNodeDTS", false);
				AutoNodeDTSTimeSpanMinutes = SystemConfig.GetValByKeyInt("AutoNodeDTSTimeSpanMinutes", 1);
				Log.DefaultLogWriteLineInfo("读取服务配置信息：[CB_IsWriteToCCIM]"+CB_IsWriteToCCIM+"[ISAutoNodeDTS]"+ISAutoNodeDTS+"[AutoNodeDTSTimeSpanMinutes]"+AutoNodeDTSTimeSpanMinutes);
				if(ISAutoNodeDTS){
					RunIt();
				}
			}
	...
	public final void DTS_Flow(BP.WF.Flow fl) {
		...
		for (DataRow dr : dtMain.Rows) {
			...
			try {
				...
				//this.SetText("@" + fl.getName() + ",第" + idx + "条,发起人员:" + WebUser.No + "-" + WebUser.getName() + "已完成.\r\n" + msg);
				Log.DefaultLogWriteLineWarning("@" + fl.getName() + ",第" + idx + "条,发起人员:" + WebUser.getNo() + "-" + WebUser.getName() + "已完成.\r\n" + msg);
			} catch (RuntimeException ex) {
				//this.SetText("@" + fl.getName() + ",第" + idx + "条,发起人员:" + WebUser.No + "-" + WebUser.getName() + "发起时出现错误.\r\n" + ex.getMessage());
