...
import cn.jflow.common.model.DoPortModel;
import cn.jflow.common.util.ContextHolderUtils;

@WebService(endpointInterface = "cn.jflow.controller.wf.admin.xap.service.CCFormSoap")
public class CCFormSoapImpl implements CCFormSoap{
    ...
    public String AttachmentFiles(String WorkID, String FK_Node, String FK_FrmAttachment, String UserNo, String UserName, byte[] FileByte, String fileName, String MyNote, String sort)
    {
        try
        {
            ...
            //如果有上传类型限制，进行判断格式
            if (athDesc.getExts() == "*.*" || athDesc.getExts() == "")
            {
                /*任何格式都可以上传*/
            }
            else
    ...
    {
    	//创建临时目录.
        String pathTemp = BP.Sys.SystemConfig.getPathOfTemp();
        ...
        ds.readXmls(filePath);

        String strs = null;
        strs = FormatToJson.ToJson1(ds);
        return strs;
    }
    /// <summary>
    /// 执行sql
    ...
    /// <param name="sql"></param>
    /// <returns></returns>
    public String RunSQLReturnTable2DataTable(String sql, String UserNo, String SID)
    {
    	if (!(WebUser.CheckSID(UserNo, SID) && Glo.getIsAdmin(UserNo))){
			throw new RuntimeException("@权限已丢失,请重新登录.");
    	}
    ...
    /// <param name="sql"></param>
    /// <returns></returns>
    public float RunSQLReturnValFloat(String sql, String UserNo, String SID) throws Exception
    {
    	if (!(WebUser.CheckSID(UserNo, SID) && Glo.getIsAdmin(UserNo))){
			throw new RuntimeException("@权限已丢失,请重新登录.");
    	}
...
//        }
    ...
    public String LoadFrmTemplete(byte[] fileByte, String fk_mapData, boolean isClear)
    {
        try
        {
            ...
            String file =fk_mapData + ".xml";
            UploadFile(fileByte, file);
            String path = ContextHolderUtils.getRequest().getSession().getServletContext().getRealPath("/")+"/DataUser/Temp/" + file;
    ...
    public String SaveEn(String vals)
    {
        ...
        try
        {
            ...
            if(enName.equals("BP.Sys.FrmAttachment")){
            	enName="BP.Sys.Frm.FrmAttachment";
            }
...
//            System.out.println(json);
    ...
    public String SaveFrm(String fk_mapdata, String xml, String sqls, String mapAttrKeyName)
    {
        ...
        for(DataTable dt:ds.Tables)
        {
            try
            {
            	 if ("wf_node".equalsIgnoreCase(dt.TableName) && dt.Columns.size() >0 && dt.Rows.size() ==1)
                 {
            		 if (dt.Rows.get(0).get("FWC_X").equals(""))
            		 {
            			 dt.Rows.get(0).put("FWC_X", 0);
            		 }
            		 if (dt.Rows.get(0).get("FWC_Y").equals(""))
            		 {
            			 dt.Rows.get(0).put("FWC_Y", 0);
            		 }
            		 if (dt.Rows.get(0).get("FWC_W").equals(""))
            		 {
            			 dt.Rows.get(0).put("FWC_W", 0);
            		 }
            		 if (dt.Rows.get(0).get("FWC_H").equals(""))
            		 {
            			 dt.Rows.get(0).put("FWC_H", 0);
            		 }
            		 if (dt.Rows.get(0).get("SF_X").equals(""))
            		 {
            			 dt.Rows.get(0).put("SF_X", 0);
            		 }
            		 if (dt.Rows.get(0).get("SF_Y").equals(""))
            		 {
            			 dt.Rows.get(0).put("SF_Y", 0);
            		 }
            		 if (dt.Rows.get(0).get("SF_W").equals(""))
            		 {
            			 dt.Rows.get(0).put("SF_W", 0);
            		 }
            		 if (dt.Rows.get(0).get("SF_H").equals(""))
            		 {
            			 dt.Rows.get(0).put("SF_H", 0);
            		 }
                 }
            	 
                str += CCFormUtil.SaveDT(dt);
//                if (dt.TableName.toLowerCase() == "wf_node".toUpperCase() && dt.Columns.size() >0 && dt.Rows.size() ==1)
//                {
//                    /* 更新审核组件状态. */
...
//        sqls += sql;
        ...
        RunSQLs(sqls,null,null);
        
        
        MapData md = new MapData();
        md.setNo(fk_mapdata);
        DataTable mapdata = ds.hashTables.get("Sys_MapData");
        if (mapdata != null && mapdata.Rows.size() == 1)
        {
            DataRow mdr = mapdata.Rows.get(0);
            md.RetrieveFromDBSources();

            md.setName(mdr.get("Name").toString());

            Float tmp = 0.00f;
            tmp=Float.parseFloat(mdr.get("FrmH").toString());
            
            md.setFrmH(tmp <= 0 ? md.getFrmH() : tmp);

            tmp=Float.parseFloat(mdr.get("FrmW").toString());
            md.setFrmW(tmp <= 0 ? md.getFrmW() : tmp);

            tmp=Float.parseFloat(mdr.get("MaxEnd").toString());
            md.setMaxEnd(tmp <= 0 ? md.getMaxEnd() : tmp);

            tmp=Float.parseFloat(mdr.get("MaxLeft").toString());
            md.setMaxLeft(tmp <= 0 ? md.getMaxLeft() : tmp);

            tmp=Float.parseFloat(mdr.get("MaxRight").toString());
            md.setMaxRight(tmp == 0 ? md.getMaxRight() : tmp);

            tmp=Float.parseFloat(mdr.get("MaxTop").toString());
            md.setMaxTop(tmp == 0 ? md.getMaxTop() : tmp);
        ...
        ////移除缓存
        md.UpdateVer();
        SystemConfig.DoClearCash_del();
        //md.getHisGEEn().CheckPhysicsTable();

        // 备份文件
        DoPortModel.WriteToXmlMapData(fk_mapdata);//, false);
