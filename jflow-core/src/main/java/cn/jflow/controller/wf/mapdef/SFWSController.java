...
import BP.WF.Glo;
@Controller
@RequestMapping("/WF/MapDef/sfws")
@Scope("request")
public class SFWSController{
	...
	@RequestMapping(value = "/SFWSAdd", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/SFWSSave", method = RequestMethod.POST)
	public void btn_Save_Click(HttpServletRequest request, HttpServletResponse response){
		try{	
			...
			if (main.getFK_SFDBSrc().length() == 0) {
				BaseModel.Alert("必须选择数据源");
			}

			if (main.getTableDesc().length() == 0) {
				BaseModel.Alert("必须选择接口");
			}

			if (main.getNo().length() == 0) {
				BaseModel.Alert("接口英文名称不能为空");
			}

			if (main.getName().length() == 0) {
				BaseModel.Alert("接口中文名称不能为空");
			}

			/*if (main.getrestype().getLength() == 0) {
				throw new RuntimeException("选择接口返回值类型");
			}*/
			...
			//设置它的数据源类型.
			main.setSrcType(SrcType.WebServices);
